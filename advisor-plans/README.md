# Caide Roadmap Execution Controller
...
- `P2-ACP-01`: the official TypeScript SDK owns framing, JSON-RPC dispatch/correlation, validation,
  ...
  and workspace package are deleted. `AcpErrors.ts` and `AcpExtensions.ts` retain only Caide-owned
...
- `PRUNE-09` — `ProviderService` no longer reads `CAIDE_PROVIDER_RUNTIME_IDLE_STOP_MS` twice through
...
cancellation, handler dispatch, and NDJSON encoding. Caide retains Effect lifecycle, bounded
