...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `caide` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@caide/shared/git`, `@caide/shared/DrainableWorker`) — no barrel index.
