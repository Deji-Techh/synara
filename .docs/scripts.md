...
- Dev commands default `CAIDE_HOME` to `~/.caide`, which keeps dev state under `~/.caide/dev`.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --home-dir ~/.caide-2`
...
- Desktop production windows load the bundled UI from `caide://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `caide` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `CAIDE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `CAIDE_DEV_INSTANCE`)
- Example: `CAIDE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `CAIDE_PORT_OFFSET` to a numeric offset.
