...
Caide Canary is a frozen local build of a chosen remote Git ref. It is intentionally separate
from both Caide Stable and the HMR development process.

## Isolation

- App name: `Caide Canary`
- Bundle ID: `com.emanueledipietro.caide.canary`
- Desktop origin: `caide-canary://app`
- Caide data: `~/.caide-canary`
- Electron profile: `caide-canary`
- Managed source: `~/.cache/caide-canary/source`
- Runtime log: `~/.caide-canary/canary.log`
...
```bash
bun run canary:setup -- --ref codex/caide-canary
```
...
```bash
CAIDE_CANARY_HOME=/path/to/data \
CAIDE_CANARY_SOURCE=/path/to/source \
bun run canary:update
```
...
```bash
env -u CAIDE_AUTH_TOKEN \
  CAIDE_DEV_INSTANCE=my-feature \
  bun run dev -- --home-dir ./.caide/dev-my-feature
```
