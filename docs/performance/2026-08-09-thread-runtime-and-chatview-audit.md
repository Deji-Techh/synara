...
- Electron owns one main process, one primary browser renderer, Chromium GPU/utility
  processes, and one Caide backend child. These are application infrastructure,
...
belong to that provider. Caide's additional cost is its Electron/backend process
...
The page exposes `window.__caidePerf.snapshot()`, `resetMetrics()`,
...
```sh
env -u CAIDE_AUTH_TOKEN CAIDE_PORT_OFFSET=4117 CAIDE_NO_BROWSER=1 \
  bun run dev -- --home-dir ./.caide-perf-audit --port 59231
```
...
| Scenario                                             | Caide CPU / memory                                                                            | Provider CPU / memory                                                              | Processes                                                               | Renderer/frame evidence                                                    | Observation                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Packaged backend, five-second sample                 | 269 MB physical, 387 MB peak; 4,110/4,211 main-thread samples in `kevent`                     | Not included                                                                       | One Caide backend                                                       | n/a                                                                        | The observed 7.9% was transient, not sustained backend CPU.                                                                                                                                                                                                                                           |
...
| Equivalent task through Caide                        | About 93 MB dev backend at the sampled instant                                                | Approximately 156 MB for the provider pair, approximately 0% CPU after completion  | Caide backend/browser plus provider tree                                | Response completed in 23 seconds                                           | The provider RSS was essentially the same as direct; Caide adds its own backend/browser processes and event pipeline. Model differed (GPT-5.5 through Caide versus GPT-5.6 direct), so wall time is not a valid latency comparison.                                                                   |
...
### 1. Durable journal safety polling (medium/high idle impact, Caide-owned)
...
### 2. Unrelated streaming updates scanned retained thread entries (medium under concurrency, Caide renderer-owned)
...
### 3. Infinite ChatView working shimmer (medium steady renderer impact, Caide-owned; retained by design)
...
### 4. Claude lifecycle lock map retained every historical thread (low per thread, unbounded, Caide-owned)
...
### 5. Per-event durable journal cost (medium/high while streaming, Caide-owned; measured but not changed)
...
a transaction. This maps provider chunk frequency directly to Caide SQLite work.
...
- Shutdown PID audit found zero remaining descendants from the isolated Caide tree.
- `bun fmt`: passed.
