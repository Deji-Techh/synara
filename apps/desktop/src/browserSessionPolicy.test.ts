...
vi.mock("electron", () => ({
  app: {
    ...
    userAgentFallback:
      "Mozilla/5.0 AppleWebKit/537.36 Chrome/140.0.0.0 Electron/40.0.0 Safari/537.36 Caide/0.5.5",
  },
  session: {
...
describe("BrowserSessionPolicy", () => {
  ...
  it("replaces identity headers case-insensitively without Electron product tokens", () => {
    ...
    expect(normalizedHeaders["user-agent"]).not.toMatch(/Electron|Caide/iu);
  ...
  it("applies the same derived identity to the partition, tabs, and popups", () => {
    ...
    expect(partitionUserAgent).not.toMatch(/Electron|Caide/iu);
  ...
  it("keeps file URLs user-facing while assigning an opaque runtime origin", () => {
    ...
    expect(runtimeUrl).toMatch(/^caide-local-preview:\/\/[a-f0-9]+\/index\.html$/u);
