...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    CAIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    CAIDE_CLAUDE_KEEPALIVE: "0",
    CAIDE_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    CAIDE_HOME: input.homeDir,
    CAIDE_HOST: "127.0.0.1",
    CAIDE_MODE: "desktop",
    CAIDE_NO_BROWSER: "1",
    CAIDE_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("caide-windows-shutdown-");
