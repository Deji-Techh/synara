...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "caide-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Caide profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "caide-dev" })).toBe(
      "/Users/tester/Library/Application Support/caide-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "caide" })).toBe(
      "/Users/tester/Library/Application Support/caide",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "caide-canary" })).toBe(
      "/Users/tester/Library/Application Support/caide-canary",
    );
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "caide");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "caide-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "caide-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Caide profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Caide database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "caide");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".caide-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "caide");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "caide-profile-seed.json"), "{");
