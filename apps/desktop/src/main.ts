...
import type { ContextMenuItem } from "@caide/contracts";
import { isKeyboardShortcutsHelpChord } from "@caide/shared/browserShortcuts";
import { getMacTrafficLightPosition } from "@caide/shared/desktopChrome";
import { DEVICE_HELPER_SOURCE_DIR_ENV } from "@caide/shared/deviceHelperCache";
import {
...
import { RotatingFileSink } from "@caide/shared/logging";
import { ensureStaticSnapshot, findAsarArchivePath } from "@caide/shared/staticSnapshot";
...
import {
  ...
  resolveCaideStorageSnapshotPath,
...
// `BASE_DIR` prefers CAIDE_HOME, which the Windows registry read hydrates whenever the
...
const desktopFlavor = resolveCaideDesktopFlavor({
  isDevelopment,
  requestedFlavor: process.env.CAIDE_DESKTOP_FLAVOR,
});
const desktopIdentity = caideDesktopIdentity(desktopFlavor);
const BASE_DIR =
  process.env.CAIDE_HOME?.trim() ||
...
const BACKEND_MAX_OLD_SPACE_ENV_KEYS = ["CAIDE_BACKEND_MAX_OLD_SPACE_MB"] as const;
...
const CAIDE_BROWSER_LABEL = "Caide browser";
const browserPerfLoggingEnabled = process.env.CAIDE_BROWSER_PERF === "1";
...
function startBrowserPerformanceLogging(): void {
  ...
  browserPerfInterval = setInterval(() => {
    ...
    console.info(`[${CAIDE_BROWSER_LABEL} perf]`, {
      ...snapshot.counters,
...
async function ensureBrowserHostPipeServer(): Promise<void> {
  if (browserHostPipeServer || !CAIDE_BROWSER_HOST_PIPE_PATH) {
    return;
  }
...
async function reserveBackendEndpoint(reason: string): Promise<void> {
  ...
  process.env.CAIDE_DESKTOP_WS_URL = backendWsUrl;
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { caideCommitHash?: unknown };
    return normalizeCommitHash(parsed.caideCommitHash);
  } catch {
    return null;
  }
}
...
function resolveEmbeddedWindowsPublisherSubjects(): string[] {
  ...
  const subject = __CAIDE_WINDOWS_UPDATER_PUBLISHER__.trim();
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.CAIDE_COMMIT_HASH);
  if (envCommitHash) {
...
async function handleDesktopMigrationRecovery(): Promise<DesktopMigrationRecoveryOutcome> {
  ...
  const outcome = await recoverDesktopMigrationIfRequired({
    ...
    choose: async ({ previousFailure }) => {
      // The user is here because Caide cannot open its database, so the
      ...
      if (canInstallUpdate) {
        choices.push({
          label: "Update Caide and restart",
          detail: "install the newest Caide release, which may already contain the fix",
      ...
      if (releaseUrl !== null) {
        choices.push({
          ...
          detail: `${canInstallUpdate ? "download that release" : "download the latest Caide release"} in a browser`,
      ...
      const result = await dialog.showMessageBox({
        ...
        title:
          previousFailure === null
            ? "Caide needs to recover its database"
            : restoreFailed
              ? "Migration recovery failed"
              : "Caide could not update itself",
        message:
          previousFailure === null
            ? "Caide stopped a database migration before it could finish safely."
            : restoreFailed
              ? "The saved database backup could not be restored."
              : "The newest Caide release could not be installed.",
...
// caide:// protocol here and, via CAIDE_STATIC_DIR, for the backend's HTTP static
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Caide failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
...
function hasConfiguredUpdateFeed(): boolean {
  return readAppUpdateYml() !== null || Boolean(process.env.CAIDE_DESKTOP_MOCK_UPDATES);
}
...
function resolveAutoUpdateDisabledReason(): string | null {
  return getAutoUpdateDisabledReason({
    ...
    disabledByEnv:
      desktopIdentity.usesScriptedUpdates || process.env.CAIDE_DISABLE_AUTO_UPDATE === "1",
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Caide ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
    ...
    void dialog.showMessageBox({
      type: "info",
      title: "Update found",
      message: "Caide is preparing the update in the background.",
      buttons: ["OK"],
    });
...
function resolveNotificationIconPath(): string | null {
  ...
  if (process.platform === "win32") {
    return resolveResourcePath("caide.png") ?? resolveIconPath("ico");
  }
  return resolveResourcePath("caide.png") ?? resolveIconPath("png");
}

function resolveAppSnapHelperPath(): string {
  if (app.isPackaged) {
    return Path.resolve(process.resourcesPath, "..", "Helpers", "caide-appsnap-helper");
  }
  return Path.resolve(__dirname, "..", ".electron-runtime", "appsnap", "caide-appsnap-helper");
}
 ...
 * package.json. We override it to a clean lowercase Caide name.
 */
...
function repairBrowserProfileBeforeElectronReady(userDataPath: string): void {
  ...
  if (browserProfileRepair.status === "repaired") {
    console.info("[desktop] Completed Caide browser profile bridge repair", {
    ...
    console.warn("[desktop] Failed to complete Caide browser profile bridge repair", {
      sourcePath: browserProfileRepair.sourcePath,
      targetPath: browserProfileRepair.targetPath,
...
function restartAfterStartupBundleSwap(error: BundleChangedDuringStartupError): void {
  ...
  void dialog
    .showMessageBox({
      type: "warning",
      title: "Caide needs to restart",
      message: "Caide changed while it was opening.",
      detail:
        "The current process cannot safely read the replaced application bundle. Restart Caide to finish opening with one consistent version.",
      buttons: ["Restart Caide"],
      defaultId: 0,
    })
    .catch(() => undefined)
    .then(() => {
...
// every archive read in this process — the caide:// protocol, the backend's static
...
function startBundleSwapWatcher(): void {
  ...
  bundleSwapPollTimer = setInterval(() => {
    ...
    void dialog
      .showMessageBox({
        type: "warning",
        title: "Caide was replaced on disk",
        message: "The installed Caide app changed while it was running.",
        detail:
...
function processInstallMarkerOnStartup(): void {
  ...
  const message = `Caide restarted, but update ${marker.toVersion} was not installed. Try again.`;
  setUpdateState(
    reduceDesktopUpdateStateOnInstallRestartFailure(
      updateState,
      marker.toVersion,
...
async function installLatestUpdateForMigrationRecovery(): Promise<string | null> {
  ...
  if (updateState.status === "up-to-date") {
    return `Caide ${app.getVersion()} is already the newest release, so updating cannot repair this database.`;
  }
  if (updateState.status !== "downloaded") {
...
function configureAutoUpdater(): void {
  ...
  // default feed while Caide versions advance independently.
  autoUpdater.channel = CAIDE_DESKTOP_UPDATE_CHANNEL;
...
// Builds process-local Node args so provider/tool children do not inherit Caide's heap guard.
...
function backendEnv(): NodeJS.ProcessEnv {
  ...
  const env: NodeJS.ProcessEnv = {
    ...resolveBrowserHostPipeBackendEnv(
      process.env,
      browserHostPipeServer ? CAIDE_BROWSER_HOST_PIPE_PATH : null,
    ...
    CAIDE_MODE: "desktop",
    CAIDE_NO_BROWSER: "1",
    CAIDE_PORT: String(backendPort),
    CAIDE_HOME: BASE_DIR,
    CAIDE_AUTH_TOKEN: backendAuthToken,
    CAIDE_DESKTOP_SHUTDOWN_TOKEN: DESKTOP_BACKEND_SHUTDOWN_TOKEN,
  };
...
function backendFailureDialogDetail(reason: string): string {
  ...
  return [
    cause,
    "Caide paused automatic restarts so a failing backend can't keep respawning in the background.",
...
function presentBackendStartupGiveUp(reason: string): void {
  ...
  const task = (async () => {
    for (;;) {
      const result = await dialog.showMessageBox({
        type: "error",
        title: "Caide's backend didn't start",
        message: `Caide's backend failed to start ${BACKEND_MAX_CONSECUTIVE_START_FAILURES} times in a row.`,
        detail,
...
function handleBackendStartupBlock(block: BackendStartupBlock): void {
  ...
  const task = (async () => {
    if (block.kind === "migration-recovery-required") {
      const result = await dialog.showMessageBox({
        type: "warning",
        title: "Caide needs to recover its database",
        message: "A database migration did not finish safely.",
        detail:
          "Restart Caide to open the verified backup recovery flow. Provider and chat processes will remain stopped until recovery completes.",
    ...
    const processDetail =
      block.ownerPid === null
        ? "Another Caide server is already using this database."
        : `Another Caide server (process ${block.ownerPid}) is already using this database.`;
    const result = await dialog.showMessageBox({
      type: "warning",
      title: "Caide is already running elsewhere",
      message: "Your local Caide data is in use by another process.",
      detail: `${processDetail}\n\nStop the other Caide app or development server, then try again. Your data has not been changed.`,
...
function startBackend(trigger: BackendStartTrigger = "lifecycle"): void {
  ...
  const child = ChildProcess.spawn(process.execPath, [...backendNodeArgs(), backendEntry], {
    ...
    env: {
      ...
      CAIDE_SERVER_ENTRY: backendEntry,
    },
...
function getTitleBarOptions(): BrowserWindowConstructorOptions {
  ...
  return {
    ...
    // Derived from the shared chat-surface header geometry (@caide/shared/desktopChrome)
...
function presentRendererCrashRecovery(
  ...
  const message =
    response.cause === "reload-budget-exhausted"
      ? `Caide's window crashed ${response.crashes} times in a row.`
      : "Caide's window stopped unexpectedly.";
  const detail = [
    `The window's renderer process exited (${reason}).`,
    response.cause === "reload-budget-exhausted"
      ? "Caide paused automatic reloads so a repeating crash can't keep reloading in the background."
      : "This exit reason repeats on reload, so Caide did not retry automatically.",
...
function configureMediaPermissions(): void {
  ...
  const permissionTargets = [
    ...
    },
    {
      ...
      // microphone grant used by Caide's own voice-composer renderer.
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    console.warn("[Caide browser] Failed to start browser host pipe", error);
  }
  startBackend();
