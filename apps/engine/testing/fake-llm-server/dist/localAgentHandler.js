...
exports.handleLocalAgentFixture = handleLocalAgentFixture;
exports.extractLocalAgentFixture = extractLocalAgentFixture;
 ...
 */
function isTodoReminderMessage(msg) {
    if (msg?.role !== "user")
        return false;
 ...
 */
function countToolResultRounds(messages) {
 ...
 */
function extractAttachmentPath(messages) {
...
async function loadFixture(fixtureName) {
    ...
    const fixtureDir = path_1.default.join((0, paths_1.resolveFixturesDir)(), "engine", "local-agent");
    // Try .ts first, then .js
    let fixturePath = path_1.default.join(fixtureDir, `${fixtureName}.ts`);
    if (!fs_1.default.existsSync(fixturePath)) {
        fixturePath = path_1.default.join(fixtureDir, `${fixtureName}.js`);
    }
    try {
 ...
 */
function getTurnsForPass(fixture, passIndex) {
 ...
 */
function createStreamChunk(content, role = "assistant", isLast = false, finishReason = null, usage) {
    const chunk = {
 ...
 */
async function streamTextResponse(res, text, usage, protocol = "openai") {
 ...
 */
async function streamToolCallResponse(res, turn, options) {
    ...
    if (options?.dropAfterToolCalls) {
        (0, log_1.fakeLlmLog)(`[local-agent] Simulating connection drop after streaming tool calls`);
...
function startAnthropicStream(res, usage) {
    res.setHeader("Content-Type", "text/event-stream");
    res.setHeader("Cache-Control", "no-cache");
    res.setHeader("Connection", "keep-alive");
...
async function streamAnthropicTextBlock(res, index, text) {
    text = normalizeFixtureText(text);
...
function finishAnthropicStream(res, stopReason, usage) {
...
async function streamAnthropicTextResponse(res, text, usage) {
    startAnthropicStream(res, usage);
    await streamAnthropicTextBlock(res, 0, text);
    finishAnthropicStream(res, "end_turn", usage);
}
async function streamAnthropicToolCallResponse(res, turn, options) {
    ...
    if (options?.dropAfterToolCalls) {
        (0, log_1.fakeLlmLog)(`[local-agent] Simulating Anthropic connection drop after streaming tool calls`);
        res.socket?.destroy();
        return;
    }
    finishAnthropicStream(res, turn.toolCalls && turn.toolCalls.length > 0 ? "tool_use" : "end_turn", turn.usage);
}
/**
 * Handle a local-agent fixture request
 */
async function handleLocalAgentFixture(req, res, fixtureName, options = {}) {
    const { messages = [] } = req.body;
    const protocol = options.protocol ?? "openai";
    (0, log_1.fakeLlmLog)(`[local-agent] Loading fixture: ${fixtureName}`);
    (0, log_1.fakeLlmLog)(`[local-agent] Messages count: ${messages.length}`);
    try {
        ...
        (0, log_1.fakeLlmLog)(`[local-agent] Loaded fixture: ${fixtureName}, Session: ${sessionId}, Pass: ${passIndex}, Turn: ${turnIndex}, Tool rounds: ${toolResultRounds}`);
        if (turnIndex >= turns.length) {
            // All turns exhausted for this pass, send a simple completion message
            (0, log_1.fakeLlmLog)(`[local-agent] All turns exhausted for pass ${passIndex}, sending completion`);
            await streamTextResponse(res, "Task completed.", undefined, protocol);
            return;
        }
        let turn = turns[turnIndex];
        (0, log_1.fakeLlmLog)(`[local-agent] Executing pass ${passIndex}, turn ${turnIndex}:`, {
            hasText: !!turn.text,
            ...
            if (attachmentPath) {
                turn = {
                    ...
                    toolCalls: turn.toolCalls.map((tc) => ({
                        ...tc,
                        args: JSON.parse(JSON.stringify(tc.args).replace(/\{\{ATTACHMENT_PATH\}\}/g, JSON.stringify(attachmentPath).slice(1, -1))),
                    })),
                };
            }
        }
        // Check if we should simulate a connection drop for this attempt
        const turnScopedDropAttempts = fixture.dropConnectionByTurn?.find((rule) => rule.turnIndex === turnIndex)
            ?.attempts ?? fixture.dropConnectionOnAttempts;
        const turnScopedDropAfterToolCallAttempts = fixture.dropConnectionAfterToolCallByTurn?.find((rule) => rule.turnIndex === turnIndex)?.attempts;
        if (turnScopedDropAttempts && turnScopedDropAttempts.length > 0) {
            ...
            (0, log_1.fakeLlmLog)(`[local-agent] Connection attempt ${currentAttempt} for ${attemptKey}, ` +
                `drop on: [${turnScopedDropAttempts.join(", ")}]`);
            if (turnScopedDropAttempts.includes(currentAttempt)) {
                (0, log_1.fakeLlmLog)(`[local-agent] Simulating connection drop on attempt ${currentAttempt}`);
        ...
        if (turn.toolCalls && turn.toolCalls.length > 0) {
            const dropAfterToolCalls = turnScopedDropAfterToolCallAttempts &&
 ...
 */
function extractLocalAgentFixture(content) {
    if (!content)
        return null;
