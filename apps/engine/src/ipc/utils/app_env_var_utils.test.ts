...
import { EnvVarSchema, REDACTED_ENV_VALUE, SetAppEnvVarsParamsSchema } from "@/ipc/types/misc";
...
vi.mock("@/paths/paths", () => ({
  getCaideAppPath: vi.fn((appPath: string) => require("node:path").join("/mock/apps", appPath)),
}));
...
describe("environment variable secret boundary", () => {
  it("redacts secret values while leaving public configuration readable", () => {
    expect(
      redactAppEnvVars([
        { key: "DATABASE_URL", value: "postgres://private" },
        { key: "STRIPE_SECRET_KEY", value: "sk_live_private" },
        { key: "RESEND_API_KEY", value: "re_private" },
        { key: "VITE_PUBLIC_URL", value: "https://example.com" },
      ]),
    ).toEqual([
      {
        key: "DATABASE_URL",
  ...
  it("preserves an existing secret when the renderer sends its mask", () => {
    expect(
      resolveRedactedEnvVarUpdates({
        existing: [
          { key: "STRIPE_SECRET_KEY", value: "sk_live_private" },
          { key: "VITE_PUBLIC_URL", value: "https://old.example.com" },
        ],
        incoming: [
          { key: "STRIPE_SECRET_KEY", value: REDACTED_ENV_VALUE },
          { key: "VITE_PUBLIC_URL", value: "https://new.example.com" },
        ],
      }),
    ).toEqual([
      { key: "STRIPE_SECRET_KEY", value: "sk_live_private" },
  ...
  it("rejects forged masks for unknown or non-sensitive values", () => {
    for (const key of ["UNKNOWN_SECRET", "VITE_PUBLIC_URL"]) {
      expect(() =>
        resolveRedactedEnvVarUpdates({
          existing: key === "VITE_PUBLIC_URL" ? [{ key, value: "https://example.com" }] : [],
          incoming: [{ key, value: REDACTED_ENV_VALUE }],
        }),
      ).toThrow(`A masked value cannot be used for ${key}`);
    }
  });

  it("rejects duplicate keys at the IPC contract boundary", () => {
    const result = SetAppEnvVarsParamsSchema.safeParse({
      appId: 1,
      envVars: [
    ...
    if (!result.success) {
      expect(result.error.issues[0]?.message).toBe("Duplicate environment variable: DATABASE_URL");
    }
  });
});
...
DEBUG=true`;
    ...
    const result = parseEnvFile(content);
    expect(result).toEqual([
      { key: "API_KEY", value: "abc123", description: "This is a comment" },
      {
        key: "DATABASE_URL",
        value: "postgres://localhost:5432/mydb",
        description: "Another comment",
      },
      { key: "DEBUG", value: "true", description: "PORT=3000 (commented out)" },
    ]);
  });
...
DEBUG=true`;
    ...
    expect(result).toEqual([
      { key: "API_KEY", value: "abc123" },
      {
        key: "DATABASE_URL",
        value: "postgres://localhost:5432/mydb",
        description: "SECRET_KEY=should_be_ignored",
      },
      { key: "DEBUG", value: "true", description: "PORT=3000" },
    ]);
  });
...
DOCS_URL=https://docs.example.com#getting-started # Documentation link`;
    ...
    expect(result).toEqual([
      ...
      },
      {
        key: "DATABASE_NAME",
        value: "myapp",
        description: "PORT=5432 (commented out)",
      },
      {
        key: "FEATURE_A",
        value: "true # Enable feature A",
      ...
      { key: "FEATURE_B", value: "false" },
      {
        ...
        description: "URLs with fragments",
      },
      {
        key: "DOCS_URL",
...
describe("Neon env var helpers", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(fs.promises.lstat).mockRejectedValue(createEnoentError());
  });

  describe("updateNeonEnvVars", () => {
    it("writes the caller-supplied cookie secret when the env file does not exist", async () => {
      vi.mocked(fs.promises.readFile).mockRejectedValueOnce(createEnoentError());

      const cookieSecret = "d".repeat(64);
      await updateNeonEnvVars({
        appPath: "my-app",
        connectionUri: "postgresql://test:test@test-development.neon.tech/test",
        neonAuthBaseUrl: "https://test-development.neonauth.us-east-2.aws.neon.tech/neondb/auth",
        frameworkType: "nextjs",
        cookieSecret,
      });

      expect(fs.promises.writeFile).toHaveBeenCalledWith(
        expect.stringContaining(`${mockEnvPath()}.`),
        expect.any(String),
      ...
      expect(envVars).toEqual(
        expect.arrayContaining([
          ...
          },
          {
            ...
            value: "https://test-development.neonauth.us-east-2.aws.neon.tech/neondb/auth",
          },
...
NEON_AUTH_COOKIE_SECRET=${existingSecret}`);
      ...
      await updateNeonEnvVars({
        appPath: "my-app",
        connectionUri: "postgresql://test:test@test-development.neon.tech/test",
        neonAuthBaseUrl: "https://old.neonauth.us-east-2.aws.neon.tech/neondb/auth",
        frameworkType: "nextjs",
...
NEON_AUTH_COOKIE_SECRET=${existingSecret}`);
      ...
      await updateNeonEnvVars({
        ...
        neonAuthBaseUrl: "https://test-preview.neonauth.us-east-2.aws.neon.tech/neondb/auth",
        frameworkType: "nextjs",
        cookieSecret: existingSecret,
      });

      const envVars = getWrittenEnvVars();
      expect(envVars.find((envVar) => envVar.key === "NEON_AUTH_BASE_URL")?.value).toBe(
        "https://test-preview.neonauth.us-east-2.aws.neon.tech/neondb/auth",
      );
      expect(envVars.find((envVar) => envVar.key === "NEON_AUTH_COOKIE_SECRET")?.value).toBe(
        existingSecret,
      );
    });
...
NEON_AUTH_COOKIE_SECRET=${staleSecret}`);
      ...
      const envVars = getWrittenEnvVars();
      expect(envVars.find((envVar) => envVar.key === "NEON_AUTH_COOKIE_SECRET")).toBeUndefined();
      expect(envVars.find((envVar) => envVar.key === "NEON_AUTH_BASE_URL")?.value).toBe(
...
describe("writeEnvFileSecurely", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(fs.promises.lstat).mockRejectedValue(createEnoentError());
  });

  it("atomically writes secrets with owner-only permissions", async () => {
    const destination = mockEnvPath();

    await writeEnvFileSecurely(destination, "API_KEY=secret");

    const temporary = vi.mocked(fs.promises.writeFile).mock.calls[0]?.[0];
    expect(temporary).toEqual(expect.stringContaining(`${destination}.`));
    expect(fs.promises.writeFile).toHaveBeenCalledWith(
      temporary,
      "API_KEY=secret",
    ...
    expect(fs.promises.rename).toHaveBeenCalledWith(temporary, destination);
    expect(fs.promises.chmod).toHaveBeenCalledWith(destination, 0o600);
  });

  it("refuses to follow an existing environment-file symlink", async () => {
    vi.mocked(fs.promises.lstat).mockResolvedValueOnce({
      isSymbolicLink: () => true,
    } as fs.Stats);

    await expect(writeEnvFileSecurely(mockEnvPath(), "API_KEY=secret")).rejects.toMatchObject({
      kind: "precondition",
    });
...
describe("EnvVarSchema", () => {
  it("accepts conventional environment variable names and secret values", () => {
    expect(
      EnvVarSchema.safeParse({
        key: "DATABASE_URL",
        value: "postgres://user:secret@example.test/app",
      }).success,
    ).toBe(true);
  });

  it("rejects key and value injection through line breaks", () => {
    expect(
      EnvVarSchema.safeParse({
        key: "SAFE_KEY\nINJECTED_KEY",
        value: "secret",
