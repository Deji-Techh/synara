...
vi.mock("@/paths/paths", () => ({
  getCaideAppPath: vi.fn((appPath: string) =>
  ...
  it("rejects duplicate keys at the IPC contract boundary", () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0]?.message).toBe(
        "Duplicate environment variable: DATABASE_URL",
      );
    }
  });
});
...
describe("writeEnvFileSecurely", () => {
  ...
  it("refuses to follow an existing environment-file symlink", async () => {
    ...
    await expect(
      writeEnvFileSecurely(mockEnvPath(), "API_KEY=secret"),
