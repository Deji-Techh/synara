...
export default defineConfig(() => ({
  ...
  plugins: [caideComponentTagger(), react()],
  resolve: {
    alias: {
...
`;
    ...
    expect(pluginsText).toBe(
      "caideComponentTagger(), react(), !process.env.CAPACITOR_BUILD && nitro()",
    );
...
`;
    ...
    expect(pluginsMatch![1].replace(/\s+/g, " ").trim()).toBe(
      "react(), !process.env.CAPACITOR_BUILD && nitro()",
    );
  });
...
`;
    ...
    const next = await readConfig("vite.config.ts");
    expect(next).toContain(`import { nitro } from "nitro/vite"`);
    expect(next).toContain("!process.env.CAPACITOR_BUILD && nitro()");
    expect(next).toMatch(
      /plugins:\s*\[react\(\),\s*!process\.env\.CAPACITOR_BUILD\s*&&\s*nitro\(\)\]/,
    );
  });
...
`;
    ...
    expect(pluginsMatch).not.toBeNull();
    expect(pluginsMatch![1].replace(/\s+/g, " ").trim()).toBe(
      "react(), tailwindcss(), !process.env.CAPACITOR_BUILD && nitro()",
    );
  });
