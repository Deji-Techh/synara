...
vi.mock("@/paths/paths", () => ({
  getCaideAppPath: vi.fn((appPath: string) => {
    ...
    return path.join("/home/user/caide-apps", appPath);
  }),
}));
...
vi.mock("@/ipc/utils/media_path_utils", () => ({
  ...
  CAIDE_MEDIA_DIR_NAME: ".caide/media",
...
describe("cleanupOldMediaFiles", () => {
  ...
  it("should delete files older than the cutoff date", async () => {
    ...
    const appPath = path.join("/home/user/caide-apps", "my-app");
    const mediaDir = path.join(appPath, ".caide/media");
  ...
  it("should skip apps without .caide/media directory", async () => {
  ...
  it("should skip subdirectories inside .caide/media", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2025-01-31T00:00:00.000Z"));
