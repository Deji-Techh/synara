 ...
 *
 * Defaults to {@link CaideErrorKind.External} so unexpected failures (network, permissions,
 * corrupted repos) surface in telemetry. Use {@link CaideErrorKind.Conflict} only when the
...
async function execOrThrow(
  ...
  path: string,
  errorMessage?: string,
  kind: CaideErrorKind = CaideErrorKind.External,
  options?: IGitStringExecutionOptions,
): Promise<void> {
  ...
  if (result.exitCode !== 0) {
    ...
    throw new CaideError(error, kind);
  }
}
 ...
 * user-visible (e.g. .caide/, pnpm-workspace.yaml) for parity with the rest of
...
export async function gitListBranches({ path }: GitBaseParams): Promise<string[]> {
  ...
  if (settings.enableNativeGit) {
    ...
    if (result.exitCode !== 0) {
      throw new CaideError(result.stderr.toString(), CaideErrorKind.Conflict);
    }
    // Parse output:
    // e.g. "* main\n  feature/login"
    return result.stdout
...
export async function gitSetRemoteUrl({ path, remoteUrl }: GitSetRemoteUrlParams): Promise<void> {
  ...
  if (remoteUrl.startsWith("-")) {
    throw new CaideError("Invalid remote URL", CaideErrorKind.Validation);
  }
...
export async function gitCurrentBranch({ path }: GitBaseParams): Promise<string | null> {
  ...
  if (settings.enableNativeGit) {
    ...
    return (branch as string | undefined) ?? null;
  }
}
...
export async function gitLogNative(path: string, depth = 100_000): Promise<GitCommit[]> {
  ...
  if (logResult.exitCode !== 0) {
    throw new CaideError(logResult.stderr.toString(), CaideErrorKind.Conflict);
  }
...
class GitConflictErrorImpl extends CaideError {
...
class GitStateErrorImpl extends CaideError {
  readonly code: string;
  constructor(message: string, code: string) {
    super(message, CaideErrorKind.Precondition);
...
export async function gitDiff({
  ...
  try {
    ...
    throw new CaideError("Failed to get git diff", CaideErrorKind.Internal);
  }
}
