...
const originalCaideEngineUrl = process.env.CAIDE_ENGINE_URL;
const originalLmStudioUrl = process.env.LM_STUDIO_BASE_URL_FOR_TESTING;

describe("call-time model service URLs", () => {
  afterEach(() => {
    if (originalCaideEngineUrl === undefined) {
      delete process.env.CAIDE_ENGINE_URL;
    } else {
      process.env.CAIDE_ENGINE_URL = originalCaideEngineUrl;
    }
  ...
  it("reads CAIDE_ENGINE_URL when called", () => {
    delete process.env.CAIDE_ENGINE_URL;
    expect(getCaideEngineBaseUrl()).toBe("https://engine.dyad.sh/v1");

    process.env.CAIDE_ENGINE_URL = "http://127.0.0.1:4321/v1";
    expect(getCaideEngineBaseUrl()).toBe("http://127.0.0.1:4321/v1");
  });
