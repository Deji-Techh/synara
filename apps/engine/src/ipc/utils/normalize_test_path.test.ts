...
describe("normalizeTestPath", () => {
  it("forces traversal attempts under tests", () => {
    expect(normalizeTestPath("../../../etc/passwd.spec.ts")).toBe("tests/etc/passwd.spec.ts");
    expect(normalizeTestPath("tests/../src/App.spec.ts")).toBe("tests/src/App.spec.ts");
  });

  it("resolves parent segments before sanitizing", () => {
    expect(normalizeTestPath("tests/foo/bar/../baz.spec.ts")).toBe("tests/foo/baz.spec.ts");
  });

  it("normalizes Windows separators", () => {
    expect(normalizeTestPath("tests\\auth\\login.spec.ts")).toBe("tests/auth/login.spec.ts");
  });
  ...
  it("coerces non-spec filenames to unique spec names instead of one shared fallback", () => {
    ...
    expect(normalizeTestPath("tests/login.test.ts")).toBe("tests/login.test.spec.ts");
    expect(normalizeTestPath("tests/checkout.test.ts")).toBe("tests/checkout.test.spec.ts");
