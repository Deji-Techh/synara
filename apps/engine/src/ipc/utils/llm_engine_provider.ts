...
import { CAIDE_INTERNAL_REQUEST_ID_HEADER } from "./provider_options";
...
type CaideEngineProviderOptions = Record<string, any>;
...
export interface CaideEngineProvider {
  /**
...
export function createCaideEngine(options: ExampleProviderSettings): CaideEngineProvider {
  ...
  const getHeaders = () => ({
    Authorization: `Bearer ${getCaideEngineApiKey(options.apiKey)}`,
    ...options.headers,
  });
  ...
  // Custom fetch implementation that adds caide-specific options to the request
  const createCaideFetch = ({
    providerId,
    caideProviderOptions,
    disableCaideOptions = false,
    includeFreeQuotaKey = false,
  }: {
    providerId: string;
    caideProviderOptions?: CaideEngineProviderOptions;
    disableCaideOptions?: boolean;
    ...
    return (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      try {
        ...
        const getCaideOption = (key: string) =>
          key in parsedBody ? parsedBody[key] : caideProviderOptions?.[key];

        const caideVersionedFiles = getCaideOption("caideVersionedFiles");
        if ("caideVersionedFiles" in parsedBody) {
          delete parsedBody.caideVersionedFiles;
        }
        const caideFiles = getCaideOption("caideFiles");
        if ("caideFiles" in parsedBody) {
          delete parsedBody.caideFiles;
        }
        ...
        const caideAppId = getCaideOption("caideAppId");
        ...
        const caideMentionedApps = getCaideOption("caideMentionedApps");
        if ("caideMentionedApps" in parsedBody) {
          delete parsedBody.caideMentionedApps;
        }
        const caideSmartContextMode = getCaideOption("caideSmartContextMode");
        if ("caideSmartContextMode" in parsedBody) {
          delete parsedBody.caideSmartContextMode;
        }
        ...
        if (!caideDisableFiles) {
          parsedBody.caide_options = {
            files: caideFiles,
          ...
          if (caideMentionedApps?.length) {
            parsedBody.caide_options.mentioned_apps = caideMentionedApps;
          }
        }

        // Return modified request with files included and requestId in headers
        const { [CAIDE_INTERNAL_REQUEST_ID_HEADER]: _, ...outgoingHeaders } =
        ...
        const modifiedInit = {
          ...
          headers: {
            ...outgoingHeaders,
            ...(modifiedRequestId && {
              "X-Caide-Request-Id": modifiedRequestId,
            }),
            ...(includeFreeQuotaKey &&
              requestId && {
                "X-Caide-Free-Quota-Key": requestId,
              }),
          },
  ...
  const createChatModel = (
    ...
    const config = {
      ...
      fetch: createCaideFetch({
        providerId: chatParams.providerId,
        disableCaideOptions: pathPrefix === "/free",
...
export async function transcribeWithCaideEngine(
  audioBuffer: Buffer,
  filename: string,
  requestId: string,
  ...
  const baseURL = withoutTrailingSlash(options.baseURL);
  const apiKey = getCaideEngineApiKey(options.apiKey);
  logger.info("transcribing with caide engine with baseURL", baseURL);
...
function getCaideEngineApiKey(apiKey: string | undefined): string {
  const loadedApiKey = loadApiKey({
    apiKey,
    environmentVariableName: "CAIDE_PRO_API_KEY",
