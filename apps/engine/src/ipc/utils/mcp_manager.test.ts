...
vi.mock("./mcp_oauth_provider", () => ({
  CaideOAuthClientProvider: class {},
...
describe("McpManager lifecycle", () => {
  ...
  it("closes and rejects a client whose initialization is disposed", async () => {
    ...
    await expect(initializationResult).resolves.toBeInstanceOf(CaideError);
...
describe("McpManager bearer token injection", () => {
  beforeEach(() => {
    mocks.rows.clear();
    mocks.createMCPClient.mockReset();
    mocks.stdioOptions.length = 0;
  ...
  it("injects the decrypted bearer token into the stdio env", async () => {
    mocks.rows.set(60, {
      id: 60,
      transport: "stdio",
      command: "test-mcp-server",
      args: [],
      envJson: { TEST_MCP: "true" },
      bearerToken: "enc:secret-token",
    });
    mocks.createMCPClient.mockResolvedValueOnce(createClient());
    const manager = new McpManager();
  ...
  it("injects the decrypted bearer token as an Authorization header for http", async () => {
    mocks.rows.set(61, {
      id: 61,
      transport: "http",
      url: "https://example.com/mcp",
      headersJson: { "X-Custom": "yes" },
      oauthEnabled: false,
      bearerToken: "enc:http-token",
    });
    mocks.createMCPClient.mockResolvedValueOnce(createClient());
    const manager = new McpManager();

    await manager.getClient(61);

    const transport = mocks.createMCPClient.mock.calls[0][0].transport as {
      url: string;
    ...
    expect(transport.headers).toEqual({
      "X-Custom": "yes",
      Authorization: "Bearer enc:http-token",
    });
  });

  it("leaves stdio env untouched when no bearer token is set", async () => {
    seedStdioServer(62);
    mocks.createMCPClient.mockResolvedValueOnce(createClient());
    const manager = new McpManager();
