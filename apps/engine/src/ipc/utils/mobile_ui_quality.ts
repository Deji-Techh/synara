...
const SUBSTANTIAL_ENTRY_FILE =
  /(?:^|\/)(?:App|Index|Home|Dashboard|Root|Layout)\.(?:jsx|tsx)$/i;
...
function scanTokenConsistency(file: string, content: string): Problem[] {
  ...
  const colours = new Set(
    [...content.matchAll(RAW_HEX_COLOUR)].map((match) =>
      match[0].toLowerCase(),
    ),
  );
...
function scanAiSlopCluster(file: string, content: string): Problem[] {
  const gradientCount = (
    content.match(/(?:bg|text)-gradient|linear-gradient/gi) ?? []
  ).length;
  const blurCount = (content.match(/backdrop-blur|filter:\s*blur/gi) ?? [])
    .length;
  const oversizedRadiusCount = (
    content.match(
      /rounded-(?:2xl|3xl|full)|border-radius:\s*(?:1\.5|2|3)rem/gi,
    ) ?? []
  ).length;
  const shadowCount = (
    content.match(/shadow-(?:lg|xl|2xl)|box-shadow:/gi) ?? []
  ).length;
  if (
    gradientCount < 2 ||
    blurCount < 2 ||
    oversizedRadiusCount < 5 ||
    shadowCount < 3
  ) {
    return [];
  }
  const index = Math.max(
    0,
    content.search(/(?:bg|text)-gradient|linear-gradient/i),
  );
  return [
    problem(
      file,
...
function scanMotionSource(file: string, content: string): Problem[] {
  ...
  if (
    infinite >= 0 &&
    !/data-caide-ambient-status|caide-approved-ambient/i.test(content)
  ) {
    issues.push(
      problem(
        file,
  ...
  if (
    longMotion >= 0 &&
    !/caide-expressive-motion|data-caide-motion-purpose=["']delight/i.test(
      content,
    )
  ) {
    issues.push(
...
function substantialUiWrite(latestByPath: ReadonlyMap<string, string>) {
  const uiWrites = [...latestByPath].filter(([file]) =>
  ...
  return {
    ...
    substantial:
      uiWrites.length >= 3 &&
      uiWrites.some(([file]) => SUBSTANTIAL_ENTRY_FILE.test(file)),
  };
}
...
export function scanMobileUiResponse(fullResponse: string): Problem[] {
  ...
  return [
    ...[...latestByPath].flatMap(([file, content]) =>
...
function scanBrowserAuditReport(
  ...
  try {
    ...
    if (!report.passed)
      add(
        "Browser design/motion audit did not pass its configured thresholds.",
      );
    if ((report.criticalIssues ?? 1) > 0)
      add("Browser audit contains critical issues.");
    if ((report.majorIssues ?? 1) > 0)
      add("Browser audit contains major issues.");
    const scores = report.scores ?? {};
    if ((scores.overall ?? 0) < 94)
      add("Browser audit overall score is below 94.");
    if ((scores.visual ?? 0) < 94)
      add("Browser audit visual score is below 94.");
    if ((scores.motion ?? 0) < 92)
      add("Browser audit motion score is below 92.");
    if ((scores.accessibility ?? 0) < 95)
      add("Browser audit accessibility score is below 95.");
    if ((scores.coreFlow ?? 0) < 98)
      add("Browser audit core-flow score is below 98.");
    if (
      !report.coreFlowResults?.length ||
      report.coreFlowResults.some((flow) => !flow.passed)
    ) {
    ...
    const viewports = new Set(
      results.map((item) => item.viewport?.id).filter(Boolean),
    );
    ...
    for (const required of [
      ...
      if (!viewports.has(required))
        add(`Browser audit is missing the ${required} viewport.`);
    }
    for (const required of ["normal-light", "normal-dark", "reduced-light"]) {
      if (!modes.has(required))
        add(`Browser audit is missing ${required} mode.`);
    }
    ...
    if (
      !results.some(
        (item) =>
          typeof item.tracePath === "string" && item.tracePath.length > 0,
      )
    ) {
    ...
    if (
      !results.some(
        (item) =>
          typeof item.videoPath === "string" && item.videoPath.length > 0,
      )
    ) {
...
async function latestRelevantMtime(
  appPath: string,
...
export function scoreMobileUiResponse(
  fullResponse: string,
): MobileUiQualityScore {
