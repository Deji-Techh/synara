  ...
  if (has("pnpm-lock.yaml")) {
    return {
      ...
      args: [
        ...PNPM_INSTALL_POLICY_ARGS,
        "add",
        // Caide can generate a pnpm-workspace.yaml (for install policy), which
...
/**
 * Environment variable that switches the generated config into record mode
 * (`video: "on"`, `trace: "on"`). Set by the Tests panel's Watch mode; unset
 * keeps the cheap defaults (no video, trace only on failure). Read at config
 * load time, so the same config file serves both modes with no regeneration.
 */
export const TEST_RECORD_ENV = "DYAD_TEST_RECORD";
 ...
 */
const CAIDE_CONFIG_SENTINEL = "Generated by Caide";

/**
 * Bump when the generated config's shape changes so existing Caide-generated
 * configs get regenerated (e.g. v1 → v2 added record-mode video/trace wiring).
 * Only ever consulted for configs that already carry the sentinel — a
 * hand-written config is never touched.
 */
const CAIDE_CONFIG_VERSION = "caide-config-v2";
...
// ${CAIDE_CONFIG_SENTINEL}. The dev server is started separately by Caide's
...
export default defineConfig({
  ...
  use: {
    ...
    // Watch mode (DYAD_TEST_RECORD=1) records a video and a full trace for
    // every test; otherwise we only keep traces of failures. Read at load
    // time so one config serves both modes.
    video: process.env.${TEST_RECORD_ENV} === "1" ? "on" : "off",
...
function configChannel(appPath: string): BrowserChannel | null {
  const text = readConfigText(appPath);
  if (!text) return null;
...
function isCaideGeneratedConfig(appPath: string): boolean {
  const text = readConfigText(appPath);
  return text != null && text.includes(CAIDE_CONFIG_SENTINEL);
}

/**
 * True when our generated config predates the current version and should be
 * regenerated to pick up shape changes (record-mode wiring etc.). Never true
 * for a hand-written config — those are preserved as-is.
 */
 ...
 *
 * Throws a CaideError (kind External) on install failure — typically offline,
...
export async function ensurePlaywrightBootstrap({
  ...
  if (!packageInstalled) {
    ...
    const installDep = await spawnStreaming({
      ...
      // Caide-managed package-manager paths (executeAddDependency, runtime).
  ...
  if (!hasPlaywrightConfig(appPath)) {
    ...
    // Upgrade a Caide-generated bundled config to the system browser when one is
    // now available, so existing apps stop needing the big download. We never
    // touch a hand-written or .js config.
    if (!usesChannel && detectedChannel && isCaideGeneratedConfig(appPath)) {
    ...
    // Regenerate a stale Caide-generated config (older version of the template)
    // so it picks up new wiring, preserving its browser-channel choice.
    if (isCaideGeneratedConfig(appPath) && needsConfigRegenerate(appPath)) {
      writePlaywrightConfig(appPath, configChannel(appPath));
    }
  }
