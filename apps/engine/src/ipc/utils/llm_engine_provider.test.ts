  ...
  test("uses Anthropic messages API for Anthropic engine models", async () => {
    ...
    await model.doGenerate({
      ...
      providerOptions: {
        "caide-engine": {
          caideAppId: 42,
          caideRequestId: "request-1",
          caideFiles: [{ path: "src/App.tsx", content: "export {}" }],
        },
      },
    ...
    expect(body).not.toHaveProperty("caideAppId");
    expect(body).not.toHaveProperty("caideRequestId");
    expect(body).not.toHaveProperty("caideFiles");
