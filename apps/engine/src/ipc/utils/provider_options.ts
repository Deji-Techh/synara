...
export function getProviderOptions({
  ...
  const providerOptions: Record<string, any> = {
    "caide-engine": {
      ...
      caideRequestId,
      caideDisableFiles,
      caideSmartContextMode: smartContextMode,
      caideFiles: versionedFiles ? undefined : files,
      caideVersionedFiles: versionedFiles,
      caideMentionedApps: mentionedAppsCodebases.map(({ files, appName }) => ({
        appName,
        files,
      })),
    },
