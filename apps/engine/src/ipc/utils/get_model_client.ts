...
import { getCaideEngineBaseUrl } from "./caide_engine_url";
...
const AUTO_CAIDE_PRO_MODEL_ALIASES = [
  "dyad/auto/openai",
  "dyad/auto/anthropic",
  "dyad/auto/google",
] as const;
...
export async function getModelClient(
  ...
  const caideApiKey = getProviderApiKeyForRequest(
    settings.providerSettings?.auto?.apiKey?.value,
    "CAIDE Engine",
  );
  // CAIDE has no subscription tier: the CAIDE Gateway routes models whenever
  // an "auto" (CAIDE Engine) key is configured, and direct provider keys
  // remain the default otherwise.
  const isCaideProEnabledForRequest = Boolean(caideApiKey);
...
async function getProModelClient({
  ...
  if (
    ...
    const fallbackModels = await Promise.all(
      AUTO_CAIDE_PRO_MODEL_ALIASES.map(async (aliasId) => {
