...
export class ViteConfigPatchError extends CaideError {
...
function pluginsArrayContainsNitroCall(pluginsArr: any): boolean {
  for (const el of pluginsArr.elements ?? []) {
    ...
    // Check for !process.env.CAPACITOR_BUILD && nitro()
    if (
      n.LogicalExpression.check(el) &&
      el.operator === "&&" &&
      n.CallExpression.check(el.right) &&
      n.Identifier.check(el.right.callee) &&
      el.right.callee.name === NITRO_LOCAL_NAME
    ) {
      return true;
    }
  }
