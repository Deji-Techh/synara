...
async function resolveMentionedApps(
  ...
  const appsToSearch = (allApps ??
    (await db.query.apps.findMany())) as (typeof apps.$inferSelect)[];
...
async function resolveMentionedAppsFromPrompt(prompt: string, excludeCurrentAppId?: number) {
  ...
  const allApps = (await db.query.apps.findMany()) as unknown as (typeof apps.$inferSelect)[];
