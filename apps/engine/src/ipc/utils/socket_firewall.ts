...
const LEGACY_CAIDE_ALLOW_BUILDS_BEGIN = "# caide-default-allow-builds=v1 begin";
const LEGACY_CAIDE_ALLOW_BUILDS_END = "# caide-default-allow-builds=v1 end";
const CAIDE_AUTO_DENIED_ALLOW_BUILDS_COMMENT = "# caide-auto-denied";
const PNPM_IGNORED_BUILDS_ERROR_CODE = "ERR_PNPM_IGNORED_BUILDS";
const CAIDE_ALLOW_BUILDS_METADATA_PATTERN =
  /^#\s*(caide-default-allow-builds-(?:schema|data-version|channel))=(.+)$/;
const CAIDE_ALLOW_BUILDS_REMOTE_URL =
  process.env.CAIDE_DEFAULT_APPROVE_BUILDS_URL ??
  "https://api.dyad.sh/v1/default-approve-builds.txt";
const CAIDE_ALLOW_BUILDS_FETCH_TIMEOUT_MS = 5_000;
export const CAIDE_ALLOW_BUILDS_CACHE_TTL_MS = 60 * 60 * 1000;
const CAIDE_ALLOW_BUILDS_MAX_BYTES = 256 * 1024;
...
type AllowBuildsSource = {
  schema: typeof CAIDE_ALLOW_BUILDS_SCHEMA;
  dataVersion: string;
  channel: AllowBuildsChannel;
  packages: string[];
};
type AllowBuildsMetadataKey =
  | typeof CAIDE_ALLOW_BUILDS_SCHEMA_KEY
  | typeof CAIDE_ALLOW_BUILDS_DATA_VERSION_KEY
  | typeof CAIDE_ALLOW_BUILDS_CHANNEL_KEY;
...
function parseAllowBuildsMetadata(
  ...
  for (const line of lines) {
    const match = line.trim().match(CAIDE_ALLOW_BUILDS_METADATA_PATTERN);
    if (!match) {
      continue;
    }
...
function parseDefaultAllowBuilds(text = defaultApproveBuildsText): AllowBuildsSource {
  ...
  if (metadata[CAIDE_ALLOW_BUILDS_SCHEMA_KEY] !== CAIDE_ALLOW_BUILDS_SCHEMA) {
    throw new Error(
      `Invalid default pnpm allow-builds list. Expected "${CAIDE_ALLOW_BUILDS_SCHEMA_KEY}=${CAIDE_ALLOW_BUILDS_SCHEMA}".`,
    );
  }
  const dataVersion = metadata[CAIDE_ALLOW_BUILDS_DATA_VERSION_KEY];
  if (!dataVersion) {
    throw new Error(
      `Invalid default pnpm allow-builds list. Expected "${CAIDE_ALLOW_BUILDS_DATA_VERSION_KEY}".`,
    );
  }
  const channel = metadata[CAIDE_ALLOW_BUILDS_CHANNEL_KEY];
  if (channel !== "local" && channel !== "remote") {
    throw new Error(
      `Invalid default pnpm allow-builds list. Expected "${CAIDE_ALLOW_BUILDS_CHANNEL_KEY}" to be local or remote.`,
    );
  }
...
function buildAllowBuildsManagedBlock(source: AllowBuildsSource, indent: string): string[] {
  return [
    ...
    `${indent}# ${CAIDE_ALLOW_BUILDS_DATA_VERSION_KEY}=${source.dataVersion}`,
    `${indent}# ${CAIDE_ALLOW_BUILDS_CHANNEL_KEY}=${source.channel}`,
    ...source.packages.map((pkg) => `${indent}${quoteYamlMapKey(pkg)}: true`),
    `${indent}${CAIDE_ALLOW_BUILDS_END}`,
  ];
}
...
function findAllowBuildsManagedBlock(lines: string[]): {
  ...
  if (beginIndexes.length === 1 && endIndexes.length === 1) {
    ...
    if (beginIndex >= endIndex) {
      throw new Error("Malformed Caide pnpm allow-builds markers.");
    }
  ...
  if (
    lines.some((line) => {
      ...
      return (
        trimmedLine.startsWith("# caide-default-allow-builds=") &&
        trimmedLine !== LEGACY_CAIDE_ALLOW_BUILDS_BEGIN &&
        trimmedLine !== LEGACY_CAIDE_ALLOW_BUILDS_END
      );
    })
  ) {
    throw new Error("Unsupported Caide pnpm allow-builds marker version.");
  }
  ...
  return {
    schema:
      metadata[CAIDE_ALLOW_BUILDS_SCHEMA_KEY] === CAIDE_ALLOW_BUILDS_SCHEMA
        ? CAIDE_ALLOW_BUILDS_SCHEMA
        : undefined,
    dataVersion: metadata[CAIDE_ALLOW_BUILDS_DATA_VERSION_KEY],
    channel:
      metadata[CAIDE_ALLOW_BUILDS_CHANNEL_KEY] === "local" ||
      metadata[CAIDE_ALLOW_BUILDS_CHANNEL_KEY] === "remote"
        ? metadata[CAIDE_ALLOW_BUILDS_CHANNEL_KEY]
        : undefined,
  };
}
...
function removeAutoDeniedPromotedBuilds(lines: string[], source: AllowBuildsSource): string[] {
  ...
  for (let index = range.end - 1; index > range.start; index -= 1) {
    ...
    if (
      ...
      lines[index].includes(CAIDE_AUTO_DENIED_ALLOW_BUILDS_COMMENT)
    ) {
      lines.splice(index, 1);
...
function insertAutoDeniedBuilds(
  ...
  lines.splice(
    ...
    ...newDeniedPackageNames.map(
      (packageName) =>
        `  ${quoteYamlMapKey(packageName)}: false ${CAIDE_AUTO_DENIED_ALLOW_BUILDS_COMMENT}`,
    ),
  );
...
async function fetchRemoteAllowBuildsSourceFromNetwork(
  ...
  try {
    ...
    if (text.length > CAIDE_ALLOW_BUILDS_MAX_BYTES) {
      return null;
    }
    ...
    remoteAllowBuildsCache.set(fetcher, {
      source,
      expiresAtMs: Date.now() + CAIDE_ALLOW_BUILDS_CACHE_TTL_MS,
    });
    return source;
  } catch (error) {
...
export async function commitPnpmAllowBuildsConfigIfChanged(
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[caide] approve pnpm dependency builds",
    });
  } catch (error) {
...
export async function recordDeniedPnpmBuilds({
  ...
  try {
    ...
    try {
      ...
      await gitCommit({
        path: appPath,
        message: "[caide] record denied pnpm dependency builds",
      });
    } catch (error) {
