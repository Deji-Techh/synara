...
interface CaideEngineProviderOptions {
...
export function parseFilesFromMessage(content: string): string[] {
  ...
  // Parse <caide-read path="$filePath"></caide-read>
  ...
  // Parse <caide-code-search-result>...</caide-code-search-result>
  const codeSearchRegex =
    /<caide-code-search-result>(.*?)<\/caide-code-search-result>/gs;
...
export async function processChatMessagesWithVersionedFiles({
  ...
  for (
    ...
    const engineOptions = message.providerOptions?.[
      "caide-engine"
    ] as unknown as CaideEngineProviderOptions;
