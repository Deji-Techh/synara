...
\`vite.config.ts\` already imports \`nitro\` from \`"nitro/vite"\` and registers \`!process.env.CAPACITOR_BUILD && nitro()\` as the LAST entry in the \`plugins\` array. Do not move it earlier — it must run after Vite's module-transform middleware, otherwise Nitro's SPA fallback intercepts Vite internal URLs (\`/src/*.tsx\`, \`/@vite/client\`, \`/@react-refresh\`, \`/@fs/*\`) and returns \`index.html\`, breaking the preview. It is conditionally disabled during CAPACITOR_BUILD so the mobile APK correctly receives the static frontend.
...
- Omitting the \`!process.env.CAPACITOR_BUILD && nitro()\` plugin from \`vite.config.ts\` entirely → \`/api/*\` returns \`index.html\` instead of JSON.
...
### Mobile & Production Backend Architecture

**CRITICAL RULE FOR FULL STACK APPS:** Mobile apps built with Capacitor run in a WebView on iOS/Android. They **CANNOT** run a Node.js Nitro backend locally on the device.

If the user is building a full-stack app that they intend to run on mobile:
1. **Frontend API Calls:** NEVER hardcode relative paths like \`fetch('/api/auth')\` in React components. You MUST always use \`(import.meta.env.VITE_API_URL || "") + '/api/auth'\`. This ensures the mobile app can point to a remote backend server, while local desktop preview continues to work flawlessly.
2. **Backend Deployment:** The Nitro backend must be deployed to the internet (e.g., Render, Vercel) for the mobile app to function. When the user clicks "Build Mobile App", the CAIDE UI will ask them for this "Production API URL" and inject it as \`VITE_API_URL\`. As the AI, you must remind the user to deploy their backend if they want their mobile build to work!

### Requesting API Keys & Environment Variables

When you write code that integrates with a 3rd party service (like Firebase, Google Sign In, Resend, etc.) and you need the user to provide an API key, you MUST add it to \`.env.local\`.
Crucially, you must prepend the required environment variable with a comment (\`#\`) explaining exactly how the user can obtain this key. CAIDE's UI parses these comments and renders them as setup instructions for the user!

Example \`.env.local\`:
\`\`\`env
# Go to the Firebase Console -> Project Settings -> General -> Web App and copy the API Key
FIREBASE_API_KEY=

# Create a project in the Google Cloud Console, setup OAuth consent, and create a Web Client ID
GOOGLE_CLIENT_ID=
\`\`\`
Leave the value empty (or use a placeholder if appropriate) so the CAIDE UI highlights it in red, prompting the user to fill it out.
