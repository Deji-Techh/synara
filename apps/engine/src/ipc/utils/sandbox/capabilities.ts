...
export function assertAllowedGuestPath(guestPath: string): void {
  if (!guestPath || typeof guestPath !== "string") {
    throw new CaideError("File path is required.", CaideErrorKind.Validation);
  }
  // Allow absolute paths if the target exists on disk for read operations
  if (
...
async function resolveSandboxPath(params: {
  ...
  assertAllowedGuestPath(params.guestPath);
  return {
    filePath: path.isAbsolute(params.guestPath)
      ? params.guestPath
      : safeJoin(params.appPath, params.guestPath),
...
async function assertResolvedPathAllowed(params: {
  ...
  if (
    ...
    !require("node:fs").existsSync(realFilePath)
  ) {
    throw new CaideError(
  ...
  if (params.displayPath.startsWith("attachments:")) {
    const realMediaPath = await fs.realpath(getCaideMediaDir(params.appPath));
  ...
  if (!stat.isFile()) {
    throw new CaideError(
      `Path is not a file: ${resolved.displayPath}. ` +
        ...
        "skip them or recurse before calling read_file().",
      CaideErrorKind.Validation,
    );
  }
...
export async function sandboxFileStats(
  ...
  if (!stat.isFile()) {
    throw new CaideError(
      `Path is not a file: ${resolved.displayPath}. ` +
        'list_files() can return directory entries ending in "/"; ' +
        "skip them or recurse before calling file_stats().",
      CaideErrorKind.Validation,
    );
  }
  return {
