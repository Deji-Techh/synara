...
import { isSandboxSupportedPlatform, runSandboxScript } from "@/ipc/utils/sandbox/runner";
...
describe("sandbox capabilities", () => {
  ...
  beforeEach(async () => {
    appPath = await fs.mkdtemp(path.join(os.tmpdir(), "caide-sandbox-"));
    ...
    await fs.writeFile(path.join(appPath, ".environment-setup.md"), "docs", "utf8");
    await fs.mkdir(path.join(appPath, ".envoy"), { recursive: true });
    await fs.writeFile(path.join(appPath, ".envoy", "config.yaml"), "x", "utf8");
  ...
  it("throws instead of silently truncating oversized project file reads", async () => {
    ...
    await expect(sandboxReadFile(appPath, "src/large.log")).rejects.toThrow("exceeding the");
    await expect(
      sandboxReadFile(appPath, "src/large.log", {
        length: SANDBOX_READ_FILE_LIMIT_BYTES + 1,
      }),
    ).rejects.toThrow("read_file length");
    await expect(sandboxReadFile(appPath, "src/large.log", { length: 5 })).resolves.toBe("hello");
  });

  it("denies protected app paths", async () => {
    await expect(sandboxReadFile(appPath, ".env")).rejects.toThrow("protected path");
    await expect(sandboxReadFile(appPath, ".envrc")).rejects.toThrow("protected path");
    await expect(sandboxReadFile(appPath, ".environment-setup.md")).resolves.toBe("docs");
    await expect(sandboxListFiles(appPath, ".envoy")).resolves.toStrictEqual([
      ".envoy/config.yaml",
    ]);
    await expect(sandboxReadFile(appPath, "../outside.txt")).rejects.toThrow("not found");
    await expect(sandboxReadFile(appPath, path.join(appPath, "src", "data.txt"))).resolves.toBe(
      "abcdef",
    );
  });

  it("rejects sandbox writes that escape the app through symlinks", async () => {
    const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "caide-outside-"));
    try {
  ...
  it("allows sandbox writes to new and existing in-app paths", async () => {
    ...
    await expect(assertSandboxWritePathAllowed({ appPath, guestPath: ".env" })).rejects.toThrow(
      "protected path",
    );
  });

  it("allows reading and listing under the internal media dir", async () => {
    const mediaDirName = getCaideMediaDir(appPath);
    await expect(
      sandboxReadFile(appPath, path.posix.join(mediaDirName, "stored-log.txt")),
    ).resolves.toBe("line1\nline2\n");
    await expect(
      sandboxReadFile(appPath, path.posix.join(mediaDirName, "attachments-manifest.json")),
    ).resolves.toContain("server.log");
    await expect(sandboxListFiles(appPath, mediaDirName)).resolves.toEqual(
      expect.arrayContaining([
        ".caide/media/attachments-manifest.json",
        ".caide/media/stored-log.txt",
      ]),
    );
  });
  ...
  it("normalizes and deduplicates logical attachment names", () => {
    ...
    expect(createUniqueAttachmentLogicalName("../server.log", usedNames)).toBe("server-2.log");
    expect(createUniqueAttachmentLogicalName("folder\\data:raw.txt", usedNames)).toBe(
      "data_raw.txt",
    );
  });
  ...
  it("allocates manifest logical names under the manifest lock", async () => {
    ...
    const [entry] = await appendAttachmentManifestEntriesWithLogicalNames(appPath, [
      {
  ...
  it("lists attachment logical paths and returns file stats", async () => {
    ...
    await expect(sandboxFileStats(appPath, "attachments:server.log")).resolves.toMatchObject({
      size: 12,
      isText: true,
    });
  });
  ...
  it("allows explicit attachment aliases with protected-looking names", async () => {
    ...
    await expect(sandboxReadFile(appPath, "attachments:.env")).resolves.toBe("ATTACHED=1");
    await expect(sandboxListFiles(appPath, "attachments:")).resolves.toEqual([
      "attachments:server.log",
      "attachments:.env",
    ]);
  ...
  it("fails sandbox scripts that read oversized attachments without a range", async () => {
    ...
    await fs.truncate(path.join(mediaDir, storedFileName), SANDBOX_READ_FILE_LIMIT_BYTES + 1);
    await appendAttachmentManifestEntries(appPath, [
      {
  ...
  it("spills oversized script output", async () => {
    ...
    await expect(fs.readFile(result.fullOutputPath!, "utf8")).resolves.toHaveLength(
      SANDBOX_UI_OUTPUT_LIMIT_BYTES,
    );
  });
