...
const IPC_ENVELOPE_MARKER = "caide-ipc-envelope-v1";
...
export type IpcInvokeEnvelope<T = unknown> =
  | {
      __caideIpcEnvelope: typeof IPC_ENVELOPE_MARKER;
      ok: true;
      value: T;
    }
  | {
...
export function createIpcSuccessEnvelope<T>(value: T): IpcInvokeEnvelope<T> {
  return {
    __caideIpcEnvelope: IPC_ENVELOPE_MARKER,
    ok: true,
    value,
  };
}
...
export function unwrapIpcEnvelope<T>(response: IpcInvokeEnvelope<T>): T {
  ...
  throw deserializeIpcError((response as { error: SerializedIpcError }).error);
}
