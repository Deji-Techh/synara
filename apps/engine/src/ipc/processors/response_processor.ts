...
export async function processFullResponseActions(
  ...
  if (
    ...
    try {
      ...
      // We don't throw here because failing to store a timestamp shouldn't block the code generation
      // from being applied. It just means time-travel for the database might be broken for this step.
    }
  }
  ...
  try {
    ...
    if (caideExecuteSqlQueries.length > 0) {
      for (const query of caideExecuteSqlQueries) {
        try {
      ...
      logger.log(`Executed ${caideExecuteSqlQueries.length} SQL queries`);
    }
    ...
    for (const tag of caideSearchReplaceTags) {
      ...
      try {
        if (!fs.existsSync(fullFilePath)) {
          // Do not show warning to user because we already attempt to do a <caide-write> tag to fix it.
        ...
        const result = applySearchReplace(original, tag.content);
        if (!result.success || typeof result.content !== "string") {
          // Do not show warning to user because we already attempt to do a <caide-write> and/or a subsequent <caide-search-replace> tag to fix it.
          logger.warn(
    ...
    for (const tag of caideCopyTags) {
      try {
    ...
    // Process all generated tests (<caide-generate-test>). These are plain file
    // writes under the hood; they get a distinct in-chat card + "View in Tests"
    // deep-link, but on disk they behave exactly like a <caide-write>.
    ...
    hasChanges =
      ...
      caideAddDependencyPackages.length > 0;
    ...
    if (hasChanges) {
      ...
      let message = chatSummary
        ? `[caide] ${chatSummary} - ${changes.join(", ")}`
        : `[caide] ${changes.join(", ")}`;
      ...
      if (!staged) {
        ...
        if (uncommittedFiles.length > 0) {
          ...
          try {
            ...
            logger.log(
              `Amend commit with changes outside of caide: ${uncommittedFiles.join(", ")}`,
            );
          } catch (error) {
            // Just log, but don't throw an error because the user can still
            // commit these changes outside of Caide if needed.
            logger.error(
              `Failed to commit changes outside of caide: ${uncommittedFiles.join(", ")}`,
            );
    ...
    ${warnings
      .map(
        (warning) =>
          `<caide-output type="warning" message="${escapeXmlAttr(warning.message)}">${escapeXmlContent(formatOutputError(warning.error))}</caide-output>`,
      )
      .join("\n")}
    ${errors
      .map(
        (error) =>
          `<caide-output type="error" message="${escapeXmlAttr(error.message)}">${escapeXmlContent(formatOutputError(error.error))}</caide-output>`,
      )
      .join("\n")}
    `;
