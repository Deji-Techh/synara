...
export function toCodeExplorerError(error: unknown): Error {
  ...
  if (
    ...
    return new CaideError(message, CaideErrorKind.Precondition);
  }
  ...
  if (
    ...
    return new CaideError(message, CaideErrorKind.Validation);
  }
...
function keyUnavailableError(): CaideError {
...
function getHost(): CodeExplorerHost {
  ...
  const child = utilityProcess.fork(workerPath, [], {
    serviceName: "caide-code-explorer",
  ...
  child.on("message", (response: CodeExplorerHostResponse) => {
    ...
    if (response.success) {
      ...
      const explorerError = (response as { error: string }).error;
      logger.error(`Code explorer host request failed: ${explorerError}`);
      request.reject(toCodeExplorerError(new Error(explorerError)));
    }
  });
