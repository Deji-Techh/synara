...
export async function getTypeCheckPreconditionGuidance({
  ...
  if (declaresTypeScript) {
    ...
    return 'Type checking could not run: TypeScript is listed in package.json but is not installed (node_modules is missing or incomplete). Tell the user to use Rebuild to reinstall dependencies, include `<caide-command type="rebuild"></caide-command>` so they can accept with one click, then retry `run_type_checks`.';
  }

  return includeAgentInstructions
    ? 'Type checking is unavailable: this project does not use TypeScript (no `typescript` entry in package.json). Do not call `run_type_checks` again in this conversation. Verify your changes by reading the files instead. At the end of your reply, recommend that the user add TypeScript to the project so you can automatically catch and fix type errors, and include `<caide-command type="add-typescript"></caide-command>` so they can accept with one click.'
