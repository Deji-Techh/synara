...
export const SocialAuthProviderStatusSchema = z.object({
  id: SocialAuthProviderIdSchema,
  name: z.string(),
...
export const supabaseContracts = {
  ...
  listSocialAuthProviders: defineContract({
    ...
    output: z.array(SocialAuthProviderStatusSchema),
  }),
  ...
  updateSocialAuthProvider: defineContract({
    ...
    input: z
      .object({
        ...
        enabled: z.boolean(),
        clientId: z.string().trim().max(4096).optional(),
        clientSecret: z.string().trim().max(8192).optional(),
      })
      .superRefine((value, context) => {
        ...
        if (!value.clientId) {
          context.addIssue({
            code: "custom",
            path: ["clientId"],
            message: "Client ID is required when enabling a provider",
          });
        }
        if (!value.clientSecret) {
          context.addIssue({
            code: "custom",
            path: ["clientSecret"],
            message: "Client secret is required when enabling a provider",
          });
        }
      }),
    output: SocialAuthProviderStatusSchema,
  }),
