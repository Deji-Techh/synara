...
export const EnvVarSchema = z.object({
  ...
  value: z
    ...
    .refine(
      (value) => !/[\0\r\n]/.test(value),
      "Environment variable values cannot contain null bytes or line breaks",
    ),
...
export const AppEnvVarSchema = EnvVarSchema.extend({
  sensitive: z.boolean(),
});

export type AppEnvVar = z.infer<typeof AppEnvVarSchema>;

export const SetAppEnvVarsParamsSchema = z
  .object({
    appId: z.number(),
    ...
    for (const [index, envVar] of envVars.entries()) {
      if (seen.has(envVar.key)) {
        context.addIssue({
          ...
          message: `Duplicate environment variable: ${envVar.key}`,
        });
      }
      seen.add(envVar.key);
    }
...
const DebugSystemInfoSchema = z.object({
  /** Caide application version (from package.json) */
  caideVersion: z.string(),
...
const DebugSettingsSchema = z.object({
  ...
  enableCaidePro: z.boolean().nullable(),
...
export const miscContracts = {
  ...
  getAppEnvVars: defineContract({
    ...
    output: z.array(AppEnvVarSchema),
  }),
