...
export const McpServerSchema = z.object({
  ...
  // True if a static bearer token is stored (encrypted at rest). The
  // token itself is never sent to the renderer.
...
export const SetMcpBearerTokenParamsSchema = z.object({
  serverId: z.number(),
  // Plaintext bearer token. The handler encrypts it before storing.
  token: z.string(),
});
...
export const mcpContracts = {
  ...
  setBearerToken: defineContract({
    channel: "mcp:set-bearer-token",
    input: SetMcpBearerTokenParamsSchema,
    output: z.void(),
  }),
