...
export const RunAppTestsParamsSchema = z.object({
  ...
  /**
   * When true, Watch mode: runs in a visible headed browser and records a video
   * + full trace for every test (sets `DYAD_TEST_RECORD` + `--headed --trace=on`),
   * so the user can watch the run and replay it afterward. Implies headed.
   */
...
export const RunAppTestsResultSchema = z.object({
  ...
  /**
   * Recordings from a Watch-mode run (video + trace), as app-relative paths
   * under `test-results/` (newest first). Absent when the run wasn't watched,
   * failed before producing artifacts, or produced none.
   */
  recordings: z
    .object({
      traceZips: z.array(z.string()),
...
export const OpenTestRecordingParamsSchema = z.object({
  appId: z.number(),
  /**
   * Recording path relative to the app root, as returned by the run result's
   * `recordings` (e.g. "test-results/signup.spec.ts-w1-trace.zip"). Validated
   * against the app dir before anything is opened.
   */
  path: z.string(),
});
...
export const testsContracts = {
  ...
  openRecording: defineContract({
    channel: "tests:open-recording",
    input: OpenTestRecordingParamsSchema,
    output: z.object({ ok: z.literal(true) }),
  }),
} as const;
