...
import { isFileWithinAnyCaideMediaDir } from "../utils/media_path_utils";
...
export function registerShellHandlers() {
  handle("open-external-url", async (_event, url: string) => {
    if (!url) {
      throw new CaideError("No URL provided.", CaideErrorKind.External);
    }
  ...
  handle("show-item-in-folder", async (_event, fullPath: string) => {
    ...
    if (!fullPath) {
      throw new CaideError("No file path provided.", CaideErrorKind.External);
    }
  ...
  handle("open-file-path", async (_event, fullPath: string) => {
    ...
    // Security: only allow opening files within .caide/media subdirectories.
    // The caide-apps tree contains AI-generated code, so opening arbitrary files
    // there via shell.openPath could execute malicious executables.
    // App paths may be under the default caide-apps base directory (normal) or
    // at an external location (imported with skipCopy).
    if (!isFileWithinAnyCaideMediaDir(fullPath)) {
      throw new CaideError(
        "Can only open files within .caide/media directories.",
