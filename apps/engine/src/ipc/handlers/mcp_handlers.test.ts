...
function seedRow(row: Partial<Row> & { id: number }): void {
  const full: Row = {
    ...
    oauthCallbackPort: row.oauthCallbackPort ?? null,
...
describe("mcp bearer token handlers", () => {
  beforeEach(() => {
    dbStore.clear();
    vi.clearAllMocks();
  });

  it("encrypts and stores the bearer token on set, then disposes the cached client", async () => {
    seedRow({ id: 50, transport: "http" });

    await invoke("mcp:set-bearer-token", {
      serverId: 50,
      token: "plaintext-token",
    });

    const stored = dbStore.get(50)!;
    expect(stored.bearerToken).not.toBeNull();
    expect(stored.bearerToken).not.toBe("plaintext-token");
  ...
  it("throws a NotFound CaideError when the server does not exist", async () => {
    const promise = invoke("mcp:set-bearer-token", {
      serverId: 999,
      token: "token",
    });
  ...
  it("clears the stored token and disposes the cached client", async () => {
    seedRow({ id: 51, transport: "stdio", bearerToken: "enc:old-token" });

    await invoke("mcp:clear-bearer-token", 51);

    expect(dbStore.get(51)!.bearerToken).toBeNull();
    expect(disposeMock).toHaveBeenCalledWith(51);
  });
});
