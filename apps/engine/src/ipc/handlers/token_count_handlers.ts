...
export function registerTokenCountHandlers() {
  handle("chat:count-tokens", async (event, req: TokenCountParams): Promise<TokenCountResult> => {
      ...
      if (settings.enableProSmartFilesContextMode) {
        codebaseTokens = estimateTokens(
          files
            ...
            .map((file) => `<caide-file=${file.path}>${file.content}</caide-file>`)
            .join("\n\n"),
        );
      } else {
        codebaseTokens = estimateTokens(codebaseInfo);
      }
      logger.debug(`Extracted codebase information from ${appPath}, tokens: ${codebaseTokens}`);
    }
    ...
    if (!willUseLocalAgentStream) {
      const mentionedAppsCodebases = await extractMentionedAppsCodebasesFromPrompt(
        req.input,
