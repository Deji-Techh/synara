...
export function transformToolTags(content: string): string {
  // Transform <caide-mcp-tool-call> to <tool-use>. Tolerates extra attributes
  // (e.g. call-id); expects server before tool, which every emitter does.
  let result = content.replace(
    /<caide-mcp-tool-call\b[^>]*?\bserver="([^"]*)"[^>]*?\btool="([^"]*)"[^>]*>\n([\s\S]*?)\n<\/caide-mcp-tool-call>/g,
    '<tool-use name="$2" server="$1">\n$3\n</tool-use>',
  );

  // Transform <caide-mcp-tool-result> to <tool-result> with truncation
  result = result.replace(
    /<caide-mcp-tool-result\b[^>]*?\bserver="([^"]*)"[^>]*?\btool="([^"]*)"[^>]*>\n([\s\S]*?)\n<\/caide-mcp-tool-result>/g,
...
export async function storePreCompactionMessages(
  ...
  // Ensure directory exists and .caide is gitignored
