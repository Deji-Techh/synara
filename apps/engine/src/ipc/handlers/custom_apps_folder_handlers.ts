...
export function registerCustomAppsFolderHandlers() {
  createTypedHandler(systemContracts.getCustomAppsFolder, async () => {
    invalidateCaideAppsBaseDirectoryCache(); // ensure UI is up-to-date
  ...
  createTypedHandler(systemContracts.setCustomAppsFolder, async (_, input) => {
    ...
    if (input) {
      ...
      await mkdir(newCaideAppsBaseDir, { recursive: true });
    }
    ...
    if (newCaideAppsBaseDir !== prevPath) {
      ...
      if (readSettings().enableNativeGit) {
        const directory = updatedSettingValue ?? getDefaultCaideAppsDirectory();
