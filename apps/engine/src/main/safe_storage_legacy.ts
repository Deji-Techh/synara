...
// Ordered by likelihood on a current install: the post-`ready` "caide" identity
// first, then the legacy "Chromium" identity from the pre-`ready` race.
const LEGACY_IDENTITIES: LegacyIdentity[] = [
  { service: "caide Safe Storage", account: "caide Key" },
...
function createDefaultKeychainPasswordReader(): KeychainPasswordReader {
  return process.env.CAIDE_SAFE_STORAGE_READER === "cli"
...
export function recoveryNeedsKeychainUnlock(): boolean {
  ...
  if (process.env.CAIDE_DISABLE_SAFE_STORAGE_RECOVERY === "1") {
    return false;
  }
  if (process.env.CAIDE_DISABLE_SAFE_STORAGE_UNLOCK_PROMPT === "1") {
    return false;
  }
