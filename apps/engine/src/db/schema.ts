...
import type { AppIdentity } from "@/shared/app_identity";
...
export const apps = sqliteTable(
  "apps",
  {
    ...
    framework: text("framework")
      .notNull()
      .default("blank")
      .$type<"blank" | "react-native" | "flutter" | "website">(),
    createdAt: integer("created_at", { mode: "timestamp" })
      .notNull()
      .default(sql`(unixepoch())`),
    updatedAt: integer("updated_at", { mode: "timestamp" })
    ...
    // app_metadata.caide_test=true) and run the tests authenticated as it. Set
    ...
    appIdentity: text("app_identity", { mode: "json" }).$type<AppIdentity>(),
...
export const mcpServers = sqliteTable("mcp_servers", {
  ...
  // (see encryptToString). Read/written only by CaideOAuthClientProvider.
  ...
  // Optional static bearer token sent as an `Authorization: Bearer …`
  // header on HTTP servers, or injected as an env var for stdio
  // servers. Encrypted via `safeStorage` (base64 plaintext fallback
  // where no keyring exists). Never sent to the renderer — the DTO
  // exposes `hasBearerToken` instead.
...
export const blockchainNetworks = sqliteTable("blockchain_networks", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  appId: integer("app_id")
    .notNull()
    .references(() => apps.id, { onDelete: "cascade" }),
