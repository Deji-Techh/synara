...
describe("getErrorMessage", () => {
  it("strips the Electron IPC wrapper from Error messages", () => {
    ...
    expect(getErrorMessage(error)).toBe("Preview branches are used for historical rollback.");
  });

  it("prefers a message field on plain objects", () => {
    expect(getErrorMessage({ message: "Neon API key expired" })).toBe("Neon API key expired");
  });
