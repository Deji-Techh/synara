...
import { buildSandboxFailureMessage } from "@/ipc/utils/sandbox/failure_message";
...
function buildScriptXml(params: {
  ...
  return `<caide-script ${attrs.join(" ")}>${escapeXmlContent(payload)}</caide-script>`;
}
...
- list_files() returns direct child entries and can include directories. Directory paths end in "/".
- Never pass a directory entry to read_file() or file_stats(); skip it or call list_files() again to recurse.
...
// Returns direct children. Directory entries end with "/" and must not be
// passed to read_file() or file_stats(). Skip them or recurse with list_files().
...
Paths are app-relative (including \`.caide/media/<stored-name>\`), or attachment paths like attachments:filename.ext for read_file/list_files/file_stats.${includeWriteFile ? " write_file accepts app-relative paths only, not attachments: paths." : ""} list_files() returns both files and directories; directories end with "/". Prefer range reads, filtering, aggregation, and small summaries over returning entire files.`;
}
