...
export const getMcpToolSchemaTool: ToolDefinition<GetMcpToolSchemaArgs> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<caide-mcp-tool-schema tools="${escapeXmlAttr(tools.join(", "))}">Loading...`;
  },

  execute: async (args: GetMcpToolSchemaArgs, ctx: AgentContext) => {
    const finish = (result: string) => {
      ctx.onXmlComplete(
        `<caide-mcp-tool-schema tools="${escapeXmlAttr(args.tools.join(", "))}">${escapeXmlContent(result)}</caide-mcp-tool-schema>`,
      );
      return result;
    };
