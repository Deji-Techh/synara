...
import { ensureReasoningConsistency } from "@/ipc/utils/ai_messages_utils";
...
export function maybeAppendRetryReplayForRetry(params: {
  ...
  // The replayed assistant messages are rebuilt from stream events and carry no
  // reasoning parts, so combined with a thinking-mode history (whose assistant
  // messages do carry `reasoning_content`) they would be inconsistent and
  // rejected by thinking-mode providers (e.g. DeepSeek). Normalize the whole
  // combined history so every assistant message stays reasoning-consistent.
  const combinedMessages = ensureReasoningConsistency([
    ...currentMessageHistoryRef,
    ...replayMessages,
  ]);

  onCurrentMessageHistoryUpdate([...combinedMessages]);
