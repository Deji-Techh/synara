...
function validateHttpUrl(url: string): void {
  ...
  try {
    ...
    throw new CaideError(`Invalid URL: ${url}`, CaideErrorKind.Validation);
  }
...
export const webFetchTool: ToolDefinition<z.infer<typeof webFetchSchema>> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<caide-web-fetch>${escapeXmlContent(args.url)}`;
  },
  ...
  execute: async (args, ctx) => {
    ...
    ctx.onXmlStream(`<caide-web-fetch>${escapeXmlContent(args.url)}`);
