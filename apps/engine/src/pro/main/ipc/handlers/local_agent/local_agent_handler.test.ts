...
function buildTestSettings(
  ...
  if (overrides.enableCaidePro && overrides.hasApiKey !== false) {
    return {
      ...baseSettings,
      enableCaidePro: true,
      providerSettings: {
...
vi.mock("@/paths/paths", () => ({
  getCaideAppPath: vi.fn((appPath: string) => `/mock/apps/${appPath}`),
}));
...
vi.mock("@/ipc/utils/provider_options", () => ({
  ...
  getAiHeaders: vi.fn(() => ({})),
  CAIDE_INTERNAL_REQUEST_ID_HEADER: "x-caide-internal-request-id",
}));
...
const caideRequestId = "test-request-id";
describe("handleLocalAgentStream", () => {
  ...
  describe("Warning propagation", () => {
    ...
    it("appends shared-module Supabase deploy warnings as caide-output", async () => {
      const { event } = createFakeEvent();
      mockSettings = buildTestSettings({ enableCaidePro: true });
      mockChatData = buildTestChat({
        supabaseProjectId: "supabase-project-id",
    ...
    it("should compact between steps when token usage crosses threshold", async () => {
      ...
      mockPerformCompaction.mockImplementation(async () => {
        ...
        mockChatData = {
          ...
          messages: [
            ...mockChatData.messages,
            {
              ...
              role: "assistant",
              content:
                '<caide-compaction title="Conversation compacted" state="finished">mid-turn summary</caide-compaction>',
        ...
        return {
          ...
          backupPath: ".caide/chats/1/compaction-test.md",
        };
      });
