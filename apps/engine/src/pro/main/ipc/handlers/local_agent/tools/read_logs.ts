...
export const readLogsTool: ToolDefinition<z.infer<typeof readLogsSchema>> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<caide-read-logs ${filters.join(" ")}>
${summary}
</caide-read-logs>`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    if (!chat || !chat.app) {
      throw new CaideError("Chat or app not found.", CaideErrorKind.NotFound);
    }
    ...
    ctx.onXmlComplete(
      `<caide-read-logs ${filters.join(" ")} count="${filtered.length}">\n${summary}\n\n${escapeXmlContent(formattedLogs)}\n</caide-read-logs>`,
    );
