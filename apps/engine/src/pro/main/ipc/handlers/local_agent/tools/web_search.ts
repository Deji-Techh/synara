  ...
  try {
    while (true) {
      ...
      buffer = parseSSEEvents(buffer, (content) => {
        accumulated += content;
        // Stream intermediate results to UI with caide-web-search prefix
        ctx.onXmlStream(
          `<caide-web-search query="${escapeXmlAttr(query)}">${escapeXmlContent(accumulated)}`,
        );
      });
    }
...
export const webSearchTool: ToolDefinition<z.infer<typeof webSearchSchema>> = {
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    ctx.onXmlComplete(
      `<caide-web-search query="${escapeXmlAttr(args.query)}">${escapeXmlContent(result)}</caide-web-search>`,
    );
