...
// parts wrapping <caide-*> blocks. Strip them so that tool/system output does
// not reach the classifier. Bodies are escaped at creation, so no breakout.
const CAIDE_TAG_RE = /<caide-([a-z0-9-]+)\b[^>]*>[\s\S]*?<\/caide-\1>/g;
  ...
  for (const m of parsed) {
    ...
    for (const part of m.content) {
      if (part.type === "text" && typeof part.text === "string") {
        const text = part.text.replace(CAIDE_TAG_RE, "").trim();
