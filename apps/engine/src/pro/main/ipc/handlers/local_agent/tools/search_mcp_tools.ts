...
export const searchMcpToolsTool: ToolDefinition<SearchMcpToolsArgs> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<caide-mcp-tool-search${buildSearchAttributes(args)}>Searching...`;
  },

  execute: async (args: SearchMcpToolsArgs, ctx: AgentContext) => {
    const finish = (result: string) => {
      ctx.onXmlComplete(
        `<caide-mcp-tool-search${buildSearchAttributes(args)}>${escapeXmlContent(result)}</caide-mcp-tool-search>`,
      );
      return result;
    };
