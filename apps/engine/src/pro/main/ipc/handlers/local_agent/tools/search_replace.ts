...
`,
  ...
  buildXml: (args, isComplete) => {
    ...
    let xml = `<caide-search-replace path="${escapeXmlAttr(args.file_path)}" description="">\n<<<<<<< SEARCH\n${escapeXmlContent(escapedOld)}`;
      ...
      if (!result.success || typeof result.content !== "string") {
        ...
        throw new CaideError(
          [
            ...
            "",
            `Recovery: re-read ${args.file_path} now, copy the current full lines with exact whitespace, and retry once.`,
