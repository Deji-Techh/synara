...
line 5`;
  ...
  beforeEach(async () => {
    ...
    const mediaDir = getCaideMediaDir(testDir);
  ...
  describe("buildXml", () => {
    ...
    it("builds XML with path only", () => {
      ...
      expect(result).toBe('<caide-read path="src/App.tsx"></caide-read>');
    });
    ...
    it("includes both line range attributes", () => {
      ...
      expect(result).toBe(
        '<caide-read path="src/App.tsx" start_line="10" end_line="50"></caide-read>',
      );
    });
  ...
  describe("execute - app_name (referenced apps)", () => {
    ...
    it("blocks .caide/ paths when targeting a referenced app", async () => {
      mockContext.referencedApps.set("other-app", otherAppDir);
      await expect(
        readFileTool.execute(
          { path: ".caide/chats/secret.md", app_name: "other-app" },
          mockContext,
        ),
      ).rejects.toThrow(/Cannot read \.caide\/ paths from referenced apps/);
    });

    it("blocks .caide/ paths reached via traversal aliases (e.g. src/../.caide/...)", async () => {
      mockContext.referencedApps.set("other-app", otherAppDir);
      ...
      await expect(
        readFileTool.execute(
          { path: "src/../.caide/secret.md", app_name: "other-app" },
          mockContext,
        ),
      ).rejects.toThrow(/Cannot read \.caide\/ paths from referenced apps/);
    });
    ...
    it.runIf(process.platform !== "win32")(
      "blocks symlinks into a referenced app's protected .caide directory",
      async () => {
        mockContext.referencedApps.set("other-app", otherAppDir);
        const caideDir = path.join(otherAppDir, ".caide");
        await fs.promises.mkdir(caideDir, { recursive: true });
