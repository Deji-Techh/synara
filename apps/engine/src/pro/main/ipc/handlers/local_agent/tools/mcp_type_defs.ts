  ...
  for (const def of params.defs) {
    map[def.jsName] = async (rawArgs: unknown) => {
      ...
      const autoApprove = buildMcpAutoApprove({
        ...
        isCaidePro: params.ctx.isCaidePro,
      ...
      params.ctx.onXmlComplete(
        `<caide-mcp-tool-call server="${escapeXmlAttr(def.serverName)}" tool="${escapeXmlAttr(def.toolName)}" call-id="${escapeXmlAttr(callId)}"${autoApprovedAttr}>\n${escapeXmlContent(contentPretty)}\n</caide-mcp-tool-call>`,
      );
      ...
      try {
        ...
        // stuck on "Running".
        params.ctx.onXmlComplete(
          `<caide-mcp-tool-result server="${escapeXmlAttr(def.serverName)}" tool="${escapeXmlAttr(def.toolName)}" call-id="${escapeXmlAttr(callId)}" is-error="true">\n${escapeXmlContent(errorMessage)}\n</caide-mcp-tool-result>`,
        );
        params.ctx.onXmlComplete(
          `<caide-output type="error" message="MCP tool '${escapeXmlAttr(def.toolKey)}' failed: ${escapeXmlAttr(errorMessage)}">${escapeXmlContent(errorStack || errorMessage)}</caide-output>`,
        );
        throw error;
      }
    };
...
export function getMcpInlineTokenThreshold(): number {
  const raw = process.env.CAIDE_MCP_INLINE_TOKEN_THRESHOLD;
  if (raw !== undefined) {
    const n = Number(raw);
