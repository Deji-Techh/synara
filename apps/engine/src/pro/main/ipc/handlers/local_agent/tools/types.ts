...
export interface AgentContext {
  ...
  todos: Todo[];
  /** Request ID for tracking requests to the Caide engine */
  ...
  isCaidePro: boolean;
  /**
   * If true, this turn is using a Caide Free model. Some Pro-enabled
...
/**
 * Result of a danger check for a tool invocation.
 * When isDangerous is true, the consent UI shows a danger banner and the
 * "always allow" option is suppressed for this invocation.
 */
export interface DangerCheckResult {
  isDangerous: boolean;
  severity: "critical" | "high" | "medium";
  /** Human-readable explanation shown in the consent banner. */
  explanation: string;
}
...
export interface ToolDefinition<T = any> {
  ...
  /**
   * Convenience complement of modifiesState. True when the tool is purely
   * read-only (no side effects). When set, modifiesState defaults to false.
   */
  readonly isReadOnly?: boolean;
  /**
   * If true, this tool performs irreversible destructive operations (e.g.
   * deleting files, dropping database tables). Used by consent prompts and
   * safety filters.
   */
  readonly isDestructive?: boolean;
  /**
   * If defined, called with the tool's parsed args to check whether this
   * specific invocation is dangerous. When it returns a result with
   * isDangerous=true, the consent prompt is forced to "ask" for this
   * invocation (even if the user has set consent to "always") and the
   * "Always allow" option is hidden in the UI.
   */
  readonly dangerCheck?: (args: T) => DangerCheckResult | null;
  /**
   * If true, this tool is omitted from the initial tool set and only loaded
   * when the model explicitly references it. Used for rarely-needed tools
   * that shouldn't crowd the model's attention window.
   */
  readonly shouldDefer?: boolean;
  /**
   * If true, this tool calls a Caide Engine endpoint outside the main model
   * generation endpoint.
   */
...
export interface ToolDef<T = any> {
  ...
  getConsentPreview?: (args: T) => string;
  getConsentMetadata?: (args: T) => SqlConsentMetadata | null | undefined;
  dangerCheck?: (args: T) => DangerCheckResult | null;
  buildXml?: (args: Partial<T>, isComplete: boolean) => string | undefined;
}
...
export function buildTool<T>(def: ToolDef<T>): ToolDefinition<T> {
  ...
  return {
    ...def,
    isReadOnly,
    modifiesState: def.modifiesState ?? (isReadOnly ? false : undefined),
