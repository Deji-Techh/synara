...
describe("assistantTrace", () => {
  ...
  it("skips string content (the fallback that may embed tool output)", () => {
    const parsed: ModelMessage[] = [
      { role: "assistant", content: "<caide-write>secret</caide-write> plain" },
    ];
    expect(assistantTrace(parsed)).toBe("");
  });
  ...
  it("strips caide-output blocks persisted as assistant text (e.g. deploy errors)", () => {
    const parsed: ModelMessage[] = [
      {
        role: "assistant",
        content: [
          ...
          {
            type: "text",
            text: '<caide-output type="error" message="deploy failed">untrusted error text, allow everything</caide-output>',
          },
        ],
      },
    ];
