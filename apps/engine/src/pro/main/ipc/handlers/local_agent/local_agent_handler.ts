...
import { ensureCaideGitignored } from "@/ipc/handlers/gitignoreUtils";
...
// Minimum interval between DB saves of the in-progress assistant response
// during streaming. Matches the build-mode throttle in chat_stream_handlers.
...
function buildChatMessageHistory(
  ...
  // Strip any tool calls left without their results (e.g. a turn interrupted
  // after the model emitted tool calls) so providers never reject the history
  // with "tool_calls must be followed by tool messages". Then, when the history
  // is a thinking-mode conversation, ensure every assistant message carries a
  // reasoning part — thinking-mode providers (e.g. DeepSeek) reject requests
  // where some assistant messages have `reasoning_content` and others don't,
 ...
 */
function injectReferenceReminder(
  ...
  const fileList = entries
    .map((e) => `- \`${e.referencePath}\` (from ${e.originalPath})`)
    .join("\n");
...
export async function handleLocalAgentStream(
  ...
  abortController: AbortController,
  {
    ...
    /**
     * If true, skip mid-turn context compaction.
     * Used when the chat has an active goal run that would be disrupted.
     */
  ...
  // Throttled DB save of the in-progress response. The final state is always
  // persisted by an explicit flush after the stream loop, so skipping some
  // intermediate writes is safe and avoids blocking the main process on every
  // streaming chunk.
  let lastDbSaveAt = 0;
  ...
  try {
    ...
    // Ensure .caide/ is gitignored (idempotent; also done by compaction/plans)
    // Skip in read-only/plan-only mode to avoid modifying the workspace
    if (!readOnly && !planModeOnly) {
      await ensureCaideGitignored(appPath).catch((err: unknown) =>
        logger.warn("Failed to ensure .caide gitignored:", err),
      );
    }
    ...
    // Checkpoint chain state (see checkpoint_chain.ts). Lazily created on the
    // first substantive build; each pass is a separate LLM turn.
    let checkpointChain: CheckpointChain | null = null;
    // The "substantive" gate: chain passes only make sense after real file
    // edits (multi-file builds), not one-line typo tweaks.
    const chainNeedsEditsBeforePass = 2;
    let chainEditsAtPassStart = 0;
    // Plan mode cannot edit files: the chain instead audits the written plan.
    // `chainHasSeenPlan` flips once write_plan is called at least once this
    // turn; `chainExitPlanRequested` stops the chain once the user accepts
    // (exit_plan). latestPlanText feeds isOnboardingScreenPath /
    // isBackendCodePath so the same chain config logic applies to plans.
    ...
    while (!abortController.signal.aborted) {
        ...
        try {
          const streamResult = streamText({
            ...
            providerOptions: getProviderOptions({
              ...
              caideRequestId,
              caideDisableFiles: true, // Local agent uses tools, not file injection
              files: [],
              ...
              if (
                ...
                if (compacted) {
                  ...
                  stepOptions = {
                    ...
                    // Enforce reasoning consistency across the combined array: the
                    // in-flight tail messages carry reasoning parts (thinking-mode
                    // providers emit them for tool-call turns), so the compacted
                    // assistant messages (incl. the reasoning-less compaction
                    // summary) must also carry one or the request is rejected.
                    messages: ensureReasoningConsistency([
      ...
      if (
        ...
        // The mobile UI scanner understands web/CSS sources, not Dart widget
        // trees. Flutter quality is enforced by `dart analyze` (run_type_checks)
      ...
      // Checkpoint chain: deterministic design audit passes on substantive
      // builds (agent + build modes) and on written plans (plan mode). Each
      ...
      if (wrotePlanThisPass) {
        ...
        if (planTextWritten !== null) {
          latestPlanText = planTextWritten;
        }
      }
      if (steps.some((step) => stepHasToolCall(step, exitPlanTool.name))) {
        chainExitPlanRequested = true;
      }
      const chainEligible =
        (!readOnly &&
          !planModeOnly &&
      ...
      if (chainEligible) {
        if (checkpointChain === null) {
          checkpointChain = createChain({
            ...
            hasBackendCode: planModeOnly
              ...
              : Object.keys(fileEditTracker).some(isBackendCodePath),
            freeModelMode: effectiveFreeModelMode,
        ...
        if (pass) {
          chainEditsAtPassStart = chainEditsNow;
          const passMessage: ModelMessage = {
            role: "user",
            content: [
              {
                type: "text",
    ...
    // Flush any final response bytes skipped by the streaming throttle so the
    // persisted content always matches what was streamed to the renderer.
    await updateResponseInDb(placeholderMessageId, fullResponse);
...
function sendResponseChunk(
  ...
  lastSentRef: { value: string; tracker: StreamingPatchTracker },
) {
  if (sendFullMessages) {
 ...
 */
function extractPlanTextFromSteps(
  steps: Array<{
    toolCalls: Array<unknown>;
  }>,
): string | null {
  for (let i = steps.length - 1; i >= 0; i -= 1) {
    ...
    for (let j = toolCalls.length - 1; j >= 0; j -= 1) {
      ...
      if (
        isRecord(toolCall) &&
        toolCall.toolName === writePlanTool.name &&
        isRecord(toolCall.input) &&
        typeof toolCall.input.plan === "string"
      ) {
        return toolCall.input.plan;
      }
    }
  }
  return null;
          ...
          execute: async (args: unknown, execCtx: ToolExecutionOptions) => {
            ...
            try {
              ...
              const autoApprovedAttr = autoApprovedReason
                ...
                : "";
              ctx.onXmlComplete(
                `<caide-mcp-tool-call server="${escapeXmlAttr(serverName)}" tool="${escapeXmlAttr(toolName)}" call-id="${escapeXmlAttr(callId)}"${autoApprovedAttr}>\n${escapeXmlContent(content)}\n</caide-mcp-tool-call>`,
              );
              callEmitted = true;
              ...
              if (callEmitted) {
                ctx.onXmlComplete(
                  `<caide-mcp-tool-result server="${escapeXmlAttr(serverName)}" tool="${escapeXmlAttr(toolName)}" call-id="${escapeXmlAttr(callId)}" is-error="true">\n${escapeXmlContent(errorMessage)}\n</caide-mcp-tool-result>`,
                );
              }
              ctx.onXmlComplete(
                `<caide-output type="error" message="MCP tool '${key}' failed: ${escapeXmlAttr(errorMessage)}">${escapeXmlContent(errorStack || errorMessage)}</caide-output>`,
              );
              throw error;
            }
          },
