...
const DESCRIPTION = `Generate an image using AI based on a text prompt. The generated image is saved to the project's .caide/media directory.
...
### After Generation
The tool returns the file path in .caide/media. Use the copy_file tool to copy it to the appropriate location in the project (e.g., public/assets/) and reference that path in your code.
`;
...
async function saveGeneratedImage(
  ...
  const relativePath = path.join(CAIDE_MEDIA_DIR_NAME, fileName);
...
export const generateImageTool: ToolDefinition<z.infer<typeof generateImageSchema>> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<caide-image-generation prompt="${escapeXmlAttr(args.prompt)}">`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    try {
      ...
      ctx.onXmlComplete(
        `<caide-image-generation prompt="${escapeXmlAttr(args.prompt)}" path="${escapeXmlAttr(relativePath)}">${escapeXmlContent(relativePath)}</caide-image-generation>`,
      );
      ...
      ctx.onXmlComplete(
        `<caide-image-generation prompt="${escapeXmlAttr(args.prompt)}"></caide-image-generation>`,
      );
      throw error;
    }
  },
