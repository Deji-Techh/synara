...
describe("listFilesTool", () => {
  ...
  describe("execute - app_name (referenced apps)", () => {
    ...
    it("excludes .caide files from referenced apps even when include_ignored is true", async () => {
      mockContext.referencedApps.set("other-app", otherAppDir);
      const result = await listFilesTool.execute(
        {
          app_name: "other-app",
      ...
      expect(result).not.toContain(".caide/rules.md");
    });

    it("excludes .caide files from referenced apps in the default (non-include_ignored) listing", async () => {
      mockContext.referencedApps.set("other-app", otherAppDir);
      const result = await listFilesTool.execute(
        { app_name: "other-app", recursive: true },
        mockContext,
