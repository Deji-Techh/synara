...
> = {
  ...
  buildXml: () => `<caide-enable-nitro></caide-enable-nitro>`,

  execute: async (_args, ctx: AgentContext) => {
    // Nitro is a Vite-only server layer. `isEnabled` already filters the tool
    // out for anything else, but a parallel/retry call must not install it into
    // a Flutter app (no Vite, no server/ tree convention).
    if (ctx.frameworkType !== "vite" && ctx.frameworkType !== "vite-nitro") {
      return `enable_nitro is only supported for Vite apps. This app has framework type "${ctx.frameworkType}" — there is no Vite server layer and no server/ tree (Flutter apps in particular have none). Keep any backend needs client-side (e.g. Supabase anon key + RLS) or in the generated app code; do NOT add Nitro.`;
    }
