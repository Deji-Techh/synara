...
async function callCodeSearch(
  ...
  ctx.onXmlStream(
    `<caide-code-search${buildCodeSearchAttributes({
      query: params.query,
...
export const codeSearchTool: ToolDefinition<CodeSearchArgs> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<caide-code-search${buildCodeSearchAttributes(args)}>Searching...`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    // Write final result to UI and DB with caide-code-search wrapper
    ctx.onXmlComplete(
      `<caide-code-search${buildCodeSearchAttributes(args)}>${escapeXmlContent(resultText)}</caide-code-search>`,
    );
