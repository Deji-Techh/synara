...
// Under the test build (a local fake LLM server announced via FAKE_LLM_PORT),
// the classifier races a single-threaded fake server that can stall on a
// machine running many integration suites in parallel, so give it a much more
...
// the feature is off or the turn is running in Caide Free mode. Shared by both
