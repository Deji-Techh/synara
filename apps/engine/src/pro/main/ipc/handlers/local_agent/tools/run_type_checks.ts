...
import { CaideErrorKind, isCaideError } from "@/errors/caide_error";
import { generateFlutterProblemReport } from "@/ipc/processors/flutter";
...
- NEVER call this tool on a file unless you've edited it or are about to edit it`,
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    if (ctx.frameworkType === "flutter") {
      return executeFlutterAnalyze(args.paths, title, ctx);
    }
    ...
    try {
      ...
      if (!isCaideError(error) || error.kind !== CaideErrorKind.Precondition) {
        throw error;
      }
      ...
      ctx.onXmlComplete(
        `<caide-output type="warning" message="${escapeXmlAttr("Type checking unavailable")}">\n${escapeXmlContent(result)}\n</caide-output>`,
      );
    ...
    ctx.onXmlComplete(
      `<caide-status title="${escapeXmlAttr(title)}">\n${escapeXmlContent(result)}\n</caide-status>`,
    );
...
async function executeFlutterAnalyze(
  ...
  let problemReport: ProblemReport;
  try {
    problemReport = await generateFlutterProblemReport(ctx.appPath);
  } catch (error) {
    ...
    ctx.onXmlComplete(
      `<caide-output type="warning" message="${escapeXmlAttr("Flutter analysis unavailable")}">\n${escapeXmlContent(message)}\n</caide-output>`,
    );
    return message;
  }
  ...
  if (paths && paths.length > 0) {
    problems = problems.filter((p) => matchesPaths(p.file, paths!));
  }
