...
const THEME_IMAGES_TEMP_DIR = path.join(os.tmpdir(), "caide-theme-images");
...
export function registerThemesHandlers() {
  ...
  handle(
    ...
    async (_, params: UpdateCustomThemeParams): Promise<CustomTheme> => {
      ...
      if (!currentTheme) {
        throw new CaideError("Theme not found", CaideErrorKind.NotFound);
      }
  ...
  handle(
    ...
    async (_, params: SaveThemeImageParams): Promise<SaveThemeImageResult> => {
      ...
      if (!data || typeof data !== "string") {
        throw new CaideError("Invalid image data", CaideErrorKind.Validation);
      }
