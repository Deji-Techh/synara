...
import type { SocialAuthProviderId, SocialAuthProviderStatus } from "@/ipc/types/supabase";

const fsPromises = fs.promises;

const logger = log.scope("supabase_management_client");

const SOCIAL_AUTH_PROVIDERS: ReadonlyArray<{
...
export async function getSupabaseClient({
  ...
  if (isTokenExpired(expiresIn)) {
    ...
    if (!newAccessToken) {
      throw new CaideError("Failed to refresh Supabase access token", CaideErrorKind.Auth);
    }
...
type SupabaseAuthConfig = Record<string, unknown>;

async function requestSupabaseAuthConfig({
  projectId,
  organizationSlug,
  method,
  body,
  ...
  const accessToken = (client as unknown as { options?: { accessToken?: string } }).options
    ?.accessToken;
  if (!accessToken) {
    throw new CaideError(
      "Supabase authorization is unavailable. Reconnect the Supabase account.",
      CaideErrorKind.Auth,
    );
  }
  const response = await fetchWithRetry(
    `https://api.supabase.com/v1/projects/${encodeURIComponent(projectId)}/config/auth`,
    {
      method,
      headers: {
    ...
    `${method === "GET" ? "Read" : "Update"} Supabase authentication configuration`,
  );
  if (!response.ok) {
    logger.error(`Supabase auth configuration request failed with status ${response.status}`);
    throw new CaideError(
      response.status === 403
        ? "Supabase did not grant permission to manage authentication. Reconnect the account with Auth configuration access."
        : `Supabase could not ${method === "GET" ? "load" : "update"} authentication settings (${response.status}).`,
  ...
  try {
    return JSON.parse(responseText) as SupabaseAuthConfig;
  } catch {
    throw new CaideError(
      "Supabase returned an invalid authentication configuration response.",
      CaideErrorKind.External,
    );
  }
}
...
function socialAuthStatus(
  ...
  return {
    ...
    callbackUrl: `https://${projectId}.supabase.co/auth/v1/callback`,
  };
}
...
export async function listSupabaseSocialAuthProviders(input: {
  ...
  if (IS_TEST_BUILD) {
    return SOCIAL_AUTH_PROVIDERS.map((provider) => socialAuthStatus(provider, {}, input.projectId));
  }
  const config = await requestSupabaseAuthConfig({
    ...input,
    method: "GET",
  });
  return SOCIAL_AUTH_PROVIDERS.map((provider) =>
    socialAuthStatus(provider, config, input.projectId),
  );
}
...
export async function updateSupabaseSocialAuthProvider(input: {
  ...
  if (!provider) {
    throw new CaideError("Unsupported social authentication provider.", CaideErrorKind.Validation);
  }
  if (IS_TEST_BUILD) {
    return {
      ...socialAuthStatus(provider, {}, input.projectId),
      enabled: input.enabled,
      configured: input.enabled,
    };
  }
  const body = buildSocialAuthProviderPatch(input);
  await requestSupabaseAuthConfig({
  ...
  return socialAuthStatus(provider, config, input.projectId);
}
...
export async function listSupabaseFunctions({
  ...
  logger.info(`Found ${functions.length} functions for project: ${supabaseProjectId}`);
  return functions;
}
...
async function deploySupabaseFunctionUnqueued({
  ...
  // Function-local and shared files are independent filesystem reads.
  const [functionFiles, sharedFiles] = await Promise.all([
    collectFunctionFiles({
      functionPath,
      functionName,
    }),
    getSharedFiles(appPath),
  ]);
