...
describe("local_agent_prompt", () => {
  ...
  it("agent mode system prompt (flutter framework uses the Flutter skill pack and contract)", () => {
    const prompt = constructLocalAgentPrompt(undefined, undefined, {
      frameworkType: "flutter",
    });
    expect(prompt).toMatchSnapshot();
    expect(prompt).toContain("PLATFORM CONTRACT — FLUTTER APP");
    expect(prompt).toContain("NavigationBar");
    expect(prompt).toContain("Module 1 — Material 3 & Flutter fundamentals");
    expect(prompt).toContain("hot restart");
    expect(prompt).not.toContain("node_modules");
    expect(prompt).not.toContain("min-height: 100dvh");
  });

  it("flutter prompts omit Playwright test-writing guidance", () => {
    const prompt = constructLocalAgentPrompt(undefined, undefined, {
      frameworkType: "flutter",
      testingEnabled: true,
    });
  ...
  it("basic agent mode system prompt (flutter framework)", () => {
    const prompt = constructLocalAgentPrompt(undefined, undefined, {
      basicAgentMode: true,
      frameworkType: "flutter",
    });
