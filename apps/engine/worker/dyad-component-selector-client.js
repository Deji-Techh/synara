(() => {
  ...
  function skipOverlayElement(el) {
    ...
    if (tag === "img" || tag === "video" || tag === "canvas" || tag === "svg" || tag === "iframe") {
      return el;
    }
    ...
    if (e.data.type === "restore-dyad-component-overlays") {
      ...
      if (Array.isArray(componentIds)) {
        ...
        for (const id of componentIds) {
          const el = document.querySelector(`[data-dyad-id="${CSS.escape(id)}"]`);
          if (el) {
          ...
          if (mutation.type === "childList") {
            for (const node of mutation.addedNodes) {
              if (node.nodeType === Node.ELEMENT_NODE) {
                if (node.hasAttribute("data-dyad-id") || node.querySelector("[data-dyad-id]")) {
                  return true;
                }
              }
            }
