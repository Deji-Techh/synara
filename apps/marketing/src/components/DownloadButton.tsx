...
export default function DownloadButton({ className = "" }: { className?: string }) {
  ...
  return (
    <Link
      href="/install"
      className={`inline-flex min-w-[10.5rem] items-center justify-center gap-2 rounded-full bg-[var(--btn-primary-bg)] px-5 py-2.5 text-[13px] font-medium text-[var(--btn-primary-fg)] shadow-md transition-all hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent-link)] ${className}`}
    >
