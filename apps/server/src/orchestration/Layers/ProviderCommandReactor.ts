...
import { isTemporaryWorktreeBranch, WORKTREE_BRANCH_PREFIX } from "@caide/shared/git";
import { claudeSelectionRequiresRestart } from "@caide/shared/model";
...
} from "@caide/shared/providerDeliveryBlock";
import { buildStalePendingRequestFailureDetail } from "@caide/shared/threadSummary";
import { resolveThreadWorkspaceState } from "@caide/shared/threadEnvironment";
...
function buildGeneratedWorktreeBranchName(raw: string): string {
  ...
  const withoutPrefix = normalized.replace(/^caide\//, "");
...
>()("caide/orchestration/Layers/ProviderCommandReactorConfig") {}
    ...
    if (input.reviewTarget !== undefined) {
      ...
      const replayWithTranscriptBootstrap = (
        ...
        Effect.gen(function* () {
          ...
          // dead cursor and replay once with Caide transcript context.
          yield* clearStaleProviderResumeState({
            threadId: input.threadId,
            cause,
          ...
          if (!startupRecoveryNotifiedThreads.has(blocker.threadId)) {
            ...
            if (skippedPromptCount > 0) {
              ...
              yield* appendProviderFailureActivity({
                ...
                detail: `Caide recovered an earlier provider failure, but ${skippedPromptCount} ${noun} skipped while the thread was blocked. Resend ${skippedPromptCount === 1 ? "it" : "them"} to continue.`,
                turnId: null,
                createdAt,
              });
