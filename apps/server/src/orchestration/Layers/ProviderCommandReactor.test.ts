...
import { PROVIDER_DELIVERY_BLOCK_SUMMARY } from "@caide/shared/providerDeliveryBlock";
import {
  Duration,
  Effect,
  Exit,
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "caide-reactor-"));
  ...
  it("renames temporary worktree branches and keeps associated worktree metadata in sync", async () => {
    ...
    await waitFor(async () => {
      const thread = await readHarnessThread(harness);
      return (
        thread?.branch === "caide/app-startup-crash" &&
        thread.associatedWorktreeBranch === "caide/app-startup-crash" &&
        thread.associatedWorktreeRef === "caide/app-startup-crash"
      );
    });

    const thread = await readHarnessThread(harness);
    expect(thread).toMatchObject({
      branch: "caide/app-startup-crash",
      worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreeBranch: "caide/app-startup-crash",
      associatedWorktreeRef: "caide/app-startup-crash",
    });
  });
  ...
  it("uses the configured Git-writing model when the chat provider cannot generate names", async () => {
    ...
    expect(harness.renameBranch.mock.calls[0]?.[0]).toMatchObject({
      oldBranch: "caide/cb661f0d",
      newBranch: "caide/provider-startup-timeouts",
    });

    await waitFor(
      async () => (await readHarnessThread(harness))?.branch === "caide/provider-startup-timeouts",
    );
  });
