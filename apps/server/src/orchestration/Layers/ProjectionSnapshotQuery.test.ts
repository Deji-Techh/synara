...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  ...
  it.effect("normalizes imported Caide model-selection shapes from projection reads", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
  ...
  it.effect("decodes persisted lastKnownPr JSON in read and shell snapshots", () =>
    Effect.gen(function* () {
      ...
      assert.equal(shellSnapshot.threads[0]?.lastKnownPr?.headBranch, "caide/greeting-1");
    }),
  );
