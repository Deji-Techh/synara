...
const BaseTestLayer = makeProjectionPipelinePrefixedTestLayer("caide-projection-pipeline-test-");
...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("persists turn-start thread settings into projection rows", () =>
    Effect.gen(function* () {
      ...
      assert.equal(providerRows[0]!.providerName, "openai");
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-message-identity-scope-")))(
  "OrchestrationProjectionPipeline",
  (it) => {
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-approval-identity-scope-")))(
  "OrchestrationProjectionPipeline",
  (it) => {
    ...
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "caide-projection-pipeline-fast-forward-",
        }),
        NodeServices.layer,
      ),
    ),
...
it.effect("drains 2,501 file-backed events to a captured high-water fence", () =>
  Effect.gen(function* () {
    ...
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "caide-projection-pipeline-paged-",
        }),
        NodeServices.layer,
      ),
    ),
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-base-")))(
  "OrchestrationProjectionPipeline",
  (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-projection-pipeline-approvals-")),
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-projection-attachments-safe-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-projection-attachments-overwrite-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-projection-attachments-rollback-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-projection-attachments-revert-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-projection-attachments-delete-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.effect("restores pending turn-start metadata across projection pipeline restart", () =>
  Effect.gen(function* () {
    ...
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "caide-projection-pipeline-restart-",
        }),
        NodeServices.layer,
      ),
    ),
...
const engineLayer = it.layer(
  OrchestrationEngineLive.pipe(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "caide-projection-pipeline-engine-dispatch-",
      }),
    ),
    Layer.provideMerge(NodeServices.layer),
  ),
  ...
  it.effect("routes telemetry activities only through their owning hot projector", () =>
    Effect.gen(function* () {
      ...
      yield* engine.dispatch({
        ...
        defaultModelSelection: { provider: "openai", model: "gpt-5-codex" },
        createdAt,
      });
      yield* engine.dispatch({
        type: "thread.create",
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("caide-projection-pipeline-turn-finish-")),
)("OrchestrationProjectionPipeline", (it) => {
