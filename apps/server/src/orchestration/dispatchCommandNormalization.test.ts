...
function projectCreateCommand(
  ...
  return {
    ...
    kind: "chat",
    title: "Chat",
    workspaceRoot: "/Users/tester/Documents/Caide/2026-06-11/chat",
...
describe("makeDispatchCommandNormalizer", () => {
  it("returns a deferred prepare effect instead of scaffolding during normalization", async () => {
    ...
    const normalizer = makeDispatchCommandNormalizer<Error>({
      ...
      chatWorkspaceRoot: "/Users/tester/Documents/Caide",
    ...
    expect(preparedRoots).toEqual(["/Users/tester/Documents/Caide/2026-06-11/chat"]);
  });
  ...
  it("does not prepare ordinary projects or the chat workspace root itself", async () => {
    ...
    const first = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          workspaceRoot: "/Users/tester/Documents/Caide/2026-06-11/app",
        }),
      }),
    );
    ...
    const second = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          workspaceRoot: "/Users/tester/Documents/Caide",
        }),
      }),
    );
  ...
  it("prepares caide-apps workspace root when createWorkspaceRootIfMissing is true", async () => {
    const preparedRoots: string[] = [];
    const normalizer = makeDispatchCommandNormalizer<Error>({
      attachmentsDir: "/tmp/attachments",
      chatWorkspaceRoot: "/Users/tester/Documents/Caide",
      fileSystem: {} as FileSystem.FileSystem,
      path: {} as Path.Path,
      canonicalizeProjectWorkspaceRoot: (workspaceRoot: string) => Effect.succeed(workspaceRoot),
      prepareChatWorkspaceRoot: () => Effect.void,
      prepareCaideAppWorkspaceRoot: (workspaceRoot: string) =>
        Effect.sync(() => {
          preparedRoots.push(workspaceRoot);
        }),
    });
    ...
    const result = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          title: "Caide App",
          workspaceRoot: "/Users/tester/caide-apps/my-app",
          createWorkspaceRootIfMissing: true,
        }),
      }),
    );
    ...
    expect(preparedRoots).toEqual(["/Users/tester/caide-apps/my-app"]);
  });

  it("defers binary attachment authority to the transactional managed ledger", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "caide-dispatch-normalize-"));
