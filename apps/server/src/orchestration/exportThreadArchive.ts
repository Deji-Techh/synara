...
// Purpose: Build a ZIP archive that exports a single Caide thread so a user
...
// The export-eligibility guard lives in @caide/shared/threadExport so the
...
export function threadArchiveFileName(input: {
  ...
  return `caide-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
