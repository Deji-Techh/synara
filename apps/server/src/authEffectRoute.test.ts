...
import { AuthSessionId } from "@caide/contracts";
import {
...
function makeServerAuth(sideEffects: { count: number }): ServerAuthShape {
  ...
  const descriptor = {
    ...
    sessionCookieName: "caide_session",
  };
  ...
  return {
    ...
    authenticateHttpRequest: (request) => {
      ...
      const cookie = request.cookies.caide_session === "cookie-token";
      if (!bearer && !cookie) {
    ...
    issueStartupPairingUrl: () =>
      Effect.succeed("https://caide.example.test/pair#token=PAIRINGTOKEN"),
  ...
  return {
    ...
    headers: {
      ...
      ...(input.credential === "bearer"
        ...
        : { Cookie: "caide_session=cookie-token" }),
...
describe("authEffectRouteLayer", () => {
  ...
  it("rejects every cookie-authenticated mutation without a trusted origin", async () => {
    ...
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://caide.example.test/"),
  ...
  it("logs out either role and clears the exact cookie with secure public-mode attributes", async () => {
    ...
    await withAuthEffectServer(config, makeServerAuth(sideEffects), async (serverOrigin) => {
      const response = await fetch(
        ...
        mutationRequest({
          origin: "https://caide.example.test",
          credential: "cookie",
        }),
      );
      ...
      expect(cookie).toContain("caide_session=");
...
describe("binaryUploadEffectRouteLayer", () => {
  it("allows credentialed Canary attachment upload preflights", async () => {
    const config = {
      host: "127.0.0.1",
      attachmentsDir: fs.mkdtempSync(path.join(os.tmpdir(), "caide-upload-cors-")),
    } as ServerConfigShape;
    try {
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          expect(response.headers.get("access-control-allow-origin")).toBe("caide-canary://app");
  ...
  it("rejects ambient cookie uploads without an origin and accepts explicit bearer auth", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "caide-upload-route-"));
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://caide.example.test/"),
      attachmentsDir,
      ...
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          const cookieResponse = await fetch(url, {
            method: "POST",
            headers: { Cookie: "caide_session=cookie-token" },
