/**
 * Provider-facing config builders for the Caide agent gateway.
 *
 ...
 *
 * - Codex: `[mcp_servers.caide]` TOML block (streamable HTTP +
...
export const CAIDE_MCP_SERVER_NAME = "caide";
export const CAIDE_AGENT_GATEWAY_TOKEN_ENV = "CAIDE_AGENT_GATEWAY_TOKEN";
export const CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV = "CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN";
export const CAIDE_AGENT_GATEWAY_URL_ENV = "CAIDE_AGENT_GATEWAY_URL";
 ...
 * the block references an env var that Caide sets per app-server process.
 *
...
export function buildCodexMcpConfigToml(endpointUrl: string): string {
  return [
    `[mcp_servers.${CAIDE_MCP_SERVER_NAME}]`,
    `url = ${JSON.stringify(endpointUrl)}`,
    `bearer_token_env_var = ${JSON.stringify(CAIDE_AGENT_GATEWAY_TOKEN_ENV)}`,
    "",
    "[shell_environment_policy]",
    `exclude = [${JSON.stringify(CAIDE_AGENT_GATEWAY_TOKEN_ENV)}]`,
  ].join("\n");
}
 ...
 * provider process dedicated to the owning Caide thread or an exclusive
...
async function postAgentGatewayJsonRpc(input: {
  ...
  if (!response.ok) {
    throw new Error(`Caide MCP request failed with HTTP ${String(response.status)}.`);
  }
  const payload: unknown = await response.json();
  if (!isRecord(payload) || payload.jsonrpc !== "2.0") {
    throw new Error("Caide MCP returned an invalid JSON-RPC response.");
  }
  if ("error" in payload) {
    const failure = isRecord(payload.error) ? payload.error : null;
    throw new Error(failure?.message ? String(failure.message) : "Caide MCP request failed.");
  }
  if (payload.id !== id || !("result" in payload)) {
    throw new Error("Caide MCP returned a mismatched JSON-RPC response.");
  }
...
export async function listAgentGatewayMcpTools(input: {
  ...
  if (!isRecord(result) || !Array.isArray(result.tools)) {
    throw new Error("Caide MCP tools/list returned an invalid tool catalog.");
  }
  return result.tools.map((value) => {
    if (
      ...
      throw new Error("Caide MCP tools/list returned an invalid tool descriptor.");
    }
    return {
      name: value.name,
...
/**
 * Build the secret-free MCP fragment installed with Caide's Antigravity
  ...
  return {
    mcpServers: {
      [CAIDE_MCP_SERVER_NAME]: {
        ...
        env: {
          [CAIDE_AGENT_GATEWAY_URL_ENV]: `$${CAIDE_AGENT_GATEWAY_URL_ENV}`,
          [CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: `$${CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV}`,
 ...
 */
export function buildAcpCaideMcpServers(input: {
  readonly connection: AgentGatewayMcpConnection;
  ...
  return [
    {
      ...
      env: [
        { name: CAIDE_AGENT_GATEWAY_URL_ENV, value: input.connection.url },
        { name: CAIDE_AGENT_GATEWAY_TOKEN_ENV, value: input.connection.bearerToken },
      ],
    },
  ];
}
