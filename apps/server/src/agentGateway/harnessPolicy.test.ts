...
import {
  renderCaideHarnessPolicy,
  CAIDE_HARNESS_POLICY_MARKER,
  takeCaideHarnessPolicyForProviderSession,
  takeCaideHarnessPolicyTextPartForProviderSession,
  takeCaideHarnessPolicyForSession,
} from "./harnessPolicy.ts";

describe("Caide harness policy", () => {
  it("identifies Caide and explains exact batch coordination when MCP is available", () => {
    const policy = renderCaideHarnessPolicy({ gatewayControlAvailable: true });
    assert.include(policy, CAIDE_HARNESS_POLICY_MARKER);
    assert.include(policy, "Caide is the host and harness");
    assert.include(policy, "one exact caide_create_threads plan");
    assert.include(policy, "before returning an operationId");
    assert.include(policy, "caide_wait_for_threads");
    assert.include(policy, "Use the browser_* tools");
    assert.include(policy, "exact thread-scoped Electron page Caide surfaces to the user");
    ...
    assert.include(policy, "do not create Caide threads");
  ...
  it("never advertises gateway mutation to providers without scoped MCP", () => {
    const policy = renderCaideHarnessPolicy({ gatewayControlAvailable: false });
    assert.include(policy, "Caide MCP control is unavailable");
    assert.notInclude(policy, "one exact caide_create_threads plan");
  });

  it("delivers a private host-context block once per provider session", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    assert.include(
      takeCaideHarnessPolicyForSession(state, { gatewayControlAvailable: true }) ?? "",
      "<caide_host_context>",
    );
    assert.isNull(takeCaideHarnessPolicyForSession(state, { gatewayControlAvailable: true }));
  });
  ...
  it("delivers once on fresh/load/fork sessions for every scoped MCP provider", () => {
    for (const provider of [
      ...
      for (const lifecycle of ["fresh", "load", "fork"] as const) {
        const state: { harnessPolicyDelivered?: boolean } = {};
        const first =
          takeCaideHarnessPolicyTextPartForProviderSession(state, {
            provider,
            scopedGatewayConnectionAvailable: true,
          })?.text ?? "";
        assert.include(first, CAIDE_HARNESS_POLICY_MARKER, `${provider}/${lifecycle}`);
        assert.include(first, "Use the caide_* tools", `${provider}/${lifecycle}`);
        assert.isNull(
          takeCaideHarnessPolicyForProviderSession(state, {
            provider,
            scopedGatewayConnectionAvailable: true,
          }),
          `${provider}/${lifecycle}`,
  ...
  it("keeps OpenCode, Kilo, and Pi identity-only until scoped setup succeeds", () => {
    for (const provider of ["opencode", "kilo", "pi"] as const) {
      ...
      assert.include(text, CAIDE_HARNESS_POLICY_MARKER, provider);
      assert.include(text, "Caide MCP control is unavailable", provider);
      assert.notInclude(text, "one exact caide_create_threads plan", provider);
    }
  });
  ...
  it("teaches the device tools well enough for a plain prompt to work", () => {
    ...
    // a window the user is not watching and leaves the Caide pane empty. A
