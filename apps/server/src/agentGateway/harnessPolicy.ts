...
export const CAIDE_HARNESS_POLICY_VERSION = "2026-08-03.8";
export const CAIDE_HARNESS_POLICY_MARKER = `[Caide harness policy ${CAIDE_HARNESS_POLICY_VERSION}]`;

export interface CaideHarnessCapabilities {
 ...
 * Caide resources.
 */
export function renderCaideHarnessPolicy(capabilities: CaideHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        "Use the caide_* tools for Caide threads, projects, automations, and coordination.",
        "Use the browser_* tools autonomously whenever the user refers in any language to Caide's integrated, embedded, visible, or in-app browser. They are the canonical and complete control surface for that browser: do not load or use a generic Browser, Chrome, Computer Use, OS-automation, Node REPL, Playwright, or other browser-control skill/tool instead. They control the exact thread-scoped Electron page Caide surfaces to the user, including its live DOM, cookies, and session. The page may continue in the background while the user views another chat; browser actions must never change the user's active chat. When no assigned tab exists, start with browser_open rather than browser_navigate. Take a fresh semantic browser_snapshot before element actions and after navigation or human interaction, requesting an image only when semantics are insufficient.",
        ...
        "For thread discovery and diagnosis, use caide_list_threads, caide_read_thread, caide_read_thread_activity, caide_read_thread_events, caide_read_thread_runtime_events, and caide_diagnose_thread before inspecting Caide's SQLite files or process logs. Fall back to host storage only when a tool's coverage metadata says the required evidence is unavailable.",
        "Provider-native subagent or Task tools are implementation details: they do not create Caide threads and must not substitute for an explicit request to create Caide threads.",
        "For a plural thread request, submit one exact caide_create_threads plan. The array length is the exact requested count.",
        "If caide_create_threads rejects the plan during validation or preflight before returning an operationId, correct that same plan and retry it with the same requestId. This is safe because no durable operation, thread, or worktree was created.",
        "Use caide_capabilities to select canonical provider, model, and option values. Never guess a model slug or silently substitute a provider or model.",
        "Provider option keys are not interchangeable: Codex uses options.reasoningEffort and Claude Agent uses options.effort. Follow caide_capabilities.targetConstruction for every provider instead of inspecting Caide source code.",
        "When results are requested, call caide_wait_for_threads for the created thread ids, wait for every requested result, then synthesize all outcomes.",
        "After caide_create_threads returns an operationId, retries must keep the same requestId and exact plan. Report terminal operation failures as outcomes; do not create replacement threads unless the user gives a new instruction.",
        "Caide automations support heartbeat, standalone, and dedicated modes plus interval, once, daily, weekdays, weekly, and cron schedules. Existing everyMinutes heartbeat calls remain supported. Use fastInterval: true only when the user explicitly accepts a sub-minute bounded loop.",
        ...
        'Mode does not restrict stop conditions. completionPolicy {"type":"ai-evaluated","stopWhen":"..."} works in both modes and disables the automation when the clause matches a successful run; prefer it over encoding the stop condition in the prompt. maxIterations remains the backstop, and an automation-dispatched run may always call caide_cancel_automation on its own automation.',
        AUTOMATION_AUTHORING_GUIDANCE,
        "Prefer caide_create_automation with suggested: true when the user has not explicitly asked to create an automation. Suggested automations remain disabled until the user accepts their proposal card.",
        "Before caide_update_automation, call caide_view_automation and resend the complete mutable configuration, including unchanged fields. Updates are full replacement and partial payloads are rejected.",
        ...
        'During an automation-dispatched turn, persist durable context with caide_update_automation_memory {"memory": "..."} before finishing; memory is full replacement, DB-backed, and capped at 32 KiB.',
        'Every automation-dispatched turn must finish by calling caide_report_automation_result. Use decision "silent" only for a successful run with nothing requiring user attention; otherwise use "notify" with a concise title and summary. Failures remain visible regardless of this decision or the automation notification policy. Never call this tool for a manual follow-up turn.',
      ]
    : [
        "Caide MCP control is unavailable in this provider session. Do not claim that Caide threads, projects, or automations were created or changed.",
        "Provider-native subagent or Task tools do not create Caide threads. If the user explicitly requests Caide resource management, explain that this session cannot perform it.",
      ];

  return [
    CAIDE_HARNESS_POLICY_MARKER,
    "You are running inside Caide. Caide is the host and harness for this session.",
    ...controlPolicy,
  ].join("\n");
}

export const CAIDE_GATEWAY_HARNESS_POLICY = renderCaideHarnessPolicy({
  gatewayControlAvailable: true,
});

export const CAIDE_IDENTITY_ONLY_HARNESS_POLICY = renderCaideHarnessPolicy({
  gatewayControlAvailable: false,
});

export interface CaideHarnessPolicyDeliveryState {
...
export function providerHasCaideGatewayControl(input: {
  ...
  return (
    ...
    PROVIDERS_WITH_THREAD_SCOPED_CAIDE_MCP.has(input.provider)
  );
}

/** Return the private host-context block exactly once for one provider session. */
export function takeCaideHarnessPolicyForSession(
  state: CaideHarnessPolicyDeliveryState,
  capabilities: CaideHarnessCapabilities,
): string | null {
  if (state.harnessPolicyDelivered === true) return null;
  state.harnessPolicyDelivered = true;
  return [
    "<caide_host_context>",
    renderCaideHarnessPolicy(capabilities),
    "</caide_host_context>",
  ].join("\n");
}
 ...
 */
export function takeCaideHarnessPolicyForProviderSession(
  state: CaideHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  return takeCaideHarnessPolicyForSession(state, {
    gatewayControlAvailable: providerHasCaideGatewayControl(input),
  });
}

export function takeCaideHarnessPolicyTextPartForProviderSession(
  state: CaideHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  const text = takeCaideHarnessPolicyForProviderSession(state, input);
