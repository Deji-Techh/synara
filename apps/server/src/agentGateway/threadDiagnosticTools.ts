import { ThreadId, type OrchestrationEvent, type OrchestrationThreadShell } from "@caide/contracts";
...
export function makeThreadDiagnosticTools(input: {
  ...
  const readRuntimeEvents: ToolEntry = {
    requiredCapability: "diagnostics:read",
    definition: {
      name: "caide_read_thread_runtime_events",
      description:
  ...
  const diagnoseThread: ToolEntry = {
    ...
    definition: {
      ...
      annotations: { title: "Diagnose a Caide thread", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
