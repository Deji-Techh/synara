...
import { CAIDE_HARNESS_POLICY_VERSION } from "./harnessPolicy.ts";
...
export function makeThreadReadTools(input: ThreadReadToolsInput): ReadonlyArray<ToolEntry> {
  ...
  const contextTool: ToolEntry = {
    ...
    definition: {
      name: "caide_context",
      description:
        "Inspect the current Caide harness identity, caller thread/turn, and authorized coordination capabilities.",
    ...
    handler: (_args, context) =>
      Effect.gen(function* () {
        ...
        return mcpToolResultJson({
          harness: { name: "Caide", policyVersion: CAIDE_HARNESS_POLICY_VERSION },
          caller: {
            threadId: caller.id,
            turnId,
  ...
  const capabilitiesTool: ToolEntry = {
    ...
    definition: {
      ...
      description: `List canonical Caide provider/model targets, exact provider option keys, examples, and gateway limits used to validate thread creation. ${AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION}`,
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: {
        title: "Caide capabilities",
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
        openWorldHint: false,
    ...
    handler: (_args, context) =>
      Effect.gen(function* () {
        ...
        return mcpToolResultJson({
          ...
          providers,
          limits: {
            maxThreadsPerOperation: CAIDE_GATEWAY_MAX_THREADS_PER_OPERATION,
            maxWaitMs: 60_000,
  ...
  const listThreads: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Discover Caide threads by project, hierarchy, provider, model, status, title, creation source, or update window. Archived threads are hidden unless includeArchived is true.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "List Caide threads", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
  ...
  const readThread: ToolEntry = {
    requiredCapability: "thread:read",
    definition: {
      name: "caide_read_thread",
      description:
        "Read one Caide thread's status and recent messages (newest last, truncated). Pass the returned nextCursor as cursor to page older messages.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Read a Caide thread", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
                ...
                return Effect.succeed(
                  pinned.map((pin) => {
                    ...
                    return {
                      ...
                      readThread: {
                        tool: "caide_read_thread" as const,
