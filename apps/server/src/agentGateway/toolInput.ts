...
export const MODEL_SELECTION_INPUT_SCHEMA = {
  ...
  properties: {
    ...
    model: {
      type: "string",
      description: "Exact model slug from caide_capabilities providers[].models[].slug.",
    },
    options: {
      type: "object",
      description: AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION,
...
export function decodeCreateThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(CaideCreateThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Caide creation plan: ${errorText(error)}`);
  }
}

export function decodeWaitForThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(CaideWaitForThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Caide wait request: ${errorText(error)}`);
  }
}
