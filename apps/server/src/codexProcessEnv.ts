...
// Purpose: Builds the exact environment used when Caide launches Codex subprocesses.
...
import { readActiveCodexProviderEnvKey } from "@caide/shared/codexConfig";
import {
  readEnvironmentFromLoginShell,
  resolveLoginShell,
  type ShellEnvironmentReader,
} from "@caide/shared/shell";

import { resolveBaseCodexHomePath, resolveCaideCodexHomeOverlayPath } from "./codexHomePaths.ts";
import { buildProviderChildEnvironment } from "./providerChildEnvironment.ts";

const CODEX_PROCESS_SHELL_ENV_NAMES = ["PATH", "SSH_AUTH_SOCK"] as const;
const CODEX_OVERLAY_SHARED_STATE_FILES = new Set(["auth.json"]);
const CAIDE_CONFIG_SUPPRESSIONS_FILE = "caide-config-suppressions-v1.json";
const CAIDE_MANAGED_MCP_TABLE_HEADER = "[mcp_servers.caide]";
export const CAIDE_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS = [
...
export async function readCaideConfigSuppressions(markerPath: string): Promise<readonly string[]> {
  try {
...
async function writeCaideConfigSuppressions(
  markerPath: string,
  sectionHeaders: readonly string[],
): Promise<void> {
...
export const CAIDE_MANAGED_CODEX_CONFIG_BEGIN = "# >>> caide managed config >>>";
export const CAIDE_MANAGED_CODEX_CONFIG_END = "# <<< caide managed config <<<";

export function extractManagedCodexConfigSection(config: string): string | undefined {
  const begin = config.indexOf(CAIDE_MANAGED_CODEX_CONFIG_BEGIN);
  if (begin === -1) {
    return undefined;
  }
  const contentStart = begin + CAIDE_MANAGED_CODEX_CONFIG_BEGIN.length;
  const end = config.indexOf(CAIDE_MANAGED_CODEX_CONFIG_END, contentStart);
  if (end === -1) {
    return undefined;
  }
...
function appendManagedCodexConfigSection(config: string, section: string): string {
  ...
  const managedMcpTableName = normalizeTomlTableHeaderName(CAIDE_MANAGED_MCP_TABLE_HEADER);
  const tables: string[] = [];
  ...
  for (const table of splitTomlTables(section.trim())) {
    ...
    if (normalizeTomlTableHeaderName(header) === managedMcpTableName) {
      // The session-scoped gateway entry is authoritative inside Caide's
      // overlay. The user's source config remains untouched.
      overlayConfig = removeTomlTableNamespace(overlayConfig, CAIDE_MANAGED_MCP_TABLE_HEADER);
      tables.push(table);
      continue;
    }
...
async function prepareCaideCodexHomeOverlayUnlocked(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  const sourceHomePath = resolveBaseCodexHomePath(input.env, input.homePath);
  const overlayHomePath = resolveCaideCodexHomeOverlayPath(input.env, sourceHomePath);
  ...
  const suppressionMarkerPath = path.join(overlayHomePath, CAIDE_CONFIG_SUPPRESSIONS_FILE);
  const suppressedSections = [
    ...new Set([
      ...
      ...(await readCaideConfigSuppressions(suppressionMarkerPath)),
    ]),
  ...
  await writeCaideConfigSuppressions(suppressionMarkerPath, suppressedSections);

  return overlayHomePath;
}

async function prepareCaideCodexHomeOverlay(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  ...
  return serializeCodexOverlayPreparation(overlayHomePath, () =>
    prepareCaideCodexHomeOverlayUnlocked(input),
  );
}
...
export async function buildCodexProcessEnv(
  ...
  const overlayHomePath = await prepareCaideCodexHomeOverlay({
    env: baseEnv,
