...
async function fetchLocalPageTitleResponse(
  ...
  const response = await globalThis.fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "CaideLocalServerMonitor/1.0",
    },
  });
...
function toServerProcess(
  ...
  const server: ServerLocalServerProcess = {
    ...
    addresses,
    isStoppable,
    ...(isStoppable ? {} : { stopDisabledReason: "Caide cannot signal this process." }),
  };
...
export async function stopLocalServer(
  ...
  if (!target.isStoppable) {
    return {
      pid: input.pid,
      stopped: false,
      message: target.stopDisabledReason ?? "Caide cannot stop this process.",
    };
  }
