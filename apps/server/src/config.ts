...
export function remoteAccessPolicyError(
  ...
  if (config.publicUrl && !normalizeHttpsPublicOrigin(config.publicUrl)) {
    return "CAIDE_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://caide.example.com).";
  }
  const isPubliclyExposed = isRemoteBind || Boolean(config.publicUrl);
  if (!isPubliclyExposed) return null;
  if (!config.authToken?.trim()) {
    return config.publicUrl
      ? "Refusing to publish Caide through CAIDE_PUBLIC_URL/--public-url without CAIDE_AUTH_TOKEN/--auth-token."
      : `Refusing to bind Caide to non-loopback host ${config.host ?? "<unspecified>"} without CAIDE_AUTH_TOKEN/--auth-token.`;
  }
  if (config.devUrl) {
  ...
  if (isRemoteBind && !config.publicUrl && !config.allowInsecureRemote) {
    return "Refusing plaintext remote access. Configure an HTTPS reverse-proxy origin with CAIDE_PUBLIC_URL/--public-url, or explicitly accept unencrypted LAN traffic with CAIDE_ALLOW_INSECURE_REMOTE/--allow-insecure-remote.";
  }
  return null;
}
...
export function resolveDefaultChatWorkspaceRoot(input: {
  ...
  return pathApi.join(homeDir, "Documents", "Caide");
}
...
/**
 * resolveCanonicalWorkspaceRoots - Derives homeDir/chatWorkspaceRoot and canonicalizes
 * each via {@link realpathNearestExisting}.
 *
 ...
 * to. Roots may not exist yet, so canonicalization walks up to the nearest
 */
...
export const resolveStaticDir = Effect.fn(function* () {
  ...
  const snapshotDir = process.env.CAIDE_STATIC_DIR?.trim();
  if (snapshotDir) {
