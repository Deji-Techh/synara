...
/**
 * A tracker identity that a *released* Caide build wrote for a migration whose
 ...
 * does the migration Caide registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Caide's latest ID,
 * every Caide migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Caide's lineage and
 ...
 *
 * Divergences caused by Caide renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Caide build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Caide migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Caide build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Caide lineage (legacy import); re-running migrations from the divergence point",
