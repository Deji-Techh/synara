...
// Purpose: Compute Profile-page stats from Caide's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@caide/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored Caide user message, plus the
...
>()("caide/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Caide prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from Caide's own DB (no external
