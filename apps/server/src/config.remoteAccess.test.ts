...
describe("remote access policy", () => {
  ...
  it("requires authentication when an HTTPS proxy publishes a loopback bind", () => {
    ...
    ).toContain("without CAIDE_AUTH_TOKEN");
  });

  it("rejects invalid public URLs in the shared embedded-server policy", () => {
    for (const publicUrl of [
      new URL("http://caide.example.test/"),
      new URL("https://caide.example.test/app"),
    ]) {
      expect(
        remoteAccessPolicyError({
          ...remoteBase,
  ...
  it("accepts only credential-free HTTPS root origins", () => {
    expect(normalizeHttpsPublicOrigin(new URL("https://caide.example.test/"))?.origin).toBe(
      "https://caide.example.test",
    );
    for (const value of [
      "http://caide.example.test/",
      "https://user:pass@caide.example.test/",
      "https://caide.example.test/app",
      "https://caide.example.test/?query=1",
      "https://caide.example.test/#fragment",
    ]) {
