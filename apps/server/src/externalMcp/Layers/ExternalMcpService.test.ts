...
const layer = ExternalMcpServiceLive.pipe(
  ...
  Layer.provide(Layer.succeed(ServerConfig, { baseDir: "/tmp/caide-test" } as never)),
);
  ...
  it("issues one pairing exchange, verifies it, and revokes it immediately", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(created.stdio.args).toContain("/tmp/caide-test");
    ...
    await run(
      Effect.gen(function* () {
        ...
        const auditId = yield* service.beginAudit(client, {
          tool: "caide_list_allowed_projects",
        });
        yield* sql`
