...
describe("buildProviderChildEnvironment", () => {
  it("strips Caide control-plane and inherited native capabilities", () => {
    const env = buildProviderChildEnvironment({
      ...
      baseEnv: {
        ...
        CAIDE_BROWSER_HOST_CAPABILITY: "private-desktop-capability",
  ...
  it("admits only explicitly granted capability keys", () => {
    const env = buildProviderChildEnvironment({
      ...
      inheritedCaideKeys: ["CAIDE_BROWSER_USE_PIPE_PATH"],
  ...
  it("does not let overlays bypass the capability policy", () => {
    const env = buildProviderChildEnvironment({
      ...
      overrides: {
        ...
        CAIDE_AUTH_TOKEN: "overlaid-control-plane-secret",
        NODE_OPTIONS: "--require=/tmp/inject.js",
      },
    });
  ...
  it.each(["codex", "engine", "opencode", "pi"] as const)(
  ...
  it("keeps stripped authority absent in descendants", () => {
    ...
    const descendantScript =
      "process.stdout.write(JSON.stringify({ xai: process.env.XAI_API_KEY, anthropic: process.env.ANTHROPIC_API_KEY, caide: process.env.CAIDE_AUTH_TOKEN }))";
