...
import { MessageId, ThreadId, TurnId } from "@caide/contracts";
...
const testLayer = Layer.mergeAll(ProfileStatsQueryLive, ProfileStatsArchiveLive).pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "caide-profile-stats-archive-test-",
    }),
  ),
      ...
      Effect.gen(function* () {
        ...
        yield* sql`
          INSERT INTO external_mcp_integrations (
            ...
            'integration-purge', 'Purge integration', 'caide.external-mcp', 'other',
