...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("caide-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("caide-codex-home-");
  ...
  it("allows images written to the CAIDE_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Caide overlay
    // directory (see resolveCaideCodexHomeOverlayPath). Generated images therefore
    // live under <CAIDE_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const caideHome = path.join(fakeRoot, "caide", "runtime");
    ...
    process.env.CAIDE_HOME = caideHome;
    try {
      const result = await resolveAllowedLocalPreviewFile({
        requestedPath: imagePath,
        cwd: null,
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("caide-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/caide-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "caide-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("caide-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("caide-image-tmp-root-");
