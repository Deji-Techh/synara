...
//          that makes Caide catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@caide/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    case "engine":
      return !segments.has(".engine");
    case "opencodeZen":
    case "opencodeGo":
      return !segments.has(".opencode");
    case "groq":
    default:
      return true;
  }
