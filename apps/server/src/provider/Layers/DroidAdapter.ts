...
export const takeDroidCaideHarnessPolicyTextPart = (
...
const DROID_ACP_DEBUG_ENV = "CAIDE_DROID_ACP_DEBUG";
...
// override with CAIDE_DROID_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const DROID_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "CAIDE_DROID_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const DROID_PLAN_MODE_PROMPT_PREFIX = [
  "Caide Droid plan mode is active.",
...
interface DroidSessionContext {
  ...
  /** Turns cancelled by Caide only because their Plan proposal was captured. */
  ...
  return Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          if (resumeSessionId !== undefined && started.sessionSetupMethod === "new") {
            return yield* new ProviderAdapterRequestError({
              ...
              detail:
                "Droid could not resume the requested native session. Caide refused the fresh fallback to avoid silently losing conversation context.",
            });
          }
    ...
    const startDroidTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeDroidCaideHarnessPolicyTextPart(
          ctx,
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Droid ACP version does not advertise session/fork; Caide will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Droid session has a turn in flight; Caide will rebuild the fork from its retained transcript.",
          });
        }
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Caide Model Discovery",
          });
          yield* runtime.start();
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Caide Command Discovery",
          });
          yield* runtime.start();
