...
import { DEFAULT_SERVER_SETTINGS, ServerProviderUpdateError } from "@caide/contracts";
...
import { CAIDE_CODEX_HOME_OVERLAY_DIR } from "../../codexHomePaths";
...
function withTempCodexHome(configContent?: string) {
  return Effect.gen(function* () {
    ...
    const tmpDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "caide-test-codex-" });
    const runtimeDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "caide-test-runtime-",
    });
  ...
  describe("disabled provider handling", () => {
    it("builds an inert status for disabled providers", () => {
      assert.deepStrictEqual(makeDisabledProviderStatus("kilo", "2026-06-16T12:00:00.000Z"), {
        ...
        authStatus: "unknown",
        checkedAt: "2026-06-16T12:00:00.000Z",
        message: "Provider is disabled in Caide settings.",
      });
    });
  ...
  describe("checkPiProviderStatus", () => {
    ...
    it.effect("uses configured Pi binary and agent dir without SDK registry reads", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Pi CLI is installed. Caide will use Pi agent dir /tmp/pi-agent.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args, command) => {
  ...
  describe("checkAntigravityProviderStatus", () => {
    it.effect("rejects versions that predate --new-project support", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Antigravity CLI 1.0.11 is too old for Caide. Upgrade to 1.0.12 or newer.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args) => {
