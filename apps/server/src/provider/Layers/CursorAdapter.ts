...
export const takeCursorCaideHarnessPolicyTextPart = (
...
// forever. Generous by design; override with CAIDE_CURSOR_TURN_IDLE_TIMEOUT_MS.
const CURSOR_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "CAIDE_CURSOR_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const CURSOR_PLAN_MODE_PROMPT_PREFIX = [
  "Caide Cursor plan mode is active.",
  ...
  return Effect.gen(function* () {
    ...
    const sendTurn: CursorAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeCursorCaideHarnessPolicyTextPart(
          ctx,
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Cursor ACP version does not advertise session/fork; Caide will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Cursor session has a turn in flight; Caide will rebuild the fork from its retained transcript.",
          });
        }
