...
import {
  ...
  takeGrokCaideHarnessPolicyTextPart,
} from "./GrokAdapter.ts";

describe("Grok Caide harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeGrokCaideHarnessPolicyTextPart(state, true)?.text).toContain(
      CAIDE_HARNESS_POLICY_MARKER,
    );
    expect(takeGrokCaideHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
...
describe("Grok native plan approval", () => {
  it("adds Plan instructions without sending a pager-only slash command as model text", () => {
    ...
    ).toMatch(/^Caide requested Grok's native plan mode\./u);
  });
  ...
  it("backs native Plan mode with a fail-closed pre-tool hook", () => {
    expect(
      resolveGrokPlanHookResponse("plan", {
        hookCallbackId: "caide-plan-guard",
  ...
  it("keeps native plan mode gated after Caide captures the plan", () => {
    expect(makeGrokExitPlanModeCapturedResponse()).toEqual({
      outcome: "cancelled",
      feedback:
        "Caide captured this plan for user review. Do not revise or implement it now. End this turn and wait for the user's next message.",
    });
  });
...
describe("Grok native user questions", () => {
  ...
  it("maps Caide answers to Grok's question-text keyed response", () => {
