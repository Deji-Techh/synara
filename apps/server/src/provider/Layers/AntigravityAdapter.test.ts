...
describe("Antigravity CLI integration helpers", () => {
  it("rotates the gateway lease per print turn and rejects a retained prior bootstrap", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-turn-lease-"));
    ...
    try {
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const bootstrapA = spawnedEnvironments[0]?.CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
          ...
          const bootstrapB = spawnedEnvironments[1]?.CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
  ...
  it("installs the generated Caide MCP plugin alongside the capture hooks", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-home-test-"));
    const stdioProxy = {
      ...
      expect(
        ...
        mcpServers: {
          caide: {
            ...
            env: {
              CAIDE_AGENT_GATEWAY_URL: "$CAIDE_AGENT_GATEWAY_URL",
              CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "$CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN",
  ...
  it("gives an Antigravity turn only its thread-scoped gateway credential", () => {
    const env = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        ...
        CAIDE_AGENT_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        CAIDE_AGENT_GATEWAY_TOKEN: "stale-token",
        CAIDE_AUTH_TOKEN: "host-control-plane-token",
        CAIDE_BROWSER_HOST_PIPE_PATH: "/tmp/desktop.sock",
        CAIDE_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock",
  ...
  it("advertises canonical browser tools only while the session owns a gateway lease", () => {
    ...
    expect(identityOnlyPrompt).toContain("Caide MCP control is unavailable");
    ...
    const envWithoutLease = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        CAIDE_AGENT_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        CAIDE_AGENT_GATEWAY_TOKEN: "stale-token",
        CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "stale-bootstrap",
      },
    });
    expect(envWithoutLease.CAIDE_AGENT_GATEWAY_URL).toBeUndefined();
    expect(envWithoutLease.CAIDE_AGENT_GATEWAY_TOKEN).toBeUndefined();
    expect(envWithoutLease.CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN).toBeUndefined();
  });
  ...
  it("keeps the globally installed hook neutral outside Caide sessions", () => {
    const command = buildAntigravityCaptureCommand(
      "__caide_gui_must_not_launch__",
  ...
  it("answers pre-tool with a decision from the capture script when capture is inactive", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-hook-test-"));
    ...
    try {
      ...
      const result = spawnSync(process.execPath, [scriptPath, "pre-tool"], {
        env: { ...process.env, CAIDE_ANTIGRAVITY_EVENTS: "" },
  ...
  it("runs the capture script for Caide-managed sessions", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-hook-test-"));
    const scriptPath = path.join(directory, "capture.cjs");
    ...
    try {
      ...
      const result = runCaptureCommand(command, payload, {
        CAIDE_ANTIGRAVITY_EVENTS: eventPath,
  ...
  it("runs packaged Electron as Node only for Caide-managed sessions", () => {
    expect(
      buildAntigravityCaptureCommand(
        "/Applications/Caide.app/Contents/MacOS/Caide",
        "/tmp/caide-capture/capture.cjs",
        "pre-tool",
        "darwin",
      ),
    ).toBe(
      `if [ -z "\${CAIDE_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '{"decision":"ask"}'; else ELECTRON_RUN_AS_NODE=1 '/Applications/Caide.app/Contents/MacOS/Caide' '/tmp/caide-capture/capture.cjs' 'pre-tool'; fi`,
    );
    expect(
      buildAntigravityCaptureCommand(
        String.raw`C:\Program Files\Caide\Caide.exe`,
        String.raw`C:\Users\test\.gemini\capture.cjs`,
        "pre-tool",
        "win32",
      ),
    ).toBe(
      String.raw`if not defined CAIDE_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo {"decision":"ask"}) else (set "ELECTRON_RUN_AS_NODE=1" && "C:\Program Files\Caide\Caide.exe" "C:\Users\test\.gemini\capture.cjs" "pre-tool")`,
    );
  });
  ...
  it("advances file offsets only past complete JSONL records", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-test-"));
  ...
  it("streams hook tool names and terminal states without arguments", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-tool-events-"));
    ...
    const spawnProcess = ((
      ...
      eventFile = options.env?.CAIDE_ANTIGRAVITY_EVENTS;
  ...
  it("answers stop hooks with a neutral allow-exit payload", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-stop-hook-"));
    const scriptPath = path.join(directory, "capture.cjs");
    const eventPath = path.join(directory, "events.ndjson");
    try {
      await fs.writeFile(scriptPath, hookScriptSource(), { mode: 0o700 });
      const result = spawnSync(process.execPath, [scriptPath, "stop"], {
        env: { ...process.env, CAIDE_ANTIGRAVITY_EVENTS: eventPath },
...
describe("Antigravity turn settle on cancel (#465)", () => {
  ...
  it("unlocks Cancel without letting a late close settle the follow-up", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-interrupt-hung-"));
