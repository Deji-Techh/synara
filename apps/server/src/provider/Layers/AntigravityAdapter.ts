...
import {
  ...
  CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV,
  CAIDE_AGENT_GATEWAY_URL_ENV,
...
/**
 * Hook output when capture is inactive (the session is not Caide-managed).
...
export function buildAntigravityCaptureCommand(
  ...
  if (platform === "win32") {
    return `if not defined CAIDE_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo ${fallback}) else (set "ELECTRON_RUN_AS_NODE=1" && ${invocation})`;
  }
  return `if [ -z "\${CAIDE_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '${fallback}'; else ELECTRON_RUN_AS_NODE=1 ${invocation}; fi`;
}
...
process.stdin.on("end", () => {
  const target = process.env.CAIDE_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.CAIDE_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    ...
    `${JSON.stringify(
      {
        ...
        name: "caide-capture",
        description: "Streams Antigravity CLI lifecycle events to Caide when requested.",
      },
      null,
      2,
    )}\n`,
...
export function buildAntigravityTurnProcessEnvironment(input: {
  ...
  const gatewayKeys = hasGatewayBootstrap
    ? [CAIDE_AGENT_GATEWAY_URL_ENV, CAIDE_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]
    : [];
  const gatewayEnvironment = hasGatewayBootstrap
    ? {
        [CAIDE_AGENT_GATEWAY_URL_ENV]: input.gatewayConnection!.url,
...
export function parseAntigravityCliModelLabel(
  ...
  // Caide never treats `slug\tName` as a single model id at dispatch.
        ...
        yield* Effect.tryPromise({
          try: () =>
            ...
            new ProviderAdapterRequestError({
              ...
              detail: messageFromCause(cause, "Failed to install the Caide capture hook."),
              cause,
            }),
        });
    ...
    const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const runDir = yield* Effect.tryPromise({
          try: () => fs.mkdtemp(path.join(os.tmpdir(), "caide-antigravity-")),
          catch: (cause) =>
            new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "turn/prepare",
        ...
        if (gatewaySessionLease && !gatewayBootstrapToken) {
          ...
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
            detail: "The Caide gateway credential is no longer active for this provider turn.",
          });
        }
