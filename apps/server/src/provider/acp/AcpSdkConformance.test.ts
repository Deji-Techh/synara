// Verifies the current Caide ACP boundary against an official-SDK subprocess.
...
function createFixtureLog(): string {
  const directory = mkdtempSync(path.join(os.tmpdir(), "caide-acp-conformance-"));
...
function runtimeLayer(logPath: string, env: Record<string, string> = {}) {
  return AcpSessionRuntime.layer({
    spawn: {
      ...
      env: {
        VITEST: "true",
        CAIDE_ACP_CONFORMANCE_LOG_PATH: logPath,
        ...env,
      },
    },
    cwd: process.cwd(),
    ...
    clientInfo: { name: "caide-conformance-test", version: "0.0.0" },
    authMethodId: "test",
    authenticateMeta: {
      primitive: 11,
      nested: { source: "caide-auth" },
    },
  });
}
...
describe("official ACP SDK conformance at the current Caide boundary", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        runtimeLayer(logPath, {
          CAIDE_ACP_CONFORMANCE_MALFORMED_PREFIX: "1",
        }),
      ),
      Effect.scoped,
      Effect.provide(NodeServices.layer),
  ...
  it.effect("round-trips extension requests and delivers extension notifications", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runtime.notify("conformance/notice", {
        primitive: false,
        nested: { source: "caide-notice" },
      });
      const response = yield* runtime.request("conformance/echo", {
        primitive: 42,
        nested: { source: "caide-request" },
      });
  ...
  it.effect("cancels an extension request through official-SDK $/cancel_request", () => {
    ...
    return Effect.gen(function* () {
      ...
      const request = yield* runtime
        .request("conformance/wait-for-generic-cancel", { source: "caide" })
        .pipe(Effect.forkChild);
...
describe("official ACP SDK client against the official SDK mock agent", () => {
  test("completes initialize, authentication, session, prompt, updates, stop, and teardown", async () => {
    ...
    const child = spawn(process.execPath, [officialMockFixturePath], {
      ...
      env: {
        ...process.env,
        CAIDE_ACP_REQUEST_LOG_PATH: requestLogPath,
        CAIDE_ACP_EXIT_LOG_PATH: exitLogPath,
        CAIDE_ACP_PROMPT_RESPONSE_TEXT: "mock says héllo 👋",
      },
