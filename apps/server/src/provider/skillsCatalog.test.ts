...
import type { ProviderSkillDescriptor } from "@caide/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "caide-skills-catalog-"));
  homeDir = path.join(root, "home");
  caideBaseDir = path.join(homeDir, ".caide");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the Caide skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, caideBaseDir });
    await expect(access(path.join(caideBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from caide and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(caideBaseDir, "skills", "portable"), "portable", "Caide skill");
  ...
  it("includes user and matching project Claude plugins but excludes other projects", async () => {
    ...
    const skills = await discoverSkillsCatalog({ cwd, homeDir, caideBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Caide for that provider", async () => {
    await writeSkill(path.join(caideBaseDir, "skills", "shared"), "shared", "Caide copy");
    ...
    const codexView = await discoverSkillsCatalog({ homeDir, caideBaseDir });
    ...
    // A provider without its own copy resolves the Caide fallback.
  ...
  it("uses documented provider alias roots before Caide fallbacks", async () => {
    await writeSkill(path.join(caideBaseDir, "skills", "shared"), "shared", "Caide copy");
  ...
  it("uses provider-native roots before shared aliases for Grok and Pi", async () => {
    ...
    expect(piView.find((skill) => skill.name === "shared")?.scope).toBe("grok");
  });
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(caideBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, caideBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(caideBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, caideBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, caideBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .caide skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".caide", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(caideBaseDir, "skills", "portable"), "portable", "Caide");
  ...
  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(caideBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(caideBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "caide"), descriptor("extra", "caide")],
    });
