...
import type { ChatAttachment } from "@caide/contracts";
import { resolveWindowsComSpec } from "@caide/shared/windowsProcess";
...
describe("toOpenCodeFileParts", () => {
  it("materializes image attachments as SDK file parts", () => {
    ...
    expect(
      toOpenCodeFileParts({
        ...
        resolveAttachmentPath: () => "/tmp/caide-attachments/screenshot.png",
      }),
    ).toEqual([
      {
        type: "file",
        ...
        url: "file:///tmp/caide-attachments/screenshot.png",
      },
    ]);
  });
  ...
  it("leaves generic files for prompt-path projection", () => {
    ...
    expect(
      toOpenCodeFileParts({
        attachments: [attachment],
        resolveAttachmentPath: () => "/tmp/caide-attachments/notes.docx",
      }),
    ).toEqual([]);
  });
});
...
describe("buildOpenCodeServerProcessEnv", () => {
  ...
  it("strips inherited Caide authority from managed server processes", () => {
    const env = buildOpenCodeServerProcessEnv({
      baseEnv: {
        OPENAI_API_KEY: "provider-key",
        CAIDE_AUTH_TOKEN: "server-secret",
