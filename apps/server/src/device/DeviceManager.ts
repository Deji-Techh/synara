 ...
 * - The Caide boot cap (`DEVICE_CAIDE_BOOT_LIMIT`). Boot past the cap is
...
/** How long a Caide-booted device stays up with no thread attached. */
...
export interface DeviceManagerOptions {
  ...
  /** Remembers Caide's boots across a crash; defaults to remembering nothing. */
...
export class DeviceManager {
  ...
  private readonly caideBooted = new Set<string>();
  ...
  constructor(options: DeviceManagerOptions) {
    ...
    this.bootLimit = options.bootLimit ?? DEVICE_CAIDE_BOOT_LIMIT;
  ...
  private async recordBootOwnership(): Promise<void> {
    await this.bootOwnership.write([...this.caideBooted]).catch(() => undefined);
  }
  ...
  async caideBootedDevices(): Promise<readonly DeviceDescriptor[]> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    this.reconcileCaideBooted(devices);
    return devices
      .filter((device) => this.caideBooted.has(device.udid))
   ...
   *
   * The set is Caide's own bookkeeping, but the simulators are not Caide's to
   ...
   */
  private reconcileCaideBooted(devices: readonly DeviceDescriptor[]): void {
    const running = new Set(
      devices
        .filter((device) => device.state === "booted" || device.state === "booting")
        .map((device) => device.udid),
    );
    for (const udid of this.caideBooted) {
      if (running.has(udid)) continue;
      this.caideBooted.delete(udid);
  ...
  async boot(udid: string): Promise<DeviceBootResult> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    // Devices that stopped without Caide doing it still held their slots, so
    ...
    // Caide from accumulating simulators, not to limit what the user watches.
    if (known?.state === "booted") {
      return { kind: "booted", device: this.describe(known) };
    }
    if (this.caideBooted.size >= this.bootLimit) {
      return {
        kind: "boot-limit-reached",
        limit: this.bootLimit,
        caideBooted: await this.caideBootedDevices(),
      };
    }
    ...
    // exists to stop Caide accumulating multi-gigabyte simulators would be
    // exceeded by however many requests arrived inside that window.
    this.caideBooted.add(udid);
    ...
    return { kind: "booted", device: { ...device, bootSource: "caide" } };
  }
  ...
  /**
   * App quit: shut down everything Caide booted, leave the user's devices
  ...
  async dispose(): Promise<void> {
    ...
    const booted = Array.from(this.caideBooted);
  ...
  private describe(device: DeviceDescriptor): DeviceDescriptor {
    const bootSource = this.caideBooted.has(device.udid) ? "caide" : device.bootSource;
   ...
   * the device down or start the idle countdown if Caide booted it.
   *
   * A switch shuts down immediately rather than waiting out the idle window.
   * The cap is three Caide-booted devices, and each one costs a couple of GB
  ...
  private async releaseDevice(
    ...
    if (!this.caideBooted.has(udid)) return;
    if (reason === "switched") {
      this.clearIdleTimer(udid);
  ...
  private async shutdownIfStillIdle(udid: string): Promise<void> {
    ...
    if (this.isAttachedAnywhere(udid) || !this.caideBooted.has(udid)) return;
    await this.shutdown(udid).catch(() => undefined);
  }
