 ...
 * directory so both land in the same place under a custom CAIDE_HOME.
 *
...
function defaultBootOwnershipPath(): string {
  const baseDir = process.env.CAIDE_HOME?.trim() || path.join(homedir(), ".caide");
          ...
          if (reclaimed.length > 0) {
            console.info(
              `[device] shut down ${reclaimed.length} simulator(s) left booted by a previous ` +
                `Caide run: ${reclaimed.join(", ")}`,
            );
          }
        });
      }

      // App quit shuts down every simulator Caide booted and leaves the
