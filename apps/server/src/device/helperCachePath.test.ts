...
describe("helper cache path agreement", () => {
  it("builds into the directory the shared key names", async () => {
    ...
    const backend = new IosSimulatorBackend({
      platform: "darwin",
      helperSourceDir: "/tmp/caide-helper-src",
      helperCacheRoot: "/tmp/caide-helper-cache",
    ...
    expect(build?.args[1]).toBe(join("/tmp/caide-helper-cache", expectedKey));
  });
  ...
  it("names the binary the build script produces", () => {
    expect(DEVICE_HELPER_BINARY_NAME).toBe("caide-device-helper");
  });
});
