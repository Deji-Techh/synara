...
function makeBackend() {
  ...
  const backend = new IosSimulatorBackend({
    platform: "darwin",
    helperCacheRoot: "/tmp/caide-device-test-cache",
  ...
  Object.defineProperty(backend, "compileHelperIfNeeded", {
    value: async () => "/tmp/caide-device-test-cache/caide-device-helper",
  });
...
describe("simulator reboot", () => {
  it("re-attaches after a Caide-driven shutdown and reboot", async () => {
  ...
  it("recovers when the device was rebooted outside Caide", async () => {
    const { backend, helper } = makeBackend();
    await backend.tap(DEVICE, 10, 10);
