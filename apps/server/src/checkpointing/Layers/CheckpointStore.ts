  ...
  const captureCheckpointOnce: CheckpointStoreShape["captureCheckpoint"] = (input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "caide-fs-checkpoint-" }),
        (tempDir) =>
          Effect.gen(function* () {
            ...
            const commitEnv: NodeJS.ProcessEnv = {
              ...
              GIT_AUTHOR_NAME: "Caide",
              GIT_AUTHOR_EMAIL: "caide@users.noreply.github.com",
              GIT_COMMITTER_NAME: "Caide",
              GIT_COMMITTER_EMAIL: "caide@users.noreply.github.com",
            };
            ...
            const message = `Caide checkpoint ref=${input.checkpointRef}`;
  ...
  const reverseCheckpointDiff: CheckpointStoreShape["reverseCheckpointDiff"] = (input) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "caide-checkpoint-undo-" }),
        (tempDir) =>
          Effect.gen(function* () {
