...
import type { ProviderRuntimeEvent } from "@caide/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousCaideHome === undefined) delete process.env.CAIDE_HOME;
    else process.env.CAIDE_HOME = previousCaideHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.CAIDE_HOME = "/caide-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/caide-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.caide/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.caide/runtime/codex-home-overlay";
