...
const verifyDesignSchema = z.object({
  scope: z
    ...
    .optional()
    .describe(
      "Audit scope (default ui). 'all' adds motion-spec enforcement for multi-screen apps.",
    ),
});
...
export function auditDesignWorkspace(appPath: string, scope: "ui" | "all" = "ui"): DesignFinding[] {
  ...
  const designSpec = readJsonSafe(path.join(appPath, ".caide", "design-spec.json")) as
  ...
  if (!designSpec || typeof designSpec !== "object") {
    findings.push({
      ...
      check: "design-spec",
      message:
        "Missing .caide/design-spec.json — create it before UI work (tokens, screens, dark mode, imagery strategy).",
    });
  } else {
    ...
    for (const key of REQUIRED_TOKEN_KEYS) {
      if (typeof tokens[key] !== "string" || !(tokens[key] as string).trim()) {
        findings.push({
          level: "major",
          check: "design-spec",
          message: `colorTokens.${key} missing or empty in .caide/design-spec.json.`,
        });
      }
    }
    ...
    if (!dark || dark.enabled !== true) {
      findings.push({
        ...
        check: "dark-mode",
        message:
          "design-spec darkMode.enabled is not true — every screen must render in light AND dark from day one.",
      });
    }
    ...
    if (!imagery || typeof imagery.strategy !== "string") {
      findings.push({
        ...
        check: "imagery",
        message:
          "design-spec imagery.strategy unset — lock generated | user-provided | illustrated-placeholder | mixed.",
      });
    }
    const viewports = Array.isArray(designSpec.viewportsVerified)
      ? (designSpec.viewportsVerified as unknown[])
      : [];
    ...
    if (missing.length > 0) {
      findings.push({
        level: "minor",
        check: "viewports",
        message: `Viewports not yet verified: ${missing.join(", ")}.`,
      });
    }
  }

  // --- motion-spec.json ---
  const motionSpec = readJsonSafe(path.join(appPath, ".caide", "motion-spec.json")) as
    | Record<string, unknown>
    | undefined;
  ...
  if (!motionSpec || !Array.isArray(motionSpec.entries)) {
    findings.push({
      ...
      check: "motion-spec",
      message:
        "Missing .caide/motion-spec.json entries — storyboard every consequential state change (trigger, technique, engine, timing, interruption, reduced-motion fallback).",
    });
  } else if (
    (motionSpec.entries as unknown[]).some(
      (e) => typeof (e as Record<string, unknown>).reducedMotionFallback !== "string",
    )
  ) {
    findings.push({
      level: "minor",
      check: "motion-spec",
      message:
        "Some motion entries lack reducedMotionFallback — honor reduce-motion on every animation.",
    });
  }
  ...
  for (const file of files) {
      ...
      findings.push(
        isFlutter
          ? {
              ...
              check: "imagery",
              message:
                "Colored-initials block — fine for CircleAvatar avatars, but never as product photos; use generated imagery or the image-asset patterns.",
            }
          : {
              level: "major",
              file: r,
              check: "imagery",
              message:
                "Flat-color initials block detected — never ship initials-on-color as product photos; use generate_image or the image-asset patterns.",
            },
      );
    }
    if (/@expo\/vector-icons|Ionicons|MaterialIcons|FontAwesome|Feather/.test(text))
      iconFamilies.add("vector-icons");
    ...
    if (
      /useColorScheme|ColorScheme|dark-first|Brightness\.dark|ThemeMode\.dark|ThemeMode\(|dark: ?["']/.test(
        text,
      )
    )
      darkHits++;
    if (
      /reduce-?motion|ReduceMotion|prefers-reduced-motion|disableAnimations|accessibleNavigation|AccessibilityInfo.*reduce|animateWithReducedMotion/i.test(
        text,
      )
    )
      motionHits++;
    if (
      /(Pressable|TouchableOpacity|TouchableHighlight|InkWell|GestureDetector)/.test(text) &&
      !/accessibility(Role|Label|Hint)|Semantics\(|semanticLabel/.test(text)
    ) {
      a11yMissing++;
      if (a11yMissing <= 5) {
        findings.push({
          ...
          check: "accessibility",
          message:
            "Interactive element without an accessibility label (accessibilityLabel / Semantics + semanticLabel) — add labels; keep 44px iOS / 48px Android minimum targets.",
        });
      }
    }
    ...
    if (mock) {
      findings.push({
        ...
        file: r,
        check: "mock-data",
        message: `Mock-data marker "${mock[0]}" — replace with real content or an authentic empty state.`,
      });
  ...
  if (iconFamilies.size >= 3) {
    findings.push({
      level: "major",
      check: "icons",
      message: `Mixed icon families (${[...iconFamilies].join(", ")}) — ${isFlutter ? "keep Material on Android and Cupertino on iOS; do not add a third family" : "one family per app (SF Symbols on iOS)"}.`,
    });
  } else if (iconFamilies.size === 2) {
    findings.push({
      level: "minor",
      check: "icons",
      message: `Two icon families (${[...iconFamilies].join(", ")}) — converge on one.`,
    });
  }
  if (files.length > 0 && darkHits === 0) {
    findings.push({
      ...
      check: "dark-mode",
      message:
        "No dark-mode code detected (useColorScheme/ColorScheme/dark theme) — light-only UI fails the definition of done.",
    });
  }
  if (files.length > 5 && motionHits === 0) {
    findings.push({
      level: "minor",
      check: "motion",
      message: "No reduced-motion handling detected — honor reduce-motion on every animation.",
    });
  }

  // --- web viewport + responsive checks (5 viewport classes) ---
  const pkg = readJsonSafe(path.join(appPath, "package.json")) as
    | Record<string, unknown>
    | undefined;
  const deps = {
  ...
  const isWeb =
    indexHtml !== null ||
    ["vite", "next", "react", "tailwindcss", "@vitejs/plugin-react"].some(
      (k) => deps[k] !== undefined,
    );
  if (isWeb) {
    if (indexHtml !== null && !/<meta[^>]+name=["']viewport["']/i.test(indexHtml)) {
      findings.push({
        ...
        file: "index.html",
        check: "viewports",
        message: 'Missing <meta name="viewport"> — responsive viewports cannot work without it.',
      });
    }
    ...
    for (const file of files) {
      ...
      if (!text) continue;
      if (
        /(sm:|md:|lg:|xl:|2xl:|@media|container-type|useMediaQuery|useWindowDimensions|Dimensions\.get)/.test(
          text,
        )
      ) {
        responsiveHits++;
      ...
      if (fixed && fixedWidthReports < 3) {
        ...
        findings.push({
          ...
          file: rel(file),
          check: "viewports",
          message: `Fixed phone width "${fixed[0]}" — recompose per viewport class instead of locking 375/390px.`,
        });
      }
    }
    if (responsiveHits === 0) {
      findings.push({
        ...
        check: "viewports",
        message:
          "No responsive rules detected (breakpoints, media/container queries, Dimensions) — tablet and landscape will reuse the phone column.",
      });
    }
  }
  ...
  if (!isWeb && isFlutter) {
    ...
    if (flutterIndex !== null && !/<meta[^>]+name=["']viewport["']/i.test(flutterIndex)) {
      findings.push({
        ...
        check: "viewports",
        message:
          'Missing <meta name="viewport"> in the Flutter web entry — responsive viewports cannot work without it.',
      });
    }
  }
  return findings;
...
export const verifyDesignTool = defineTool({
  ...
  execute: async (args, ctx) => {
    ...
    const lines = findings.map(
      (f) => `- [${f.level}] ${f.check}${f.file ? ` (${f.file})` : ""}: ${f.message}`,
    );
