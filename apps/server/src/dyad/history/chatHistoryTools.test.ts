...
function writeSession(id: string, events: Array<Record<string, unknown>>): void {
  const lines = events.map((data, i) =>
    JSON.stringify({ id: `${id}-${i}`, parentUuid: null, sessionId: id, seq: i, time: 1000 + i, type: "harness/event", data }),
  );
...
describe("dyad chat history tools", () => {
  ...
  it("reads chronological pages and around slices", async () => {
    const tool = { type: "tool_call", sessionId: "x", id: "t1", name: "read_file", args: {}, status: "completed" as const };
    writeSession("chat1", [token("first topic"), tool, token("second topic"), tool, token("third topic")]);
    ...
    const around = await executeReadChat({ chat_id: "chat1", around_message_id: 4, limit: 3 }, "current");
  ...
  it("explores with citations or no_match", async () => {
    ...
    const report = await executeExploreChatHistory({ question: "what did we decide about payments?" }, "now");
