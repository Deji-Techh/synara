// FILE: checkpointChain.test.ts
// Purpose: 008-m9b3 gate — checkpoint chain construction, retry-once
// advance, pass prompts, and path predicates (donor test verbatim,
// adapted imports).
...
describe("buildCheckpointChain", () => {
  ...
  it("loads non-empty skill bodies for every pass", () => {
    for (const pass of buildCheckpointChain({
      ...config,
      isNewApp: true,
      hasOnboardingScreens: true,
      hasBackendCode: true,
    })) {
      expect(pass.body.trim().length).toBeGreaterThan(0);
    }
  });
...
describe("buildPassPrompt", () => {
  it("embeds the skill body in a checkpoint directive", () => {
    ...
    if (!first) throw new Error("expected a first pass");
  ...
  it("adds a retry note when requested", () => {
    const pass = advanceChain(createChain(config), true).pass;
    if (!pass) throw new Error("expected a first pass");
    const prompt = buildPassPrompt(pass, { retry: true });
  ...
  it("targets the plan instead of the app when requested", () => {
    const pass = advanceChain(createChain(config), true).pass;
    if (!pass) throw new Error("expected a first pass");
    const prompt = buildPassPrompt(pass, { target: "plan" });
