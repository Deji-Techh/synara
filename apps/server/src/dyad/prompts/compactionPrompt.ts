// FILE: compactionPrompt.ts
// Purpose: Context-compaction summary prompt (verbatim donor text).
// Donor: dyad x caide src/prompts/compaction_system_prompt.ts (verbatim —
// COMPACTION_SYSTEM_PROMPT body byte-identical; only this header added).
...
5. **Preserve plan references.** If an implementation plan was created or discussed (write_plan / <dyad-write-plan>), include its title, its status (accepted / refined / partially implemented), and the remaining steps in the Active section or Next Move.
