// FILE: neonPrompt.ts
// Purpose: Neon available/disconnected prompt builders (donor logic kept;
// guide loading adapted: donor Vite `?raw` imports → fs readGuide, same as
// filter_guide_by_framework's import-line adaptation — builders, templates,
// and decision trees byte-identical).
// Donor: dyad src/prompts/neon_prompt.ts.
...
} from "./neonPromptRules.ts";
import type { AppFrameworkType } from "./frameworkType.ts";
import { readGuide } from "./skillLoader.ts";

const addAuthenticationGuide = () => readGuide("add-authentication");
const addEmailVerificationGuide = () => readGuide("add-email-verification");
const addPasswordResetGuide = () => readGuide("add-password-reset");
