// FILE: platformContracts.ts
// Purpose: Short, behavioral, checkable product contracts injected near the top
// of the build system prompt so the build target (mobile vs web) survives the
// whole session instead of being diluted by a long prompt.
// Donor: dyad x caide src/prompts/platform_contracts.ts (verbatim content,
// import rewired from "@/lib/schemas" to ./appTarget.ts).
// Keep these concise and verifiable — the model audits its own output
