// FILE: agentPrompt.ts
// Purpose: System prompt for Local Agent v2 mode — tool-based agent with
// parallel execution support (Pro / Basic / Ask read-only variants).
// Donor: dyad x caide src/prompts/local_agent_prompt.ts (verbatim blocks and
// constructor; imports rewired from "@/..." to local modules, skill packs to
// fs-loaded ./skillPacks.ts, test guidance to ./testGuidance.ts).
...
function developmentWorkflowBlock({
   ...
   **Mid-task rule:** if ambiguity appears mid-implementation (unclear UX choice, missing asset, conflicting requirements), STOP and ask via \`planning_questionnaire\` instead of guessing. Never guess when a quick question would lock in what the user wants. Always set \`why\` (one sentence per question); if dismissed, proceed with stated defaults — say what you assumed in one line — and never restart completed work or re-ask the same questions.
   The tool accepts ONLY a \`questions\` array (no empty objects). It returns the user's answers as the tool result.
   **Interactive tools beat text, always:** when the user explicitly asks for a questionnaire, a todo list, task tracking, or background delegation, you MUST call \`planning_questionnaire\` / \`update_todos\` / \`spawn_subagent\` — NEVER render questionnaires, checklists, or "I will now delegate" prose as plain chat text. The tool call renders the interactive UI; text fallback is only for when the tool itself is unavailable.`,
