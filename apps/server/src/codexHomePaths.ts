...
export const CAIDE_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveCaideCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.CAIDE_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".caide", "runtime");
  return path.join(overlayRoot, CAIDE_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Caide keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveCaideCodexHomeOverlayPath(env, source);
