 ...
 * fact (e.g. a workspace root created lazily prior to being scaffolded) while
 * still matching what `realpath` will return once the directory exists — which
 * is exactly what stored/reported roots must agree on for downstream
 * classifiers (`isHomeChatContainerProject`, etc.) to compare correctly.
 */
