...
export function assertProjectIdOnly(input: { projectId?: string; workspaceRoot?: string }): ProjectId | never {
  if (input.workspaceRoot !== undefined) {
    throw new WsRpcError({ message: "workspaceRoot string not allowed — use projectId and server will resolve" });
  }
