...
const originalCaideStaticDir = process.env.CAIDE_STATIC_DIR;

function makeTempDir(prefix = "caide-config-test-"): string {
...
afterEach(() => {
  ...
  tempDirs.clear();
  if (originalCaideStaticDir === undefined) {
    delete process.env.CAIDE_STATIC_DIR;
  } else {
    process.env.CAIDE_STATIC_DIR = originalCaideStaticDir;
  }
});

describe("resolveStaticDir", () => {
  it("uses the desktop static snapshot exposed through the Caide environment", async () => {
    const snapshotDir = makeTempDir("caide-static-snapshot-test-");
    fs.writeFileSync(path.join(snapshotDir, "index.html"), "<main>Caide</main>");
    process.env.CAIDE_STATIC_DIR = snapshotDir;
...
describe("resolveDefaultChatWorkspaceRoot", () => {
  it("places the managed chat workspace under Documents/Caide on macOS and Linux", () => {
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Caide");
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Caide");
  });
  ...
  it("uses Windows separators when deriving the managed chat workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Caide");
  });
...
describe("resolveCanonicalWorkspaceRoots", () => {
  ...
  it("canonicalizes the nearest existing ancestor when the workspace root itself does not exist yet", async () => {
    ...
    // Caide/ nor Caide/Studio exist yet underneath it.
