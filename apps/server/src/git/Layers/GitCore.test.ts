  ...
  describe("stashAndCheckout", () => {
    ...
    it.effect("drops only the temporary stash it created after a successful reapply", () =>
      Effect.gen(function* () {
        ...
        expect(stashList).not.toContain("caide: stash before switching to feature");
        expect(yield* readTextFile(path.join(tmp, "README.md"))).toBe("dirty changes\n");
      }),
    );
    ...
    it.effect("keeps the stash when reapplying dirty changes conflicts", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["stash", "list"])).toContain(
          "caide: stash before switching to conflicting",
        );
      }),
    );
  });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "caide/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "caide/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "caide/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "caide/tmp-working",
          newBranch: "caide/feat/session",
        });

        expect(renamed.branch).toBe("caide/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "caide/feat/session")).toBe(true);
        expect(branches.branches.some((branch) => branch.name === "caide/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("caide/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "caide/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "caide/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "caide/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "caide/tmp-working" });
        ...
        expect(renamed.branch).toBe("caide/feat/session-2");
      }),
    );
  ...
  describe("createGitWorktree", () => {
    ...
    it.effect("creates a branch-backed managed worktree when newBranch is provided", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(wtPath, ["symbolic-ref", "--short", "HEAD"])).toBe("caide/abcd1234");
        expect(yield* git(tmp, ["rev-parse", "refs/heads/caide/abcd1234"])).toBe(expectedHead);
        ...
        const remainingBranches = yield* git(tmp, ["branch", "--list", "caide/abcd1234"]);
    ...
    it.effect("does not report branch creation before preliminary validation", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["branch", "--list", "caide/notstarted"])).toBe("");
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const result = yield* Effect.exit(
          core.createDetachedWorktree({
            ...
            ref: "HEAD",
            path: wtPath,
            newBranch: "caide/rollback1",
          }),
        );

        expect(Exit.isFailure(result)).toBe(true);
        expect(yield* git(tmp, ["branch", "--list", "caide/rollback1"])).toBe("");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:example-org/caide.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:example-org/caide.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "caide/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
