...
import { MAX_CHAT_THREAD_TITLE_WORDS } from "@caide/shared/chatThreads";
...
export function buildThreadRecapPrompt(input: {
  ...
  return {
    prompt: [
      "You are writing a compact live recap for Caide's chat side panel.",
...
export function buildAutomationIntentPrompt(input: {
  ...
  return {
    prompt: [
      "You extract structured Caide automation creation intents.",
      ...
      "- Caide automations run a saved prompt on a schedule.",
...
export function buildAutomationCompletionEvaluationPrompt(input: {
  ...
  return {
    prompt: [
      "You evaluate whether a completed Caide heartbeat automation should stop.",
