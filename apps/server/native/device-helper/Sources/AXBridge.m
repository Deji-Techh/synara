...
id CaideEmptyTranslatorResponse(void);
...
static id CaideMsgSend0(id target, NSString *selectorName) {
...
static NSString *CaideStringAttribute(id element, NSString *selectorName) {
  id value = CaideMsgSend0(element, selectorName);
  return [value isKindOfClass:NSString.class] ? value : nil;
}

static BOOL CaideBoolAttribute(id element, NSString *selectorName) {
  SEL selector = NSSelectorFromString(selectorName);
...
static id CaideJSONValue(id value) {
  if (value == nil) {
    return NSNull.null;
  }
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _callbackQueue = dispatch_queue_create("dev.caide.device-helper.ax", DISPATCH_QUEUE_CONCURRENT);
    _requestTimeout = 5.0;
  }
  return self;
}
...
- (id)accessibilityTranslationDelegateBridgeCallbackWithToken:(NSString *)token {
  ...
  id (^callback)(id) = ^id(id request) {
    if (device == nil) {
      return CaideEmptyTranslatorResponse();
    }
    ...
    return response ?: CaideEmptyTranslatorResponse();
  };
...
- (nullable NSDictionary *)frontmostTreeWithMaxDepth:(NSInteger)maxDepth error:(NSError **)error {
  if (_translator == nil) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.caide.device-helper.ax"
                                   code:1
...
- (NSDictionary *)serializeElement:(id)element depth:(NSInteger)depth maxDepth:(NSInteger)maxDepth {
  ...
  NSString *role = CaideStringAttribute(element, @"accessibilityRole");
  ...
  NSString *label = CaideStringAttribute(element, @"accessibilityLabel");
  if (label.length > 0) {
    node[@"label"] = label;
  }
  id value = CaideJSONValue(CaideMsgSend0(element, @"accessibilityValue"));
  if (value != NSNull.null) {
    node[@"value"] = value;
  }
  NSString *identifier = CaideStringAttribute(element, @"accessibilityIdentifier");
  if (identifier.length > 0) {
    node[@"identifier"] = identifier;
  }
  NSString *title = CaideStringAttribute(element, @"accessibilityTitle");
  if (title.length > 0) {
    node[@"title"] = title;
  }
  ...
  NSString *subrole = CaideStringAttribute(element, @"accessibilitySubrole");
  ...
  node[@"enabled"] = @(CaideBoolAttribute(element, @"isAccessibilityEnabled"));
  ...
  id rawChildren = CaideMsgSend0(element, @"accessibilityChildren");
