// caide-device-helper — the native side of Caide's Device Pane.
//
...
final class HelperSession {
  ...
  private(set) var hid: CaideHIDBridge?
  private(set) var accessibility: CaideAXBridge?
  ...
  func requireHID() throws -> CaideHIDBridge {
  ...
  func requireAccessibility() throws -> CaideAXBridge {
...
///
/// Every injection path in CaideHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: CaideHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = CaideHardwareButton.home
    guard CaideHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
...
let requestQueue = DispatchQueue(label: "dev.caide.device-helper.rpc")
