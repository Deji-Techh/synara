...
import { BROWSER_TOOL_NAMES } from "@caide/contracts";
...
test("production MCP controls one persistent Electron page across visibility changes", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "caide-visible-browser-e2e-"));
  ...
  try {
    ...
    await electronApp.evaluate(() => {
      (
        globalThis as typeof globalThis & {
          __caideVisibleBrowserE2E: { setPanelRevealEnabled(enabled: boolean): void };
        }
      ).__caideVisibleBrowserE2E.setPanelRevealEnabled(false);
    });
    ...
    await electronApp.evaluate(() => {
      ...
      ).__caideVisibleBrowserE2E.setPanelRevealEnabled(true);
    });
    ...
    const runtimeCount = await electronApp.evaluate(() => {
      const manager = (
        globalThis as typeof globalThis & {
          __caideVisibleBrowserE2E: { browserManager: { runtimes: Map<string, unknown> } };
        }
