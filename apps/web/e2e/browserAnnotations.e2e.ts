...
import type { BrowserAnnotationEvent, BrowserAnnotationTheme } from "@caide/contracts";
...
test("a real Electron guest commits and reprojects a continuous annotation session", async () => {
  const mainPath = process.env.CAIDE_E2E_ELECTRON_MAIN;
  ...
  const home = mkdtempSync(join(tmpdir(), "caide-browser-annotations-e2e-"));
  ...
  const electronApp = await electron.launch({
    ...
    env: {
      ...
      HOME: home,
      CAIDE_HOME: home,
      CAIDE_BROWSER_HOST_PIPE_PATH: pipePath,
      CAIDE_BROWSER_HOST_CAPABILITY: capability,
      CAIDE_E2E_SHELL_PATH: shellPath,
      CAIDE_E2E_THREAD_ID: threadId,
      CAIDE_E2E_BROWSER_ANNOTATION_PRELOAD: annotationPreloadPath,
    },
  });
  ...
  try {
    ...
    const sendNativeInput = (event: Record<string, unknown>) =>
      electronApp.evaluate(
        (_electron, input) => {
          ...
          ).__caideVisibleBrowserE2E.browserManager;
    ...
    const annotationEvents = async (): Promise<BrowserAnnotationEvent[]> =>
      electronApp.evaluate(() => {
        const fixture = (
          globalThis as typeof globalThis & {
            __caideVisibleBrowserE2E: { annotationEvents: BrowserAnnotationEvent[] };
          }
        ).__caideVisibleBrowserE2E;
