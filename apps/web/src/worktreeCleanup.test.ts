...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.caide/worktrees/caide-mvp/caide-4e609bb8",
    );
    expect(result).toBe("caide-4e609bb8");
  });

  it("keeps legacy .caide worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.caide/worktrees/caide-mvp/caide-legacy123",
    );
    expect(result).toBe("caide-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.caide\\worktrees\\caide-mvp\\caide-4e609bb8",
    );
    expect(result).toBe("caide-4e609bb8");
  });

  it("uses the final segment even when outside ~/.caide/worktrees", () => {
