...
import { isSupportedLocalPreviewFilePath } from "@caide/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
  isWorkspaceRelativePathSafe,
  workspaceRelativePathOf,
} from "@caide/shared/path";
import { isScratchWorkspacePath } from "@caide/shared/threadWorkspace";
...
const CAIDE_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const CAIDE_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveCaidePublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!CAIDE_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${CAIDE_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveCaidePublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
