...
describe("workspacePathsStore", () => {
  ...
  it("keeps chat workspace root while server config is still loading", async () => {
    ...
    useWorkspacePathsStore.getState().setChatWorkspaceRoot("/Users/tester/Documents/Caide");
  ...
  it("keeps studio workspace root while server config is still loading", async () => {
    ...
    useWorkspacePathsStore
      .getState()
      .setStudioWorkspaceRoot("/Users/tester/Documents/Caide/Studio");
    ...
    expect(useWorkspacePathsStore.getState().studioWorkspaceRoot).toBe(
      "/Users/tester/Documents/Caide/Studio",
    );
  });
  ...
  it("migrates cached paths without retaining legacy workspace pages", async () => {
    ...
    localStorage.setItem(
      "caide:workspace-pages:v2",
      JSON.stringify({
        state: {
          homeDir: "/Users/legacy",
          chatWorkspaceRoot: "/Users/legacy/Documents/Caide",
    ...
    expect(localStorage.getItem("caide:workspace-pages:v2")).toBeNull();
    expect(localStorage.getItem("caide:workspace-paths:v1")).not.toBeNull();
  });
});
