...
> = {
  preview: {
    command: "preview",
    label: "/preview",
    description: "Toggle the floating preview stage (M11 visual verification)",
    source: "app",
  },
  build: {
    command: "build",
    label: "/build",
    description: "Build distributable (apk/appbundle/ipa/web per framework)",
    source: "app",
  },
  test: {
    command: "test",
    label: "/test",
    description: "Run test suite with live diagnostics (qualityGate)",
    source: "app",
  },
  analyze: {
    command: "analyze",
    label: "/analyze",
    description: "Run analyze to check for errors and missing imports",
    source: "app",
  },
  plan: {
    command: "plan",
  ...
  review: {
    command: "review",
    label: "/review",
    description: "Trigger comparative benchmark review",
    source: "app",
  },
  fork: {
    command: "fork",
...
export function getAvailableComposerSlashCommands(input: {
  ...
  // Perfect set = 14 only — single list for all providers (Router decides model per role).
