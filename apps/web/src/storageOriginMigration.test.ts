...
describe("storageOriginMigration", () => {
  ...
  it("imports missing keys without overwriting current-origin state", async () => {
    globalThis.localStorage.setItem("caide:theme", "current");
    const { importCaideStorageSnapshot } = await import("./storageOriginMigration");

    expect(
      importCaideStorageSnapshot({
        version: 1,
        exportedAt: "2026-07-09T00:00:00.000Z",
        entries: {
          "caide:theme": "snapshot",
          "caide:composer-drafts:v1": "draft",
        },
      }),
    ).toBe(true);
    expect(globalThis.localStorage.getItem("caide:theme")).toBe("current");
    expect(globalThis.localStorage.getItem("caide:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("rejects an invalid snapshot before writing any entry", async () => {
    ...
    expect(globalThis.localStorage.getItem("caide:theme")).toBeNull();
  });
  ...
  it("imports snapshots containing large composer drafts", async () => {
    ...
    expect(globalThis.localStorage.getItem("caide:composer-drafts:v1")).toBe(largeDraft);
  });
  ...
  it("keeps the snapshot retryable after a partial storage failure", async () => {
    ...
    const snapshot = {
      version: 1 as const,
      exportedAt: "2026-07-09T00:00:00.000Z",
      entries: { "caide:theme": "dark", "caide:composer-drafts:v1": "draft" },
    };

    expect(importCaideStorageSnapshot(snapshot, storage)).toBe(false);
    storage.setItem = setItem;
    expect(importCaideStorageSnapshot(snapshot, storage)).toBe(true);
    expect(storage.getItem("caide:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("acknowledges the desktop snapshot only after a complete bootstrap import", async () => {
    ...
    expect(globalThis.localStorage.getItem("caide:theme")).toBe("dark");
  });
