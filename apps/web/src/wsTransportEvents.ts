...
import type { WsCompatibilityError } from "@caide/contracts";

export type WsTransportState = "connecting" | "open" | "closed" | "incompatible" | "disposed";

export const CAIDE_WS_TRANSPORT_STATE_EVENT = "caide:ws-transport-state";
export const CAIDE_WS_COMPATIBILITY_ISSUE_EVENT = "caide:ws-compatibility-issue";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(CAIDE_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(CAIDE_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(CAIDE_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
...
export function emitWsCompatibilityIssue(issue: WsCompatibilityError | null): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsCompatibilityIssueEventDetail>(CAIDE_WS_COMPATIBILITY_ISSUE_EVENT, {
      detail: { issue },
    }),
  );
}
...
export function addWsCompatibilityIssueListener(
  ...
  window.addEventListener(CAIDE_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  return () => {
    window.removeEventListener(CAIDE_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  };
}
