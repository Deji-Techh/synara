import { MessageId, TurnId, type OrchestrationThreadActivity } from "@caide/contracts";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("keeps created-automation milestones and exposes their card fields despite a null turn id", () => {
    const activities: OrchestrationThreadActivity[] = [
      ...
      makeActivity({
        ...
        summary: "Created automation: Watch Caide PR 231 - Every 5m",
        tone: "info",
        payload: {
          source: "chat-composer",
          automationId: "automation-7",
          automationName: "Watch Caide PR 231",
  ...
  it("exposes a provider-independent Caide thread creation recap", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "caide-created-threads",
        createdAt: "2026-02-23T00:00:05.000Z",
        turnId: "turn-1",
        kind: "caide.threads.created",
        summary: "Created 2 Caide threads",
        tone: "info",
        payload: {
    ...
    expect(entry?.caideThreadCreation).toEqual({
      operationId: "gateway:create:two-workers",
      requestedCount: 2,
      createdCount: 2,
      threads: [
  ...
  it("hides repeated non-adjacent recovery rows for the same provider turn", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        summary: "Caide recovered a stale running state",
        turnId: "turn-stale",
              ...
              commandActions: [
                {
                  ...
                  name: "README.md",
                  path: "/Users/emanueledipietro/Developer/Testing/caide/README.md",
                },
              ],
            },
          },
        ...
        payload: {
          ...
          data: {
            item: {
              ...
              command: "/bin/zsh -lc 'git status --short'",
              cwd: "/Users/emanueledipietro/Developer/Testing/caide",
              status: "completed",
              commandActions: [{ type: "unknown", command: "git status --short" }],
  ...
  it("preserves cancellation when an owning turn aborts", () => {
    const turnId = TurnId.makeUnsafe("turn-with-cancelled-caide-tool");
    const entries = deriveWorkLogEntries(
      [
        makeActivity({
          id: "cancelled-caide-start",
          createdAt: "2026-02-23T00:00:01.000Z",
          kind: "tool.started",
          summary: "Caide create thread",
          turnId,
          payload: {
            itemType: "mcp_tool_call",
            title: "Caide create thread",
            data: {
              toolCallId: "cancelled-caide-call",
      ...
      [
        makeActivity({
          ...
          payload: {
            ...
            status: "interrupted",
            data: {
              toolCallId: "interrupted-caide-call",
              toolName: "mcp__caide__caide_create_thread",
            },
          },
        }),
  ...
  it("presents Caide MCP activity consistently across provider item shapes", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "caide-mcp-create-thread-progress",
        kind: "tool.updated",
        ...
        payload: {
          ...
          title: "MCP tool call",
          data: {
            toolCallId: "caide-mcp-create",
            toolName: "mcp__caide__caide_create_thread",
          },
        },
      }),
      makeActivity({
        id: "caide-dynamic-send-message-progress",
        kind: "tool.updated",
        summary: "Tool call",
        payload: {
          itemType: "dynamic_tool_call",
          title: "Caide__caide_send_message",
          data: {
            toolCallId: "caide-dynamic-send",
          },
        },
      }),
      makeActivity({
        id: "caide-file-change-list-threads-progress",
        kind: "tool.updated",
        summary: "File change",
        payload: {
          itemType: "file_change",
          title: "mcp__Caide__caide_list_threads",
          data: {
            toolCallId: "caide-file-change-list",
          },
        },
      }),
    ];
    ...
    expect(entries.map((entry) => [entry.itemType, entry.toolTitle])).toEqual(
      expect.arrayContaining([
        ["mcp_tool_call", "Caide is creating a thread"],
        ["dynamic_tool_call", "Caide is sending a message"],
        ["file_change", "Caide is listing threads"],
      ]),
    );
    expect(entries).toHaveLength(3);
  });

  it("preserves a failed Caide MCP result as a failed activity sentence", () => {
    const [entry] = deriveWorkLogEntries(
      [
        makeActivity({
          id: "caide-create-threads-failed",
          kind: "tool.completed",
          summary: "caide__caide_create_threads",
          payload: {
            itemType: "mcp_tool_call",
            status: "failed",
            data: {
              toolCallId: "caide-create-failed",
    ...
    expect(entry).toMatchObject({
      toolStatus: "failed",
      toolTitle: "Caide couldn't create threads",
