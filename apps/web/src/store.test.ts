...
describe("store facade", () => {
  ...
  it("renames a project locally without changing its remote or folder names", () => {
    ...
    const next = renameProjectLocally(state, ProjectId.makeUnsafe("project-1"), "caide");
  ...
  it("persists project aliases immediately when the local alias changes", async () => {
    ...
    try {
      ...
      freshStore.useStore.getState().renameProjectLocally(projectId, "caide");

      expect(setItem).toHaveBeenCalled();
      expect(JSON.parse(storage.get("caide:renderer-state:v8") ?? "{}")).toMatchObject({
        projectNamesByCwd: {
          "/tmp/project": "caide",
        },
      });
