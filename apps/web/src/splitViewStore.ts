...
import { type ProjectId, type ThreadId, type TurnId } from "@caide/contracts";
...
// Keep the v1 suffix stable while using the Caide namespace; legacy
// `caide:*` and `caide:*` keys are copied over by
// `storageKeyMigration` before this store hydrates, so older payloads still
// flow through the v1 -> v2 schema migration below.
const SPLIT_VIEW_STORAGE_KEY = "caide:split-view-state:v1";
