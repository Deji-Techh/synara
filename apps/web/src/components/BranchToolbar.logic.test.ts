import type { GitBranch } from "@caide/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.caide/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.caide/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .caide worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.caide/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/caide-pi",
        existingAssociatedWorktreeBranch: "caide/pi",
        existingAssociatedWorktreeRef: "caide/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/caide-pi",
      associatedWorktreeBranch: "caide/pi",
      associatedWorktreeRef: "caide/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.caide/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.caide/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.caide/worktrees/feature-b",
      nextWorktreePath: "/repo/.caide/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.caide/worktrees/feature-a",
      nextWorktreePath: "/repo/.caide/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
