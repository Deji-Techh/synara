...
export function buildExternalMcpClientConfiguration(
  ...
  if (client === "codex") {
    ...
    return {
      ...
      instruction: /win/i.test(platform)
        ? "Run this command in PowerShell. Codex will save Caide as a local MCP server; then open a new Codex task."
        : "Run this command in Terminal. Codex will save Caide as a local MCP server; then open a new Codex task.",
    };
  }
  ...
  if (client === "claudeCode") {
    ...
    return {
      ...
      instruction: /win/i.test(platform)
        ? "Run this command in PowerShell. Claude Code will make Caide available in all your projects."
        : "Run this command in Terminal. Claude Code will make Caide available in all your projects.",
    };
  }
  ...
  return {
    ...
    instruction:
      client === "claudeDesktop"
        ? "In Claude Desktop, open Settings → Developer → Edit Config. Add the Caide entry without removing existing servers, save, and restart Claude Desktop."
...
export function buildExternalMcpExamplePrompt(projectTitle: string | null): string {
  return [
    projectTitle === null
      ? "Use Caide to create a new task: call caide_overview first, pick the most relevant project, and tell me which one you chose."
      : `Use Caide to create a new task in the project named ${JSON.stringify(projectTitle)}.`,
    "First inspect Caide's capabilities and choose an exact available provider and model; do not guess model names.",
...
// another MCP-capable app). The agent pairs the machine, registers Caide in
...
export function buildExternalMcpSetupPrompt(input: {
  ...
  sections.push(
    [
      'Step 2 — Register Caide as a stdio MCP server named "caide" in your own configuration, using whichever mechanism your app supports:',
      "",
    ...
    'Step 3 — Verify. Reload your MCP servers if needed, then call the "caide_overview" tool and summarize the projects, providers, and permissions it returns.',
  );
