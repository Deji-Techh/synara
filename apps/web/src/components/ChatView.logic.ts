...
import { normalizeModelSlug } from "@caide/shared/model";
import { buildCaideBranchName } from "@caide/shared/git";
import { isGenericChatThreadTitle } from "@caide/shared/chatThreads";
import { isGenericTerminalThreadTitle } from "@caide/shared/terminalThreads";
import {
  type ChatMessage,
  type SessionPhase,
  type Thread,
...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "caide:last-invoked-script-by-project";
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "caide:dismissed-provider-health-banners";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for Caide, then try again.";
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildCaideBranchName(input.associatedWorktreeBranch ?? input.title);
}
