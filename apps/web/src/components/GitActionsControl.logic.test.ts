import type { GitStatusResult } from "@caide/contracts";
...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Caide as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "caide/fix-toast-copy");
  });

  it("normalizes an existing legacy caide namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "caide/refine-toolbar-actions");
    assert.equal(branch, "caide/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Caide", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "caide/feature/refine-toolbar-actions");
  });
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "caide/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "caide/deadbeef";
