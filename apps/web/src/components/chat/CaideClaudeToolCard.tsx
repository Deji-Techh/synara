...
function getToolMeta(name: string, attrs: Record<string, string>): ToolMeta {
  ...
  if (
    clean.includes("search replace") ||
    clean.includes("multi replace") ||
    clean.includes("edit")
  ) {
    return {
      verb: "Edit",
  ...
  if (clean.includes("rename") || clean.includes("copy file") || clean.includes("copy")) {
    return {
      verb: clean.includes("rename") ? "Rename" : "Copy",
      target: attrs.path || attrs.file || "",
      icon: <IconFileCode size={14} />,
      accent: "success",
    };
  ...
  if (
    ...
    clean.includes("lint") ||
    clean.includes("test") ||
    clean.includes("typecheck") ||
    clean.includes("type check")
  ) {
    return {
  ...
  if (
    ...
    clean.includes("grep") ||
    clean.includes("code search") ||
    clean.includes("explore")
  ) {
    return {
      verb: "Search",
      target: attrs.query || attrs.pattern || attrs.path || "",
  ...
  return (
    <div className="my-0.5 select-none text-left">
      ...
      >
        ...
        <DisclosureChevron
          open={isExpanded}
          className="size-2.5 text-muted-foreground/40 group-hover:text-foreground/60"
        />
      </button>
