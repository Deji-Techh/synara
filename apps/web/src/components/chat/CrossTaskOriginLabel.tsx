...
// Purpose: Identify the source thread for conversations created by another Caide agent.
// Layer: Chat transcript UI

import { type ProviderKind, type ThreadId } from "@caide/contracts";
...
// Caide thread, so it always reads as "Sent by Caide" with the Caide mark
// (the origin provider is not surfaced here to keep one consistent label).
function OriginContent(): ReactNode {
  return (
    <>
      <span className="flex size-4 shrink-0 items-center justify-center text-muted-foreground/70">
        <CaideLogo className="h-4 w-auto" aria-label="Caide" />
      </span>
      <span className="truncate">Sent by Caide from another thread</span>
    </>
  );
}
