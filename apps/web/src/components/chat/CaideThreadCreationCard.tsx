// FILE: CaideThreadCreationCard.tsx
// Purpose: End-of-turn recap for threads created through the Caide MCP harness.
// Layer: Chat transcript UI

import { PROVIDER_DISPLAY_NAMES } from "@caide/contracts";
import { formatModelDisplayName } from "@caide/shared/model";
import { memo } from "react";

import type { WorkLogCaideThreadCreation } from "../../session-logic";
import { ProviderIcon } from "../ProviderIcon";
import { CaideLogo } from "../CaideLogo";
import { Button } from "../ui/button";

function threadMeta(thread: WorkLogCaideThreadCreation["threads"][number]): string {
...
export const CaideThreadCreationCard = memo(function CaideThreadCreationCard({
  creation,
  onOpenThread,
}: {
  readonly creation: WorkLogCaideThreadCreation;
  ...
  return (
    <div
      ...
      data-caide-thread-creation-card="true"
    >
      <div className="flex min-w-0 items-center gap-3 px-3 py-2.5">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[var(--color-background-elevated-secondary)] text-foreground">
          <CaideLogo className="h-[22px] w-auto" aria-label="Caide" />
        </span>
