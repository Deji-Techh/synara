...
import { describeErrorMessage } from "@caide/shared/errorMessages";
import {
  TERMINAL_MAX_COLS,
  TERMINAL_MAX_ROWS,
  TERMINAL_MIN_COLS,
...
import type { TerminalSessionSnapshot } from "@caide/contracts";
...
const TERMINAL_PARKING_CONTAINER_ID = "caide-terminal-parking";

type CaideTerminalOptions = NonNullable<ConstructorParameters<typeof Terminal>[0]> & {
...
const TERMINAL_CURSOR_STYLE: NonNullable<CaideTerminalOptions["cursorStyle"]> = "bar";
const TERMINAL_INACTIVE_CURSOR_STYLE: NonNullable<CaideTerminalOptions["cursorInactiveStyle"]> =
  "bar";
...
function syncTheme(entry: TerminalRuntimeEntry): void {
  ...
  const terminalOptions = entry.terminal.options as CaideTerminalOptions;
...
export function createRuntimeEntry(config: TerminalRuntimeConfig): TerminalRuntimeEntry {
  ...
  const terminalOptions: CaideTerminalOptions = {
    cursorBlink: true,
