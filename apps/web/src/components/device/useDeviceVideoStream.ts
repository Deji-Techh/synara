...
import type { DeviceUdid } from "@caide/contracts";
import type { DeviceFrame } from "@caide/shared/deviceFrame";
...
export function useDeviceVideoStream(input: {
  ...
  useEffect(() => {
    ...
    const configureDecoder = (frame: DeviceFrame) => {
      ...
      if (!codec) {
        failStream("The simulator stream sent parameters Caide could not read.");
        return;
      }
      teardownDecoder();
      ...
      setStatus({
        ...
        message:
          reason === "decode-failed"
            ? "The simulator stream sent a frame Caide could not read."
