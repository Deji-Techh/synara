...
describe("splitViewStore", () => {
  ...
  it("keeps writing split views to the Caide v1 storage key so persisted state can migrate", async () => {
    vi.resetModules();
    ...
    const persisted = globalThis.localStorage.getItem("caide:split-view-state:v1");
    expect(persisted).not.toBeNull();
    expect(globalThis.localStorage.getItem("caide:split-view-state:v2")).toBeNull();
