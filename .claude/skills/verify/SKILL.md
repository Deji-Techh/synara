# Verify: run Caide locally for runtime verification

How to launch an isolated Caide instance (server + web) to observe UI changes, without touching `~/.caide` or the default dev ports.

## Launch

```bash
# 1. Server (from the directory you want as the workspace/project cwd):
CAIDE_HOME=<scratch>/caide-home \
CAIDE_PORT=3899 CAIDE_MODE=web CAIDE_NO_BROWSER=1 \
