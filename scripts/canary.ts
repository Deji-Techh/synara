// FILE: canary.ts
// Purpose: Maintains and launches an isolated, frozen Caide Canary checkout.
...
export function resolveCanaryPaths(
  ...
  const home = Path.resolve(
    env.CAIDE_CANARY_HOME?.trim() || Path.join(homeDirectory, ".caide-canary"),
  );
  const cacheBase = env.XDG_CACHE_HOME?.trim() || Path.join(homeDirectory, ".cache");
  const source = Path.resolve(
    env.CAIDE_CANARY_SOURCE?.trim() || Path.join(cacheBase, "caide-canary", "source"),
  );
  return {
    home,
    source,
...
function assertManagedSourceIsClean(paths: CanaryPaths): void {
  ...
  if (status.length > 0) {
    throw new Error(
      `Caide Canary's managed source has tracked local changes. Refusing to overwrite ${paths.source}.`,
    );
  }
}
...
function startCanary(paths: CanaryPaths): void {
  ...
  if (existingPid !== null && isRunning(existingPid)) {
    console.log(`Caide Canary is already running (pid ${String(existingPid)}).`);
    return;
  }
  ...
  if (
    ...
    throw new Error("Caide Canary is not built. Run `bun run canary:setup` first.");
  }
  FS.mkdirSync(paths.home, { recursive: true });
  ...
  Object.assign(env, {
    CAIDE_DESKTOP_FLAVOR: "canary",
    CAIDE_DISABLE_AUTO_UPDATE: "1",
    CAIDE_HOME: paths.home,
    CAIDE_COMMIT_HASH: commit,
  });
  ...
  try {
    ...
    if (child.pid === undefined) {
      throw new Error("Caide Canary failed to return a process id.");
    }
    child.unref();
    FS.writeFileSync(paths.pid, `${String(child.pid)}\n`, { mode: 0o600 });
    console.log(`Started Caide Canary at ${commit.slice(0, 12)} (pid ${String(child.pid)}).`);
...
function rollbackCanary(paths: CanaryPaths): void {
  ...
  if (state?.previousCommit === null || state?.previousCommit === undefined) {
    throw new Error("Caide Canary has no previous successful commit to restore.");
  }
  assertManagedSourceIsClean(paths);
  stopCanary(paths);
...
function printStatus(paths: CanaryPaths): void {
  ...
  console.log(`Caide Canary: ${running ? `running (pid ${String(pid)})` : "stopped"}`);
