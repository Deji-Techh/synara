...
describe("canary tooling", () => {
  it("keeps managed source and Canary data separate from Stable", () => {
    expect(resolveCanaryPaths({}, "/Users/tester")).toEqual({
      home: "/Users/tester/.caide-canary",
      source: "/Users/tester/.cache/caide-canary/source",
      state: "/Users/tester/.caide-canary/canary-state.json",
      pid: "/Users/tester/.caide-canary/canary.pid",
      log: "/Users/tester/.caide-canary/canary.log",
    });
  });
  ...
  it("supports explicit path overrides", () => {
    expect(
      resolveCanaryPaths(
        {
          CAIDE_CANARY_HOME: "/tmp/canary-data",
          CAIDE_CANARY_SOURCE: "/tmp/canary-source",
        },
        "/Users/tester",
      ),
    ).toEqual({
  ...
  it("tracks main by default and accepts a stacked PR ref", () => {
    ...
    expect(parseCanaryArgs(["setup", "--ref", "codex/caide-canary"])).toEqual({
      command: "setup",
      ref: "codex/caide-canary",
    });
  });

  it("checks out the managed source during clone so the cleanliness guard starts clean", () => {
    expect(canaryCloneArgs("git@example.com:caide.git", "/tmp/canary-source")).toEqual([
      "clone",
      "--",
      "git@example.com:caide.git",
      "/tmp/canary-source",
    ]);
  });
  ...
  it("keeps updating the selected stacked ref until explicitly moved to main", () => {
    ...
    expect(resolveCanaryRef(parseCanaryArgs(["update"]), "codex/caide-canary")).toBe(
      "codex/caide-canary",
    );
