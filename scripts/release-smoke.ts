...
files:
  - url: Caide-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
path: Caide-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
`,
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: Caide-9.9.9-smoke.0-x64.zip
    sha512: x64zip
    size: 132000112
path: Caide-9.9.9-smoke.0-x64.zip
sha512: x64zip
...
function verifyCanonicalIdentity(): void {
  ...
  if (serverPackage.name !== "@caide/cli") {
    throw new Error(`Expected CLI package @caide/cli, got ${serverPackage.name ?? "<missing>"}.`);
  }
  const expectedBinaries = {
    caide: "dist/index.mjs",
    "caide-restore-migration-backup": "dist/restoreMigrationBackup.mjs",
  };
  if (JSON.stringify(serverPackage.bin ?? {}) !== JSON.stringify(expectedBinaries)) {
    throw new Error(
      "Expected the CLI to expose only the Caide entry point and migration recovery binary.",
    );
  }
  if (CAIDE_PRODUCTION_BUNDLE_ID !== "com.emanueledipietro.caide") {
    throw new Error(`Unexpected production bundle ID: ${CAIDE_PRODUCTION_BUNDLE_ID}.`);
  }
  if (CAIDE_DESKTOP_UPDATE_CHANNEL !== "caide") {
    throw new Error(`Unexpected desktop update channel: ${CAIDE_DESKTOP_UPDATE_CHANNEL}.`);
  }
  ...
  if (
    ...
    throw new Error("Expected stable clean Caide releases to publish on GitHub Latest.");
  }
}
...
function verifyReleaseWorkflowSafety(): void {
  ...
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.CAIDE_PUBLISH_CLI == '1'",
    "Expected CLI publication to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.CAIDE_FINALIZE_RELEASE == '1'",
    "Expected release finalization to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "CAIDE_PUBLISH_RELEASE: ${{ needs.preflight.outputs.publish_release }}",
  ...
  assertContains(
    desktopBuildConfig,
    "__CAIDE_WINDOWS_UPDATER_PUBLISHER__",
...
const tempRoot = mkdtempSync(join(tmpdir(), "caide-release-smoke-"));
...
try {
  ...
  assertContains(
    mergedManifest,
    "Caide-9.9.9-smoke.0-arm64.zip",
    "Merged manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedManifest,
    "Caide-9.9.9-smoke.0-x64.zip",
