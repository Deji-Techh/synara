...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Caide.app"), [
      "Caide.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Caide.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Caide.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Caide.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Caide.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Caide.app/Contents/Info.plist",
        "Caide.app/Contents/Info.plist",
        "Caide.app/Contents/MacOS/Caide",
      ]),
      "Caide.app",
    );
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    assert.equal(
      resolveSingleMacUpdateZipFileName([
        "Caide-0.1.5-arm64.dmg",
        "Caide-0.1.5-arm64.zip",
        "latest-mac.yml",
      ]),
      "Caide-0.1.5-arm64.zip",
    );
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Caide-0.1.5-arm64.zip", "Caide-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Caide-0.1.5-arm64.dmg"]),
...
files:
  - url: Caide-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Caide-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Caide-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Caide-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Caide-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Caide-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Caide-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Caide-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Caide-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Caide-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
