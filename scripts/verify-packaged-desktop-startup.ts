...
function prepareWindowsLaunch(assetsDirectory: string, extractionRoot: string): LaunchCommand {
  ...
  const executables = findFiles(applicationRoot, (candidate) =>
    /[/\\]Caide\.exe$/i.test(candidate),
  );
  if (executables.length !== 1) {
    throw new Error(`Expected one extracted Caide.exe, found ${executables.length}.`);
  }
...
export function createPackagedDesktopSmokeEnvironment(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    CAIDE_HOME: join(root, "caide-home"),
  ...
  if (options.platform === "mac") {
    const userDataPath = join(env.HOME!, "Library", "Application Support", "caide");
...
export async function verifyPackagedDesktopStartup(
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), `caide-packaged-smoke-${options.platform}-`));
  ...
  try {
    ...
    const logPath = join(env.CAIDE_HOME!, "userdata", "logs", "desktop-main.log");
