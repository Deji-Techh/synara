...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit CAIDE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "CAIDE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid CAIDE_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("boolean precedence", () => {
    ...
    it.effect("rejects invalid boolean environment values", () =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(
          readDevRunnerBooleanEnvironment({ CAIDE_LOG_WS_EVENTS: "sometimes" }),
        );
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("marks an inherited terminal PATH as already hydrated", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CAIDE_PATH_HYDRATED, "1");
        assert.match(env.PATH ?? "", /\/opt\/homebrew\/bin/);
      }),
    );

    it.effect("defaults CAIDE_HOME to ~/.caide when not provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.CAIDE_HOME, resolve(homedir(), ".caide"));
        assert.equal(env.CAIDE_HOST, "127.0.0.1");
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CAIDE_HOME, resolve("/tmp/custom-caide"));
        assert.equal(env.CAIDE_PORT, "4222");
        assert.equal(env.CAIDE_NO_BROWSER, "1");
        assert.equal(env.CAIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.CAIDE_LOG_WS_EVENTS, "1");
        assert.equal(env.CAIDE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            CAIDE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          caideHome: undefined,
        ...
        assert.equal(env.CAIDE_MODE, "web");
        assert.equal(env.CAIDE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CAIDE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom caideHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.CAIDE_HOME, resolve("/tmp/my-caide"));
        assert.equal(env.CAIDE_HOME, resolve("/tmp/my-caide"));
        assert.equal(env.CAIDE_HOME, resolve("/tmp/my-caide"));
      }),
    );
