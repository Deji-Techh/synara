...
export const DEFAULT_CAIDE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".caide"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@caide/contracts",
    "--filter=@caide/web",
    "--filter=@caide/cli",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=@caide/cli"],
  "dev:web": ["run", "dev", "--filter=@caide/web"],
  "dev:desktop": ["run", "dev", "--filter=@caide/desktop", "--filter=@caide/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("CAIDE_PORT_OFFSET"),
  devInstance: optionalStringConfig("CAIDE_DEV_INSTANCE"),
});
const HomeConfig = optionalStringConfig("CAIDE_HOME");
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid CAIDE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `CAIDE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric CAIDE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed CAIDE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_CAIDE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly caideHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(caideHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      CAIDE_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://${formattedClientHost}:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      CAIDE_HOME: resolvedBaseDir,
      CAIDE_HOST: serverHost,
    };
    ...
    if (authToken !== undefined) {
      output.CAIDE_AUTH_TOKEN = authToken;
    } else {
      delete output.CAIDE_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.CAIDE_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.CAIDE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.CAIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.CAIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.CAIDE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.CAIDE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.CAIDE_MODE = "web";
      delete output.CAIDE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read CAIDE_PORT_OFFSET/CAIDE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.CAIDE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.CAIDE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  caideHome: Flag.string("home-dir").pipe(
  ...
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to CAIDE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("CAIDE_AUTH_TOKEN")),
  ),
  noBrowser: optionalBooleanFlag("no-browser", {
    description: "Disable browser auto-open (equivalent to CAIDE_NO_BROWSER).",
  ...
  autoBootstrapProjectFromCwd: optionalBooleanFlag("auto-bootstrap-project-from-cwd", {
    description:
      "Enable project auto-bootstrap (equivalent to CAIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
  }),
  logWebSocketEvents: optionalBooleanFlag("log-websocket-events", {
    description: "Enable WebSocket event logging (equivalent to CAIDE_LOG_WS_EVENTS).",
    aliases: ["log-ws-events"],
  }),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to CAIDE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("CAIDE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to CAIDE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("CAIDE_PORT")),
  ),
