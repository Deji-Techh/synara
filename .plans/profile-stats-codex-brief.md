...
You are GPT-5.5 (xhigh) working in the Caide monorepo at `/Users/emanueledipietro/Developer/caide`.
...
- BRANCH dev DB (what the dev instance uses): `./.caide/electron-dev/dev/state.sqlite`  (~40 turns)
- REAL app DB: `~/.caide/userdata/state.sqlite`  (~437 turns, 192 MB) — the important one
...
2. `npx turbo run typecheck --filter=@caide/contracts --filter=@caide/cli` must pass. DO NOT run the web
   (`@caide/web`) typecheck — its UI is being rewritten in parallel and will be red until Opus finishes.
