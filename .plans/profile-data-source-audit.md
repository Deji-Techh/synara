...
You are GPT-5.5 (xhigh) in the Caide monorepo at `/Users/emanueledipietro/Developer/caide`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `CAIDE_HOME` (dev: `./.caide/electron-dev`; prod: `~/.caide` or
  `~/Documents/Caide`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `CAIDE_HOME/CAIDE_HOME/CAIDE_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.caide/electron-dev/dev/state.sqlite` (dev) and `~/.caide/userdata/state.sqlite`
     (prod) where relevant.
