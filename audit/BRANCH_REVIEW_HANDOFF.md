# Caide Code-Quality Branch — Complete Review Handoff
...
- Effect remains in Caide for lifecycle, cancellation, queues, errors, and application policy.
...
| Commit              | Summary                                      |                                         Size |
| ------------------- | -------------------------------------------- | -------------------------------------------: |
| `0e5f6af9c`         | Refactor Caide orchestration and web UI flow |                  387 files, +43,323 / -9,299 |
...
AcpSessionRuntime
  ...
  └─ Caide: lifecycle, bounds, logging, policy, normalized events
        │
        ▼
...
| Workstream                              | Why it was needed                                                                   | Main change                                                             | Result to verify                                                       |
| --------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `P2-ACP-01` ACP foundation              | Caide maintained a custom wire stack beside the official SDK                        | Official SDK owns all production ACP wire behavior; custom wire deleted | Grok, Droid, and Cursor have one wire implementation and no fallback   |
...
Use the official TypeScript SDK for the protocol wire, while keeping Effect for Caide's application
lifecycle.
...
| Provider process supervision                  | Caide/Effect           |
| Queue and resource limits                     | Caide/Effect           |
| Session/product policy                        | Caide/Effect           |
| Normalized Caide events                       | Caide/Effect           |
| Error translation into Caide domain errors    | Thin local adapter     |
...
3. Provider adapters should depend on a stable Caide runtime interface, not choose a protocol
   implementation.
...
The package currently retains only generated/schema/error adapters still imported by Caide. A
...
SQLite persistence, or the full Caide app.
...
```text
Review the Caide branch `codex/audit-code-quality` as a senior architecture, correctness,
