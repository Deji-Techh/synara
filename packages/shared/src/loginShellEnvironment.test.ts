...
function makeTemporaryRoot(): string {
  const root = FS.mkdtempSync(Path.join(OS.tmpdir(), "caide-login-shell-"));
...
describe("createCachedLoginShellEnvironmentReader", () => {
  ...
  it("runs uncached when the home directory cannot be resolved", () => {
    ...
    // call-count assertion below) instead of touching the developer's real Caide home.
    const env = { CAIDE_HOME: Path.dirname(Path.dirname(fixture.cachePath)) };
...
describe("loginShellEnvironmentCachePath", () => {
  it("anchors the cache in the Caide home both processes resolve", () => {
    expect(loginShellEnvironmentCachePath({ env: { CAIDE_HOME: "/tmp/caide-home" } })).toBe(
      Path.join("/tmp/caide-home", "cache", "login-shell-environment.json"),
    );
    expect(loginShellEnvironmentCachePath({ env: {}, homeDirectory: "/users/test" })).toBe(
      Path.join("/users/test", ".caide", "cache", "login-shell-environment.json"),
    );
  });
});
