 ...
 */
export const WORKTREE_BRANCH_PREFIX = "caide";
...
const CAIDE_BRANCH_FALLBACK = "update";
...
export function buildCaideBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || CAIDE_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueCaideBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildCaideBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
