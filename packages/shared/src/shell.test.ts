...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__CAIDE_ENV_PATH_START__\n/a:/b\n__CAIDE_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__CAIDE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__CAIDE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__CAIDE_ENV_PATH_END__",
        "__CAIDE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__CAIDE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      ["__CAIDE_ENV_CUSTOM_VAR_START__", "  padded value  ", "__CAIDE_ENV_CUSTOM_VAR_END__"].join(
        "\n",
      ),
    );
