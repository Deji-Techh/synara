...
import type { ServerLocalServerProcess } from "@caide/contracts";
...
describe("localServerPrimaryLabel", () => {
  it("prefers the live page title when one was resolved", () => {
    expect(localServerPrimaryLabel(makeServer({ pageTitle: "Caide", displayName: "Vite" }))).toBe(
      "Caide",
    );
  });
...
describe("localServerFolderLabel", () => {
  it("returns the final segment of a POSIX cwd", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/caide-website" }))).toBe(
      "caide-website",
    );
  });

  it("ignores a trailing separator", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/caide/" }))).toBe("caide");
  });
