...
- [x] Scope transcript offsets, current turns, terminal settlement, tool
      activity, approvals, questions, consent, todos, work logs, goals, and MCP
      events to their owning project/thread/chat/turn. (EngineAdapter: transcriptKey turnId::messageId, chatSettlementKey chatId::turnId, assertChatOwnership per notification a322c167)
...
- [x] Verify persisted engine chat mappings against their owning app/project
      during restart and create a fresh chat when no valid mapping exists. (ensureThreadChat verifies get-chats under appId a322c167)
...
- [x] Test two projects with unique workspaces and chats: activity in either
      project emits no events or messages into the other. (EngineAdapter.isolation.test.ts 8 tests a322c167)
- [x] Test two chats in one project: each has its own engine chat, streamed
      transcript, current turn, tools, interactions, and terminal settlement.
- [x] Test restart restoration without chat ID reuse or first-chat fallback.
