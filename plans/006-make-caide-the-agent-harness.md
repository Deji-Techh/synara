# Plan 006: Make Caide the Authoritative Agent Harness
...
An explicit request to start two Caide threads produced six threads. The
...
`caide_create_thread`, tried several invalid provider/model combinations, and
...
1. **Model awareness** — every provider is told, through its strongest native
   instruction channel, that Caide is the hosting harness and that Caide
   resources are controlled through `caide_*` tools.
2. **Capability truth** — agents can ask Caide which providers, models,
...
```text
user asks for exactly two Caide threads
  -> parent reads caide_context / caide_capabilities
  -> parent calls caide_create_threads once with exactly two entries
  ...
  -> parent calls caide_wait_for_threads until both runs are terminal
...
- Caide is the harness; provider-native subagents are implementation details
  inside a provider turn and never substitute for user-requested Caide
  threads.
- All model-facing host text comes from one versioned
  `CaideHarnessPolicy`. Provider adapters only choose the delivery mechanism.
- Every state-changing gateway call is bound to a live Caide provider session
...
- Unsupported provider/model/options fail before `thread.create`, git branch,
  worktree, or provider side effects. Caide never interprets `Low` as part of
...
- `apps/server/src/agentGateway/Layers/AgentGateway.ts:73-81` owns the only full
  “Caide is hosting this session / use caide\_\*” policy, but
...
- `apps/server/src/provider/Layers/ClaudeAdapter.ts:880-885` tells Claude it is
  running inside Caide, but does not define Caide-thread versus native
  subagent precedence.
- `apps/server/src/codexAppServerManager.ts:475-486,620-648` sends Codex
  ...
  include the Caide harness contract.
- Cursor, Grok, and Droid receive MCP transport configuration and a
  Caide `clientInfo`, but no shared model-facing host policy. See
...
- `apps/server/src/provider/runtimeLayer.ts:54-70` explicitly excludes OpenCode
  ...
  and a clear “Caide control unavailable” capability instead of pretending
  parity.
...
- A background native agent that retains the same MCP connection can therefore
  mutate Caide after the visible parent turn has completed.
...
- predecessor naming — Caide identity must remain consistent;
...
### `caide_context` (read-only, idempotent)

```ts
type CaideContextResult = {
  harness: { name: "Caide"; policyVersion: string };
  caller: {
    threadId: ThreadId;
    turnId: TurnId | null;
...
### `caide_capabilities` (read-only, idempotent)
...
### `caide_create_threads` (destructive, idempotent with request id)

```ts
type CaideCreateThreadsInput = {
...
Keep `caide_create_thread` as a backwards-compatible single-entry wrapper
...
### `caide_wait_for_threads` (read-only, idempotent)

```ts
type CaideWaitForThreadsInput = {
...
- Rebranding or renaming `caide_*` tools.
- Disabling provider-native subagents globally. They remain available for
  ...
  explicit Caide-resource requests.
...
- UI redesign. Existing Caide MCP logo/labels may consume tool annotations,
...
- Branch from the current feature branch with `codex/caide-harness-control-plane`.
...
### Step 2: Centralize and deliver the Caide harness policy

Create a pure `harnessPolicy.ts` exporting:

- `CAIDE_HARNESS_POLICY_VERSION`;
...
1. “You are running inside Caide. Caide is the host/harness for this
   session.”
2. Use `caide_*` for Caide threads, projects, automations, and coordination.
3. Provider-native `spawn_subagent`/Task/collaboration tools do not create
   Caide threads and must not substitute for an explicit request for Caide
   threads.
4. Use one `caide_create_threads` call for plural requests; the array count is
   exact.
5. Resolve canonical models/options through `caide_capabilities`; do not guess
   slugs or silently change provider/model.
6. Wait with `caide_wait_for_threads`, then synthesize all results.
...
- **Claude**: append it to the existing Claude system-prompt append. Add MCP
  ...
  list.
- **Cursor/Grok/Droid ACP paths**: include consistent Caide clientInfo
  ...
  user-visible Caide message projection.
- **OpenCode/Kilo**: deliver truthful host identity, but advertise Caide MCP
...
- **Pi**: deliver host identity through its supported prompt/extension path and
  explicitly advertise that Caide MCP control is unavailable. Do not simulate
  successful Caide actions.

Keep MCP initialize `instructions` generated from the same policy. Add tool
annotations and make every description begin with its user-facing Caide
...
- caller thread id;
- a Caide-generated provider-session key (not an upstream provider id);
- provider kind;
...
- return the same catalog from `caide_capabilities` that validation uses;
...
Implement `caide_wait_for_threads` as a read-only long poll over projections:
...
This cap protects Caide projections from provider fan-out, but it does not
disable native subagent execution. Keep native-child budgeting separate from
the ordinary top-level `caide_create_threads` operation.
...
- the active parent path uses one two-entry Caide batch;
...
- [x] Every provider receives the same versioned Caide host identity through
      its strongest available instruction channel.
- [x] Every provider truthfully reports whether it can call Caide MCP; no
      provider simulates unsupported host actions.
- [x] MCP initialize identity, tool descriptions, and native host instructions
      all say Caide, never a predecessor or provider-branded harness.
- [x] Every destructive call requires a live scoped provider session and active
      caller turn.
- [x] A detached background agent cannot create or mutate Caide threads after
...
- `CaideHarnessPolicy` is the single source of truth. Any new provider must add
